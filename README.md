...
As of **2026-08-17** (all figures on the **`name_check` ruler**):

| Area | Functions matched | Code bytes | Progress |
|---|---:|---:|:---|
| **Whole binary** | **44,444 / 69,227 (64.2%)** | **36.08%** | `█████████████░░░░░░░` |
| Game code (`src/band3/`) | 16,694 / 19,282 (86.6%) | 64.15% | `█████████████████░░░` |
| Milo engine (`src/system/`) | 27,058 / 33,543 (80.7%) | 56.09% | `████████████████░░░░` |
| Quazal network (`src/network/`) | 446 / 1,549 (28.8%) | 11.68% | `██████░░░░░░░░░░░░░░` |
| Third-party libs (vendored under `src/system/`) | 238 / 284 (83.8%) | 89.23% | `█████████████████░░░` |
| Xbox 360 SDK (`src/xdk/`)² | 8 / 4,468 (0.2%) | 0.02% | `░░░░░░░░░░░░░░░░░░░░` |
| Not yet attributed¹ | 0 / 10,101 | — | `░░░░░░░░░░░░░░░░░░░░` |
<!-- progress-table:end -->

*(Table hand-refreshed 2026-08-17 from the report regenerated at HEAD
`6e13ee3f`; `tools/update_readme_progress.py` reproduces it from a build tree.)*

¹ Functions not yet mapped to a source translation unit — CRT and unattributed
vendor middleware (RAD BINK, Quazal, and XDK code that has not been attributed
to a TU), plus exception-handling funclets. Not to be confused with the
...
to translation units and so are fully **mapped**, but 229 of the 230 units have
no source in the tree — writing Microsoft vendor source is out of scope, so this
row is expected to stay near zero. It is ~20% of the whole-binary denominator,
which is the single largest reason the reachable ceiling is ~61%, not 100%.
...
the whole binary is **48.59%** against that **36.08%** — i.e. ~12.5 pp of
...
~20% is Microsoft SDK source we will not write and ~1.7% is map scaffolding, so
the reachable ceiling is **61.1%** and the 36.08% above is **59.0% of the
surface that can actually pair**. Current partition, provenance and roadmap:
[`docs/decomp/CAMPAIGN_STATE_2026-08-17.md`](docs/decomp/CAMPAIGN_STATE_2026-08-17.md).
...
emits `report.json` with the numbers in the table above. **3,088 units** appear
in that report — units pinned to address ranges in `splits.txt` plus the
auto-generated ones for code not yet attributed to a TU. The matching campaign
is in full swing — the current phase is grinding near-miss functions to 100% and
identifying/porting the remaining anonymous clusters.
...
- **`.claude/skills/`** — 25 slash-command skills wrapping the tools above into
