...
| **W16-CG** (dispatched; brief `~/tmp/brief_w16cg.md`; ★ **ESCALATION LANE — routed to Fable under the standing rule that a repeated Opus "can't be fixed" is a trigger, not a result**; ⚠ **ROUTING DEVIATION, RECORDED DELIBERATELY: Fable was dispatched THREE times and every attempt was aborted by an API capacity error (429 "all accounts are at capacity", model `claude-fable-5-1`) before executing a single step — the worktree was verified pristine at `9ced11e2` after each. The third failure came after a deliberate 10-minute cooldown, so this is sustained unavailability, not momentary load. Re-routed to Opus. The standing rule sends a repeated Opus refusal to Fable; Fable is not DECLINING here, it is UNREACHABLE, and an idle lane slot has its own cost. The deviation is defensible on one specific ground and it is worth stating so a future reader does not read this as the rule being ignored: **this brief is materially different from the six Opus lanes that failed** — it HARD-BANS the source-spelling program all six ran, and mandates the record's ProbeX transplant falsification test as the first move, so "Opus with this brief" is not a seventh repetition of the same attempt. Δ 0 is authorised, which bounds the downside. If this lane also fails, the honest conclusion is that the row needs Fable specifically and should WAIT for capacity rather than be re-dispatched to Opus again**) | opus | `?Handle@CustomizePanel@@UAA?AVDataNode@@PAVDataArray@@_N@Z` — **5,036 B behind exactly ONE charged site**, `[530] delete: clrlwi r11,r11,24`. Six consecutive lanes have tried source spellings; the in-tree record at `src/band3/meta_band/CustomizePanel.cpp:283-565` is their combined deliverable and it is a **PRICED REFUSAL**, not a mispricing — it also corrects the stale RESIDUAL-1 framing that four briefs kept re-issuing (the row is 5,036 B behind ONE instruction and closing it buys **+1 function AND +5,036 bytes**, not "+1 function and zero bytes"). The record explicitly forbids a seventh spelling lane and names the single untried channel: compiler/codegen (a different 10224 QFE, or a pragma affecting bool canonicalization). **Spelling attempts are hard-banned for this lane.** Mandatory first move is the record's own ProbeX transplant falsification test — a 20-second check whose burden is to move the transplant. Retail site `0x82619774`-`0x82619788`: `bl fn_82575670` (HasLicense → bool in r3), `clrlwi r11,r3,24`, then `.L_82619778` where `has_patch` jumps in with `FindPatchIndex()+1` in r11, `subic`/`stw`/`subfe`, and the missing `clrlwi r11,r11,24`. **Δ0 authorised** — if ProbeX refutes the channel, that verdict closes the vein and is the deliverable. | — |
