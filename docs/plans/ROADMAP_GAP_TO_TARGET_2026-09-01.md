...
| **W16-EY** `DrawTails`' 17 charges are allocation **walk order**, not a spelling (fable) | `48ab546c` | **+0 / +0 B** (evidenced negative) | Dispatched at `?DrawTails@TrainerGemTab@@QAAXABVGameGem@@HHMM@Z` — 888 B, fuzzy 99.3018, **mpn 100.0**, so the pre-registered shape was +888 B and **+0 functions**. Baseline: 17 charged sites, 222 instructions, schedule **instruction-for-instruction identical to retail** — only register *names* differ. **Eight spellings measured byte-identical** (rearranged interpolation, in-loop `t`, reversed declaration order, two hoist positions, dropped float copy, `Vector3 operator*=`, const-ref alias); **one regressed** to 52 sites / fuzzy 95.9144 (`__savefpr_23`→`_22`, frame `-0x1e0`→`-0x1f0`). ★★★ **The 14 sites of cluster A are ONE fact, not fourteen:** four volatile-FPR webs are handed out in the *same* cyclic order on both sides; retail ranks `yRange` **last** (`f11`), we rank it **first** (`f0`), and the live interval is identical on the final schedule (def idx 45, use idx 61) — so it is **not interval-driven**, and six spellings that move the value's position, statement, name, def point and copy all leave its colour at `f0` bit-for-bit. Cluster C (3 sites) is a second single fact: our compiler puts the **loaded** operand first at a commutative `fmuls` regardless of the scalar, proved with a *fresh* scalar rather than assumed. ★★★ **Three side results are worth more than the row:** naming a **member** load LICM-hoists it into a callee-saved FPR exactly as naming a pre-loop local does; a second syntactic spelling of a CSE-able expression is **not** merged back; and making `overhang` single-consumer contracted our multiply-subtract to `fnmsubs` where retail keeps `fmuls`+`fsubs` — which **proves retail's `overhang` has ≥2 consumers, i.e. our ORIGINAL spelling is correct**. A positive fact about our source recovered from a failed probe. **Falsifier recorded in the doc** so the vein is not re-hunted: any spelling that leaves the 222-instruction schedule identical and either moves `yRange` off `f0` or puts `overhang` in the first `fmuls` slot at idx 176. Everything upstream of spelling was checked clean (no `frsp`/`fmr` differences a `double` would produce; every member offset matches). ⚠ The honest verdict is **not** "unfixable" but "not fixable by the instrument we are permitted to use" — permuter-class, permuter OFF by standing directive — which is a different claim, and is why the falsifier is written down. `ab_measure` deliberately **not** run: the only candidate patch was the empty patch, and a Δ0 by construction measures nothing. Docs only, +149 lines; native PASS 18/18 skipped=0. |
| **W16-FA** one ICF membership installed, and **two corrections to the recipe I gave it** (opus) | `18d35dd5` | **+4 / +3,516 B** | Dispatched at the ICF-alias vein with a five-star instruction from W16-EZ that **turned out to be false, and the lane refuted it**. ★★★ **RESULT: `?reserve@?$vector<int,StlNodeAlloc<int>>@@QAAXI@Z` added to group `0x823715e0` — folded 3→4 memberships, one line of `symbol_aliases.json`, +4 functions / +3,516 B**, every pre-registered key exact to the digit (43995 fns · 4137976 B · code% **40.382050**, the f32 of the exact quotient, predicted before the build ran · masked_equal **+0** · honest **+4** · `total_code` unchanged). Exactly **4** rows crossed — `??0DataArraySongInfo` 2,496 B · `??0GuitarController` 732 B · `?SetUpTrackDifficulties@SongData` 164 B · `??0QuickFinding@Matchmaker` 124 B — **0 fell out**, and *a fifth crossing row was pre-declared an integrity alarm rather than a bonus*. ★★★★ **THE LOAD-BEARING HALF IS THE FIVE ROWS THAT DID *NOT* CROSS**: `??0PlayerTrackConfigList` (276 B, the sharpest), `?Process@PlayerTrackConfigList`, `??0HitTracker`, `?ClearQuarantinedPhrases@SongDB`, `?Poll@VocalPlayer` were each named in advance and each **improved without reaching 100** (99.92754 / 99.92754 / 99.88889 / 99.89584 / 93.83825) — an alias lifts `name_check` **by construction** and the `none` control **cannot** catch a fabrication, so the only admissible evidence is that the membership forgives *folding* and not *wrongness*, and that is what a would-be false positive declining to cross demonstrates. The merge was made conditional on **GATE D**, a re-decode of the complement check **from retail `band.exe` bytes** rather than from the lane's report — PE section table parsed in Python, `bl` opcode 18 with `AA=0`, `LI` sign-extended on `0x02000000` — returning **exactly 4** sites reaching the twin `0x827a36f0` (`0x8276FEC4`, `0x827745D0`, `0x827A4714` = `DataArraySongInfo+0x6f4`, `0x827A4804` = `+0x7e4`) and **60** reaching the survivor, reproducing both FA's and W16-AL's independently recorded counts. ⛔⛔ **CORRECTION 1 — TO MY OWN W16-EZ ROW, WHICH I HAD PROPAGATED INTO THREE BRIEFS:** I wrote at five stars that `??0DataArraySongInfo` failed to cross because *"the RULER DECLINED to pay the twin site"*. **FALSE.** The twin-targeted sites are **not charged at all** — no map row names `0x827a36f0`, so the target keeps the placeholder `fn_827A36F0`, which objdiff **forgives by construction**; the ruler had nothing to decline. The row was held back by its **survivor-targeted** `vector<int>` charges, which is exactly why this membership crossed it. **The PRACTICE survives and is good; the MECHANISM does not** — and FB and FC were already dispatched carrying the wrong wording. ⚠ **CORRECTION 2 — TO FA'S OWN RECORD, verified by me:** its evidence string calls `0x827a36f0` *"a JSON-null map row"*; measured, the key is **absent from the map entirely**. **101 rows *are* JSON null**, so that class is real — this is not one of them. Identical consequence, different fact; fixed by follow-up commit, never by rewriting. ⛔ **What it did NOT do:** it **refused 25,372 B** of further alias candidates with reasons rather than installing them, and it **demoted its own displacement decoder from proof to screen** — for a *charged* site objdiff's retail-side name **is** the map name at the branch destination, so decoding and looking it up **re-derives objdiff's own input**; **9 of 48 displacement-"proven" pairs are refuted by chased T1, a 17.7% fabrication rate by bytes.** ⚠ Do **not** over-correct: its narrower job — choosing *which* body-identical twin a caller meant, **after** the body is proven — is not circular (the destination address is a fact about retail bytes), which is how EZ and FA both used it, so **EZ's chain stands**. ★★★ Side finding that closes a 6,112 B candidate: **chased T1 can return PROVEN vacuously** and `--chasetest`'s anti-vacuity arms cannot catch it — `??3BinStream@@SAXPAX@Z`↔`OggFree` has a **4-byte body** with 396 retail twins, evidence line `VACUOUS-BUT-IDENTICAL`; **read `retail_size` alongside the verdict**, which drops honest supply 70,592 B → 48,304 B. |
| **W16-FB** the 316 named-but-zero rows: 42% are **absent by design**, and one re-home exposed a real ctor defect (opus) | `32d47316` | **+8 / +1,032 B** | Dispatched at the named-but-zero vein — rows carrying a map name that score 0 because objdiff never diffed them. It reproduced the 316 rows / 46,000 B population **byte-for-byte** (CLAUDE.md's 267 / 38,096 B is stale) and classified it with an instrument that **predicts objdiff's own behaviour instead of guessing**: does our compiled object's COFF symbol table DEFINE the symbol, merely reference it (UNDEF), or not mention it? **Validated before use** — all 263 rows whose object does not define the symbol have **no `match_percent_normalized` key at all**, and 43 of the 53 that do define it were diffed. Census: **class 4 absent by design 47 rows / 19,360 B** (XDK/CRT, vendor DSP, XAudio2, LEAPCORE) · class 1 missing source 115 / 13,136 B · class 3 mis-homed pin 101 / 7,512 B · **C5 paired-but-DIVERGENT-BODY 43 / 5,684 B — a class the brief's taxonomy did not contain** · C6 defined-but-unpaired 10 / 308 B. ★★★ **TWO RESULTS OUTRANK THE ROW COUNT: 42% of the bytes are class 4 and closed by standing directive, so the actionable vein is far smaller than 46,000 B; and C5 is NOT a pairing failure at all** — those rows pair fine and score ~0 because **our body differs**, with bodies *larger* than retail, so they are not stubs either. **That is ordinary decomp work and must not be briefed to a map lane.** ★★★★ **WHAT LANDED IS THE SEQUENCE, NOT THE BYTES:** `??0Player` (708 B) was pinned to `GemPlayer.cpp` in a lone `.text` block `0x826A7558`–`0x826A79D0` — **`0x14388` from the nearest real GemPlayer block and exactly contiguous between two Player.cpp blocks, filling a hole in Player.cpp's own range**; our `Player.obj` DEFINES the symbol, `GemPlayer.obj` only references it. **Adjudicated on geometry and retail structure, not on the metric**, and dtk independently agreed by re-deriving the orphan `.pdata` line into Player.cpp by itself. Re-homing made the row **legible at 99.435%**, exposing **one mismatch in 177 instructions** — `stw r29, 0x2f8(r30)` — and the **compiler-authoritative** layout puts `mUnkTU5_tail` at exactly `0x2f8`, uninitialised by this ctor though `Player.h`'s other ctor sets it; `mUnkTU5_tail(0)` closed it. ⇒ **That separation IS the lane's own control: pure forgiveness would have handed over 100 for free. The re-home bought LEGIBILITY and stopped at 99.435; the last 0.565% required a retail-byte-adjudicated source repair.** Landing gate reproduced the geometry from `splits.txt` independently (owners as claimed, contiguity true on both sides, distance `0x14388` exact, **total pinned `.text` bytes delta exactly 0 — a MOVE, not an ADD**), and every pre-registered key landed exact: 44003 fns · 4139008 B · code% **40.392120** (f32 of 40.392119970) · masked_equal **+7** · honest **+1** · `total_code` unchanged. Row assertion held: ctor `fuzzy = mpn = 100.0` with **`masked_equal` FALSE** — it crossed **on merit, not by funclet byte-signature** — and both controls held (`??0BeatMatchController` still mpn 0.0; **GemPlayer's at-100 count 305 before and after**, so the 9 rows that left were none of them). ★★★★ **THE PRICING MISS IS THE TRANSFERABLE PART:** FB pre-registered `{0, +708}` bytes and ±1 row for the re-home and measured **+324 B across 9 rows**. It **priced the ROW when a re-home moves the BLOCK** (1,136 B / 9 rows, the remainder EH funclets) — which is exactly why **7 of the 8 gains are `masked_equal` disclosure with Δhonest 0**. ⇒ **PRICE THE BLOCK, NOT THE ROW**, and the census's 3,208 B of unique-home class-3 **understates** what re-homing moves. ⛔ **Refused deliberately:** three further re-home candidates (`_M_allocate_and_copy<RndBone>` 100 B, `ObjDirItr<BandList>::operator++` 84 B, `list<CheatLog>::~list` 4 B) — **all template COMDATs, which a TU may legitimately emit itself**, so the evidence is weaker than for a class's own ctor and bundling them would muddy attribution. ⚠ **Coordinator defect caught in my own landing gate, recorded so it is not inherited:** my "a source patch must recompile ≥1 TU" counter ran a `grep -cE` whose alternation spelled `msvc` in **lower case** against a log that prints `MSVC`, so it read **0 — the alarming value — for a reason unrelated to the build**. It returns 0 whether or not the patch compiled, so it **cannot discriminate**; gating on it would have aborted this correct merge and would have passed a genuinely absent-vs-absent one just as silently. The build in fact shows exactly one edge, `MSVC build/45410914/src/band3/game/Player.obj`. **Use `-ciE` and assert ≥1.** |
| **W16-FC** the invisibility backlog: 13 anonymous rows made adjudicable, and one of them was hiding a **wrong-oracle signature** (fable) | `e19ac916` | **+0 / +0 B** (by design) | Dispatched at the 6,691 anonymous rows / 1,228,556 B sitting in units that **already have a compiled object** — rows objdiff has nothing to pair against, so a body riddled with defects and a body with nothing wrong both read **fuzzy 0 and look identical**. It named 13 (30,240 B) across two pre-registered waves and measured **exactly the +0 / +0 it predicted**: under `name_check` the ruler already **forgives a placeholder target**, so naming an anonymous address has zero call-site upside and the payout is **pairability**, not bytes. Δfuzzy +0.201326 (wave 1) + 0.017710 (wave 2) is the entire visible movement and is the right size for what happened. ★★★★ **The value showed up one step later, exactly as W16-EW predicted it would.** Naming `0x82433268` = `RndPostProc::Load` made the body adjudicable, and adjudicating it against retail bytes **before editing** showed our `Load`/`LoadRev` carried **DC3's shape** (`BinStreamRev&`) where retail carries **rb3-Wii's** (`BinStream&, int`) — two co-addressed file-static shorts (altRev +0, rev +4), `LoadRev(bs, gRev)`, `mLuminanceMap` streamed with no altRev gate. `LoadRev` went **0 → 94.0** and `Load` reached **95.6875**, with **6 of 6** wave-2 predictions hit, same-TU controls holding to the digit, and exactly 3 rows differing A→B. ⇒ **our source was verbatim one oracle and still wrong, because the oracle we copied (DC3) is NEWER than RB3 and the one we did not (rb3-Wii) is the contemporary shape** — the pairability memory's exact mechanism, now with a second instance. ★★★ Byte geometry was proved for all 13 **before** any was named (`.pdata` entry begins at the row address, decoded length equals the report size, next entry starts where the extent ends) — a phantom row is otherwise **indistinguishable** from an unidentified one. Left open on purpose, each needing its own pre-registered A/B: `Load`'s frame 0xa0 vs our 0xb0 plus an unexplained `mr r28,r3` placement; `LoadRev`'s three retail `fdivs` against our reciprocal + 3 `fmuls`; `?Load@RndDir@@`'s name sitting on an **8-byte this-adjust thunk** at `0x82402fa0` scoring 100 while the real 428 B body is anonymous at `fn_82404F80` (re-homing an existing name is measured non-neutral). Quazal rows were excluded wholesale — 7-line map scaffolds, so naming them buys a pairable row at 0% with no content. |
| **W16-FD** two game-layer rows crossed, and **both were our source faithfully reproducing a WRONG ORACLE** (opus) | `2b9bd5d5` | **+2 / +308 B** | Four freshly-measured game-layer rows dispatched; it closed two and **abandoned two with their falsifiers recorded**. ★★★★ `?IsFinished@MakeQuazalSessionJob@@` **65.66 → 100** (+128 B): the rb3-Wii oracle left `QuazalSession::QuazalSession(bool) {}` as a never-decompiled stub, and that **one stub caused TWO independent defects** — fixing either alone leaves the row short, which is why the intermediate **86.44** reading is the informative one rather than a failed attempt. (1) an empty ctor is **always inlined at `/O1 /Ob2`**, deleting the `new`→null-check→call sequence, so `#pragma auto_inline(off)` restores retail's call shape (house precedent `MetaPerformer.cpp`/`BandCharacter.cpp`; `__declspec(noinline)` is **not** a substitute here); (2) even out of line MSVC still **saw** the empty body, proved it nothrow and **elided the caller's entire EH frame** — no r31 frame pointer, no object-under-construction slot at `0x50(r31)`, `this` in r31 not r30, frame 0x60 not 0x70. Retail-byte proof the ctor can genuinely throw: retail's `fn_823F2F08` does two heap allocations (`li r3,0x20`, `li r3,0x38`, each `new`-shaped with a null check). ★★★★ `?AssignLocalOwner@NetSession@@` **37.38 → 100** (+180 B), and the load-bearing fact is an **instrument reading, not a code reading**: at instruction 23→25 the result of `UserHasOnlinePrivilege` is computed and then **overwritten untested** — not a dead call but **`MILO_ASSERT`, which in this build is `((void)(cond))`**, so it evaluates and discards. Reading that call as an assert is what let the loop reconstruct; reading it as a branch would not have matched. The old body assigned from `GetOwnerOfGuest(0)` — **a real behavioural bug**, not merely a codegen mismatch. ⛔ **Abandoned with evidence:** `OnMsg@NetSession(AddUserRequestMsg)` 664 B is **not a source defect at all** — **my brief said "one charge from crossing" and there are TWO**, both relocation-name charges against ICF fold twins (`Vector3` and `UserGuid` are both 16 B), so **our spelling is correct**; under the project's own T1 comparator (run with a negative control that refuted, so the instrument can fail) the `push_back` charge is provable via a 2-link chain but the copy-ctor charge chains to a **4-byte `blr` thunk the anti-vacuity guard refuses BY CONSTRUCTION**, and since `matched_code` is all-or-nothing the provable half buys **0 bytes** — reachable **iff** that link is adjudicated by **address** rather than bytes. `GetNoteSliceWeight@VocalPart` 484 B: 39 charges, one cause (one extra callee-saved FP register, `__savefpr_20` vs retail `_21`); **both principled levers were BUILT AND MEASURED AND BOTH REGRESSED** — constant-folding `f22` −11.94 pp, the oracle's `frameMs` hoist −6.80 pp ⇒ **the oracle is WORSE than our source here**; permuter-shaped, permuter off by directive. Two of its own predictions failed usefully: `auto_inline(off)` alone was predicted to reach 100 (got 86.44, which **exposed** the nothrow defect), and `AssignLocalOwner` was hedged at "≥95, not 100" expecting an explicit `int n = users.size()` (MSVC hoisted the bound unaided). |
| **W16-FE** the 1,664 B row has **NO ORACLE IN EITHER SIBLING REPO**, and the hole next door was worth 340 B (opus) | `de39ab37` | **+4 / +340 B** | Dispatched at `?OnMsg@Game@@QAA?AVDataNode@@ABVButtonDownMsg@@@Z` (1,664 B, fuzzy 8.0096), the best size-if-it-crosses game-layer row on the board. **It did not close that row and says so plainly — for a structural reason, not an effort one.** ⛔⛔ **THE LOAD-BEARING FINDING, which neither my brief nor W16-EH stated: the body has NO SOURCE ORACLE.** Measured, not assumed — `OnMsg(ButtonDownMsg)` and `OnMsg(ButtonUpMsg)` are **both ABSENT from rb3-Wii's `band3/game/Game.cpp`**, all five `audition_*` Symbols are absent from rb3-Wii (only `deploy_if_possible` exists, on an unrelated `Player` action), and **neither the handlers nor `VocalGuidePitch` appear anywhere in dc3-decomp**. ⇒ this is **TU5-era Harmonix code** — rb3-Wii is a vanilla-RB3 dev build and DC3 is a different game, so **all ~416 instructions must be reverse-engineered from retail bytes**; a successor planning to "port it from the oracle" is planning against a file that does not exist. ⚠ And it is **not** RB3DX-specific: the DX-lineage TU5 image differs from clean TU5 by 53 words / 10 byte-patch groups, far too small to hold a 1,664 B handler. Combined with `matched_code` being all-or-nothing (400 of ~416 instructions banks **exactly zero**), MSVC **sparse-switch lowering** (value-mapping cascade → `mtctr` → `bdz` chain), local statics under `/DRB3_HANDLE_LOCAL_STATIC`, and `fn_8274B0F8` still unresolved (called **both** as `Array` and as `Int`), the lane declined to write a speculative body that pays 0 B either way while adding a large untested body the native gate must carry. **W16-EH's eight-case decode remains the correct starting map; what FE adds is that the decode was never the bottleneck.** ★★★★ **INSTEAD IT TOOK THE NEIGHBOURHOOD, AND THAT IS THE TRANSFERABLE RESULT: pulling one thread out of the 1,664 B row's decode (`fn_826C9160`) yielded +340 B and a COMPLETED UNIT in a single lane**, because the `VocalGuidePitch`/`MidiInstrument` neighbourhood was full of **declared-but-undefined methods and one wrong map name** — the same hole class W16-EH found for `Shuttle::SetActive`. Four rows crossed, **0 fell out**: `?ReleaseNote@MidiInstrument@@QAAXE@Z` 104 B (anonymous, now mapped at `0x827141d8`) · `?Pause@MidiInstrument@@QAAX_N@Z` 96 B (map row `0x82714350` **RENAMED** off a wrong `?clear@?$ObjPtrList@VTask@@VObjectDir@@@@QAAXXZ`, which scored 54.4583 pre-merge) · `?StopNote@VocalGuidePitch@@QAAXXZ` 76 B (`0x826c9160`, W16-EH's "one genuine unknown") · `?Pause@VocalGuidePitch@@QAAX_N@Z` 64 B. ⇒ **`default/band3/game/VocalGuidePitch` is COMPLETE, 1728/1728 B, 16/16 fns.** ★★★★ **THE ORACLE WAS WRONG TWICE IN ONE 200-BYTE PAIR** — a third and fourth instance of the day's pattern after FC's `RndPostProc` signature and FD's two. rb3-Wii spells `MidiInstrument::Pause`'s loop body `(*it)->Pause(b)`; retail does `lwz r3, 0x28(r11)` **before** the vcall and `mSample` is at 0x28, so the oracle's one-line `NoteVoiceInst::Pause` is **inlined away** and the real spelling is `(*it)->Sample()->Pause(b)` (`SampleInst::Pause` is slot 22 = 0x58, compiler-reported). **Writing the oracle literally emits a vcall on the wrong object.** ⇒ **a file that matches the oracle perfectly is not thereby correct.** ★★★ `StopNote`'s body was **calibrated against a matching neighbour rather than guessed**: its emitted sequence is byte-identical to the block inside `EnableGuideTrack` (`0x826C9098`), already at 100% with exactly that spelling, and retail saves only `r31` (frame 0x60) vs `EnableGuideTrack`'s `r30+r31` (0x70) — consistent with one fewer live value. Phantom check run **before** naming (`.pdata@82234798`, unwind `0x40001304`, `symbols.txt` size `0x4C`, complete prologue/epilogue, two `bl` callers), and **anonymous pairing was tested first and refuted** — the unit's other anonymous row reaches 100 only as `masked_equal`, while `fn_826C9160` is `masked_equal: false`, so a map name was genuinely required. ⚠⚠ **NAME PROVENANCE, recorded by the lane itself: do NOT cite `StopNote` as an identification.** Body, class and signature are proven on retail bytes; the **spelling is a lane-assigned descriptive label with no oracle backing** (rb3-Wii has no such method, DC3 has no `VocalGuidePitch`) — free to change if a successor finds the true name. ★★ **Two real behavioural bugs fixed, both worth 0 bytes and landed on merit:** `MidiInstrument::Pause`/`::ReleaseNote` were **declared in the header and defined NOWHERE in the tree**, so every `VocalGuidePitch` pause/note-release path called a function that did not exist; and `Game::UpdatePausedState` **never stopped the guide note** when pausing with movie-sync on (retail's tail at `0x8267AA48`), so the vocal guide pitch kept sounding across a pause — 66.296 → **70.000** fuzzy on a 972 B row that **cannot cross**, pre-registered as exactly that. ⛔ **CORRECTION TO MY OWN BRIEF, the second in two days:** I priced `OnMsg(ButtonUpMsg)` (156 B, fuzzy 98.5897 / mpn 100.0) as *"a relocation-NAME charge withholds all 156 B — 156 B for a one-line fix"*. **REFUTED: no name is involved anywhere in the row.** `canonical_match_percent` **100.0**, and all **11** charged rows are ONE 3-way callee-saved rotation (retail `r31/r30/r29` for sret/`this`/pad vs our `r29/r31/r30`) with every opcode, offset, constant and branch identical in identical order; the single `bl` resolves to placeholder `fn_825150C8`, which objdiff **forgives**. ⇒ **the trap is that `diff_arg` covers REGISTERS as well as relocation names**, so inferring "name charge" from an arg-only shape is invalid — this row is permuter-class and the permuter is OFF by directive. ⛔ **Also did NOT:** install any alias or touch `symbol_aliases.json`, attempt the ButtonUp rotation, resolve `fn_8274B0F8`, or correct `ROADMAP_GAP_TO_TARGET_2026-09-01.md:2369` (another lane's dated record, outside its staged paths — still repeating two refuted claims). ⚠ **Landing note — the gate caught a hazard that byte-exactness would have passed:** FE's base map had 29,563 rows while main's had 29,576 after W16-FC's +13, so a wholesale take of FE's map file would have **silently deleted all 13 of FC's rows and a byte-exact gate against FE's own snapshot would have PASSED on exactly that failure**. The map was therefore gated **SEMANTICALLY** (29,578 rows · FC's 13/13 present under FC's names · FE's 3 changes applied · the wrong old name gone · no duplicate names introduced) with byte-exact applied only to the other 7 files. ⚠ **Two defects in that gate, mine, recorded so they are not inherited:** its duplicate-name `Counter` filtered on truthiness rather than `isinstance(v, str)` and **crashed on the map's five legitimate list-valued metadata keys** (`_bijection_arbitrary`, `_denylist`, `_denylist_unadjudicated`, `_icf_arbitrary`, `_internal_linkage_allow`) — failing safe, but only after every substantive check had passed; and **two of its three controls were VACUOUS**, because I spelled the `OnMsg` mangled names from memory in rb3-Wii's virtual-returning-`bool` shape (`?OnMsg@Game@@MAA_NABVButtonDownMsg@@@Z`) instead of reading the real `?OnMsg@Game@@QAA?AVDataNode@@ABVButtonDownMsg@@@Z` out of `report.json`. **A control that cannot be found cannot fail.** Both were verified to have held after the fact (8.0096 and 98.5897, unchanged). Every pre-registered key exact: 44009 fns · 4139656 B · code% **40.398440** · masked_equal **23235 (+0)** · `total_code` and `total_functions` unchanged; fuzzy 50.259110 → **50.261640**, not predicted. Native gate `PASS 18/18, skipped=0, rc=0`. |
| **W16-FF** both PostProc residuals MOVED and **NEITHER CROSSED** — landed for the four veins it closed, not for the bytes (opus) | `aed4e990` | **+2 / +40 B** (⚠ **disclosure-class; Δhonest +0**) | Dispatched at W16-FC's two leftovers — `LoadRev` (1,728 B at 94.0000) and `Load` (192 B at 95.6875), a 1,920 B size-if-it-crosses. Both moved substantially (**94.0000 → 97.358795**, mpn 94.40509 → 97.671295; **95.6875 → 99.854164**) and **neither reached 100**, so by `matched_code`'s all-or-nothing rule they contributed **exactly zero bytes**. ⛔ **THE +40 B IS A DIFFERENT ROW AND MUST NOT BE BOOKED AS PROGRESS ON THE 1,920 B.** The lane's own A→B set-diff shows four rows moved: the two targets plus `fn_824316B8` and `fn_82431690`, two 40-byte EH funclets **both `masked_equal`**, freed when the `MILO_FAIL` was removed; only one crossed. **`Δhonest +0` is the ruler saying the same thing in one number**, and the landing set-diff reproduced it exactly — **1 row crossed, 40 B, `fn_824316B8`**. Every pre-registered key landed exact: 44011 fns · 4139696 B · code% **40.398834** · masked_equal **23237**, the last **derived rather than guessed** (honest = matched − masked_equal, so Δhonest +0 with Δmatched +2 *forces* Δmasked_equal +2). Both target rows reproduced on main to six decimals and the control `??0RndPostProc@@IAA@XZ` **held at 100.0**. ★★★★ **WHAT THIS LANE IS WORTH IS FOUR CLOSED VEINS, THREE OF THEM MINE OR W16-FC'S.** (1) ⛔ **W16-FC's designated experiment #1 is BYTE-INERT** — the rb3-Wii bloom respelling (hoist `float red = c.red;`, `(4.0f - red)/range`, `mBloomThreshold` after the divisions) measures **97.358795 to six decimals, unchanged**. ⇒ **the `/fp:fast` reciprocal is NOT reachable by respelling the expression.** (2) ⛔ **W16-FC's size figure was wrong** — ours is **1,756 B, not 1,784** (over-counted by 28 B), i.e. net **+7 instructions** over retail's 1,728. (3) ⛔ **W16-FC's prologue hypothesis is refuted on retail bytes** — the cause is not the `rev > 0xE` region or its two unnamed callees but the **`rev < 0x12` `Transform*` block**. (4) **Declaration order is inert** for `Load`'s stack slots (tested twice), as are initializer form and `MILO_ASSERT` removal; **inverting the bloom if/else REGRESSES to 94.1412** (block order flips); and **`RB3_TU_OBJPTR_FORCEINLINE_CTOR` is net NEGATIVE** — LoadRev 97.3588 → 98.5509 but `??0RndPostProc` collapses **100.0 → 60.9581, −668 B** — reverted, ctor verified back at 100. ★★★ **IT ALSO CORRECTED ITS OWN WRITE-UP RATHER THAN SHIPPING IT.** Its "Wall A" (no per-TU lever can express this residual) was **wrong in its REASON**: `RB3_OBJPTR_INLINE_TWOARG_CTOR` targets precisely the `ObjPtr<RndDrawable>` two-arg ctor retail inlines as three stores (vtable@0x70, mOwner@0x74, mObject@0x78) and `rndobj/` is PCH-excluded, so the lever **is** valid here. It still declined to pull it, on the better ground that **F5 already inlined that ctor, reached 98.5509 and STILL paid 0 B** while the `/fp:fast` cluster, the f29/f30 constant swaps and the r10/r11 swaps all survived. ⇒ **right conclusion, wrong reason — recorded as both**, and the ordering that follows is that **the reciprocal must be solved FIRST because every other fix on this row pays 0 B behind it** (that one cause plausibly explains ~14 of the 42 remaining charges). ⚠ **A prediction miss worth carrying:** it enumerated Δbytes as multiples of its two targets — {0, 192, 1728, 1920} — and measured **+40**, because a third row crossing was not in its model at all. Every individual band held, but **a prediction set that cannot express the observed outcome is under-specified**; P2 also partially missed (one non-volatile freed, `__savegprlr_26→_25` and frame +8, not the predicted two). ★★★★ **THE ONE LEAD IT LEFT, DELIBERATELY UNTESTED AND NOW ESCALATED TO FABLE AS W16-FH:** retail's bloom block **round-trips through a stack-resident 4-float aggregate** at `0x80/0x84/0x88/0x8c(r31)` — alpha set to 0.0, **red read back before being overwritten** — where we assign members directly. Hypothesis: retail divides into a local `Hmx::Color` and it is the **AGGREGATE TEMP, not the expression spelling**, that denies MSVC the reciprocal CSE (retail emits three `fdivs`; we emit `fdivs` + reciprocal `fmuls`). **That is the only remaining path to the 1,728 B**, and it was correctly left to a lane that can pre-register and measure it rather than to a rushed addendum. ⛔ Also not done: naming `fn_823A0918`/`fn_823A07A8` (forgiven placeholder targets — naming is a bet, and moot behind the `/fp:fast` wall), and no global `MILO_FAIL` change (its F1 is per-site behind `HX_NATIVE`, since the arg-evaluating form is load-bearing for the `ObjectDir::Find<T>` family). |
| **W16-FG** `FxSendReverb360::SyncEffectParams` — **the largest single-row prize of the session, and it CROSSED** (opus) | `68221e5c` | **+1 / +3,280 B** (**HONEST** — `masked_equal` Δ0) | `?SyncEffectParams@FxSendReverb360@@UBAXPAUIXAudio2SubmixVoice@@@Z`, **3,280 B, fuzzy 0.0000 → 100.0 / mpn 0.0000 → 100.0 on the FIRST build**; unit `matched_code` 732 → 4,012 of 4,760. Our source was a **16-line stub** — the whole body was absent, so this is written code, not a re-spelling. Landed with **every pre-registered key exact, miss 0.000000 on all six**, and the A→B set-diff shows **exactly 1 row crossed, 3,280 B, zero collateral**. Because `masked_equal` did not move, honest goes 20,774 → **20,775** — a REAL match, not disclosure (contrast W16-FF's +40 B, which was entirely `masked_equal`). ★ **A pricing note worth keeping: composing ROUNDED pp-deltas is not the same arithmetic as the ruler's.** Adding the lane's `+0.032014 pp` to main's `40.398834` gives **40.430848**; the byte ratio `4142976/10247068` gives **40.430843**, and `report.json` computes the ratio. Predicted from the ratio, measured **40.430843 exactly**. Compose BYTES, then divide — never compose percentages. ★★★★★ **WHAT THIS LANE IS WORTH BEYOND THE BYTES: IT CAUGHT AN XDK VERSION DELTA THAT WOULD HAVE COST THE ENTIRE PRIZE.** `sizeof(XAUDIO2FX_REVERB_PARAMETERS)` is **0x34 in RB3, NOT DC3's 0x38** — `RB3 0x82B68D14 li r6,0x34` vs `DC3 0x82E32334 li r6,0x38`, **the same instruction slot in the same eight-instruction sequence**, and DC3's unit is 100.0% matched, so **0x38 is correct FOR DC3 and 0x34 FOR RB3** (DC3's trailing `WetDryMixPct` is a post-RB3 XDK revision). Carrying DC3's value would have charged that one immediate, and **`matched_code` is all-or-nothing — that single byte is the difference between 3,280 B and 0**. ⚠ **It took BOTH binaries to call this a version delta rather than a DC3 bug**: one binary alone reads as a defect in the oracle. ★★★★ **AND THE REST OF THE ORACLE HELD — ADJUDICATED ON RETAIL BYTES RATHER THAN ASSUMED**: 390/390 float constants, 30/30 preset names, identical order, entry count 30 (`cmplwi r9, 0x690` = 30 × 0x38, corroborated independently by objdiff counting 30 `??0Symbol@@QAA@PBD@Z`), the 0x38 stride, and the bit-0 `??_B` guard shape. `MILO_FAIL` is `((void)(...))` in the match build, so **DC3's miss-check is dead code — which is exactly why retail has no branch there**. `mEnvironmentPreset` at `this-0x20` fell out of the existing layout, so **no layout change was needed**. ⇒ this is the four-mode oracle-fidelity rule landing on its **favourable** side for once: the oracle was right about everything observable and wrong about exactly one number, and only retail bytes could tell which. ★★★★ **INSTRUMENT LESSON: A SPILL-BLIND DECODER UNDER-VERIFIES WHILE LOOKING COMPLETE.** Three constants (`hallway.DecayHFRatio`, `mountains`/`medium_hall.ReflectionsDelay`) live in **stack slots `r31+0x50/0x54/0x58`**; the lane's first symbolic-execution decoder reported them `UNRESOLVED` and would have covered **387/390 while presenting as exhaustive**. It was fixed by decoding **FPR spills through the frame**, not by lowering the bar. ⇒ when a decoder's coverage is one or two short of total, suspect the DECODER's model of the frame before concluding the constants are unreachable. ⚠ Two tooling notes: `ab_measure` **deleted the lane's findings doc** because it was written during the run (it warns loudly; the doc was rewritten), and it **correctly REFUSED the first A/B over the untracked new header** — so the header was committed separately (inert until included) and the `.cpp` measured as the patch. Both are the tool behaving as designed. ⛔ **Deliberately NOT done, each with a stated reason:** `?ReverbConvertI3DL2ToNative@@` (740 B, same unit) left at **fuzzy 0** — MS vendor implementation, **XDK porting is out of scope by standing directive**, and the landing gate re-verified it is **still 0**, i.e. the lane did not quietly do what it said it did not do; DC3's extra `FxSendReverb360` virtuals (`Recreate`/`UpdateMix`/`OnParametersChanged`/`IsStandard`) **not added**, because our vtable already yields 10 matched rows here and reordering it risks them for no measured gain; and `XAUDIO2FX_REVERB_PARAMETERS`'s **internal field layout below 0x34 is inherited from DC3 and is UNVERIFIED** — only its SIZE is observable from RB3's `.text`, and it is recorded as unverified rather than presented as known. |
| **W16-FJ** the fuzzy-0 rows were in the **WRONG OBJECT**, not missing — and the brief's prize was overpriced (opus) | `108d848a` | **+10 / +1,264 B** (⚠ **+6 of the 10 are `masked_equal`; Δhonest +4**) | Dispatched at two Synapse::DSP units whose source appeared entirely absent — `PeakDetector.cpp` was a **single comment line** (its header one line) and `GranularSynth.cpp` held **only two STL vector ctors** — against DC3 row oracles of 91.6–100.0. Six rows crossed 0 → 100 (`Synthesize` 780 B, `Flush` 196 B, `~GranularSynth` 136 B, `??0PeakDetector` 68 B, plus `fn_82B74AD4` 44 B and `fn_82B74B00` 40 B), **zero rows fell out anywhere in the binary**, and the three walls reproduced DC3's scores to four decimals exactly as the lane claimed (`Detect` **96.881860**, `gaussianWindow` **92.761190**, `ExtractGranules` **91.622220**). ★★★★★ **THE FINDING THAT BEATS THE PREMISE: a 0.0% row here meant "no source IN THIS UNIT", not "no source".** `~GranularSynth`, `Flush@GranularSynth` and `Detect@PeakDetector` existed as **undefined external references in `Synapse_dsp.obj`** — which carries its own local, layout-compatible class definitions — and were **absent from `GranularSynth.obj` / `PeakDetector.obj`**. **objdiff pairs a target row against the base object OF THE SAME UNIT**, so those rows read 0% however correct the code was. ⇒ **a fuzzy-0 row of this kind is INDISTINGUISHABLE in `report.json` from one whose body we genuinely lack, and the two remedies are unrelated.** ⚠ **It was already on the record and went unread**: the `_denylist_comment` for `0x82b74600` says our `~GranularSynth` compiles into the wrong object — and it proposed re-homing the **pin**, where **defining the body in the right `.cpp` is cheaper** and is what landed. ★★★★ **THE 136-vs-140 DESTRUCTOR IS *NOT* W16-FG's VERSION-DELTA SHAPE — DC3 IS VINDICATED.** The brief flagged our 136 B against DC3's 140 B as a possible second instance; adjudication went the other way. Lane W12-D had already read `0x82b74600` as destroying a `vector<vector<float>>` at **+0x38**, a buffer at **+0x2c** stride **0x18** and a buffer at **+0x20** stride **0x40** — exactly DC3's `mGranules@0x20`(0x40) / `mVoices@0x2c`(0x18) / `mWindows@0x38` — and our **136 B body then reached byte-exact 100.0 FROM DC3's source**. ⇒ the 4 bytes separate the two **retail images**, not the source, and **forcing our size to 140 would have broken a row that was already right.** ★★★★ **+780 B CAME FROM CROSSING A FLOOR DC3 DOCUMENTS AS SOURCE-INERT.** `Synthesize` ported in at **99.9487** with one charged site — the accumulate operand order (`fadds f0, f10, f0` vs ours `f0, f0, f10`). DC3's own source comments measure `+=` and `s + out[][k]` as inert and call it a commutative-operand-order floor; a spelling DC3 never tried closes it: `float acc = out[gr.mVoice][k]; out[gr.mVoice][k] = acc + s;`. ⇒ **a twin's "measured inert" is evidence about the spellings it TRIED — DC3 tried two REASSOCIATIONS; the lever was a LIFETIME.** Worth backporting: DC3's identical 780 B row is still at 99.9487. ⛔⛔ **TWO CORRECTIONS TO THE BRIEF I WROTE, BOTH OF WHICH RE-PRICE THE CENSUS THAT PRODUCED IT.** (1) **The "3,228 B oracle-backed prize" OVERPRICES by design.** `matched_code` pays only at `fuzzy == 100`, so a **FAITHFUL port of a row whose oracle sits below 100 pays ZERO**: 1,112 B crossed and **2,116 B landed on DC3's wall**, reproducing DC3's scores to four decimals — which is itself strong evidence the port is faithful and both images carry the same code. ⇒ **price an oracle-backed row by the ORACLE'S DISTANCE FROM 100, not by its size.** (2) **The two rows the brief called "no DC3 row (anonymous)" were never unoracled — they were unnamed in OUR map.** `fn_82B737C0` (68 B) is DC3's `??0PeakDetector` (68 B, **fuzzy 100**) and `fn_82B746E8` (916 B) is DC3's `??0GranularSynth` (916 B, 97.4018). The first was **proved on retail bytes** — 17 instructions, every offset and access width matching including the one-byte `stb` at `+0x20` for the `bool mTracking` — installed, and went byte-exact 100 (**+68 B**). Map edit gated **semantically, not byte-exact** (the file is reformatted, so 31,299 lines changed for a +1 row edit and a wholesale overwrite would have been invisible — the W16-FE hazard): **29,579 rows, +1 address, 0 lost, 0 renamed**, forced re-split confirmed (`1832 files patched`, `symbols.txt` at its fixed point). ★★★ **A PREDICTION MISS I PRE-REGISTERED AS THE LIKELY ONE, AND IT LANDED THERE:** `masked_equal` was predicted Δ0 and measured **+6** (23,237 → 23,243), so **honest is 20,779, not the 20,785 a naive reading of +10 functions gives** — six of the ten new functions are **disclosure**, four are real. The pre-registration named this as the least-confident key and said movement there is information rather than failure; recording it as measured. ★★★ **AND `matched_code_percent` IS STORED AS f32 — WHICH RETIRES THE 6th DECIMAL OF W16-FG's PRICING RULE WITHOUT RETIRING THE RULE.** Predicted **40.443178** from the byte ratio, measured **40.443180**. The exact f64 ratio is 40.443178478, whose **f32 round is 40.443180084** — the two are **the same number at the precision `report.json` stores**. FG's case is consistent (f64 40.430843242 → f32 40.430843353, measured 40.430843, miss 0). ⇒ **compose BYTES then divide is still right; the last decimal of an f64-derived prediction is not.** That the naive rounded-pp sum happened to equal the f32 value here is coincidence — it was wrong by 5e-6 on FG. ⛔ **Deliberately NOT done, each with a stated reason:** `0x82b746e8` → `??0GranularSynth` was **PROVED** (229 instructions opening `stw r4,0x0`, `fcfid`/`frsp` of r6, `stw r6,0x10`, `stfs f31,0x8/0xc`, `stw r28,0x18` — our ctor's init list exactly) and **NOT installed**, because DC3's row is 97.4018 so there is **zero byte upside**, while naming converts forgiven placeholder call sites into `name_check`-checked ones — a real unmeasured downside; left for a lane that can measure it alone, its value being legibility and bug exposure. The **2,116 B residual was not ground** — DC3's in-source notes record **seven** measured spellings on `ExtractGranules` alone, and unlike `Synthesize` these are 20–40 charged sites, not one, i.e. a permuter/regalloc vein. DC3's `__uninitialized_fill_n<ObjectDir::Viewport>` instantiation **not copied**: RB3's counterpart row is anonymous (`fn_82B744E8`) so no spelling can pair it — inert, and it drags in `obj/Dir.h`. `Synapse_dsp.cpp` untouched, verified unmoved at 3,864 B by the A/B. Native gate `PASS 18/18, 0 SKIPs, rc=0`. |
| **W16-FI** `kNumberOfBuffers` is **SIX** in retail, not DC3's seven — the `+4` shift was a consequence, not the cause (opus) | `9642c231` | **+5 / +296 B** (Δhonest **+3**; `masked_equal` +2) | Dispatched at the `ContentMgr_Xbox` neighbourhood from the fact that made no sense: `XboxContentMgr::Init()` was 120 B at **fuzzy 0.0 / mpn 1.333** while our source was character-for-character DC3's matched 15-line body, the only visible difference being `unk938/unk93c` against DC3's `unk934/unk938`. ★★★★★ **THE BRIEF'S HYPOTHESIS WAS RIGHT IN DIRECTION AND WRONG IN MECHANISM — THE `+4` SHIFT IS A CONSEQUENCE, AND THE CAUSE IS ONE CHARACTER.** `kNumberOfBuffers` is **6 in retail and 7 in DC3**, which shifts every member past `mXDatas`. **Six independent retail witnesses, not one arithmetic argument:** `li r11,0x6` (Init's clear loop) · `li r28,0x6` and `cmpwi cr6,r28,0x6` (StartRefresh) · `addi r10,r3,0x7e0` · `mulli r11,r28,0x138; addi r4,r11,0x90` (`mXDatas` at 0x90) · `addi r11,r28,0x1e; slwi` (`mEnumHandles` at `0x78+4i`). It is also **arithmetically unique** — `0x78 + 4n + 0x138n == 0x7e0` solves only at **n=6** — and a **seventh witness is structural**: DC3's enumerator dispatch has **four** arms where retail's has **three**, because `i == 6` is unreachable. ⇒ **another RB3-vs-DC3 version delta, the third this session** (cf. W16-FG's `XAUDIO2FX_REVERB_PARAMETERS` 0x34-vs-0x38), and like that one it took **retail bytes** to call it a version difference rather than an oracle bug. ★ **It also corroborates the previous lane's `unk70` filler from a constraint that lane never used**: at DC3's 0x74 base the array lands at 0x7dc, not 0x7e0. Per-row attribution sums **exactly**: Init 120 + `fn_82521374` 40 + `fn_8252139C` 40 + `IsCorrupt` 20 + deleting dtor 76 = **296**; one unit moved (`default/ContentMgr_Xbox` 64 → 69), units at 100% unchanged at 191. **Fuzzy movement that banks ZERO, stated as fuzzy so it is not mistaken for bytes:** StartRefresh **69.36 → 99.957**, Poll **0 → 89.65**, ctor **0 → 23.52**. ★★★★ **StartRefresh's 932 B ARE BLOCKED BY A TOOLING GAP, NOT BY SOURCE — and the lane correctly did NOT paper over it.** It sits at 99.957 with two charged `bl` sites: retail names `list<Hmx::Object*>::insert` where we name `list<Content*>::insert`, and **an ICF group already exists at that survivor**. `comdat_fold_gate.py` **refused** it, and the lane **did not install the alias**. The refusal chains through an inner `_M_create_node` whose **stage-1 body compare PASSED** ("15/16 words compared as FULL 32-bit values") before stage 2 refused on a `base_addr` the lane had to **fabricate** — the tool does `int(r["base_addr"],16)` **unconditionally and has NO REPRESENTABLE INPUT for a map-absent folded spelling, which is the strongest CF1 case**. ⇒ **fixing the gate collects 932 B with the body evidence already written down**; this is the "a tool's confident refusal closes veins" pattern with a named, cheap repair. ⛔ **A CORRECTION TO THE BRIEF I WROTE, AND IT COST 27% OF THE LANE'S TOTAL TO FIND:** it filed **nine rows / 384 B** as the drained arg-only class; **only six (264 B) are** — the three 40 B rows read **`mpn == fuzzy` sub-100 on BOTH rulers**, i.e. genuinely divergent. **Two of them crossed as collateral, paying 80 B = 27% of this lane's bytes.** ⇒ **a row misfiled into a closed class is invisible to the lane told to skip it**, and the drained class must be computed as `mpn == 100 AND fuzzy < 100`, never as "fuzzy looks like 99.x". ⛔ **Deliberately NOT done:** `PollRefresh` (824 B) left at **0.0** — DC3's own copy is unsolved at 95.17 and ours is 105 lines against its 139, so **transcription cannot close it**; it is the largest row left in the unit and wants its own lane. The ctor's **31 deletes** say retail **inlined** `ContentMgr::ContentMgr()` where we call it, which ripples into the `ContentMgr` unit and needs a cross-unit A/B. ⚠ **One trap recorded for whoever comes next:** `fn_825208E0` does `lwz r3,0x70(r31)`, which reads like a 4-byte member access **refuting** the bool at 0x70. It is not — **`r31` is the FRAME POINTER there** (`subi r31,r1,0xc0`), and reading it as `this` manufactures a layout conflict that does not exist. **Landing:** the map hazard was live and gated — FI is based four commits back and its map lacked W16-FJ's `0x82b737c0`, so a wholesale take would have **silently deleted** it (the W16-FE shape, invisible to a byte-exact gate); the rebase conflicted exactly once and was resolved **programmatically** (main's map + only this commit's key deltas), verified at **29,583 rows, +4 as declared, 0 lost, 0 renamed, FJ's row intact**. **Every pre-registered key exact, miss 0.000000 on all six**, including `code_pct` **40.446068** — predicted by rounding the byte ratio **through f32**, the correction W16-FJ's 2e-6 miss had established one landing earlier. Native gate `PASS 18/18, 0 SKIPs, rc=0`. |
| **W16-FH** The `/fp:fast` wall on `RndPostProc::LoadRev` was the **divisor SPELLING**, not FF's aggregate temp (opus) | `053e916a` | **+1 / +1,728 B** (Δhonest **+1**; `masked_equal` +0; **+0.016862 pp**) | FF left one named untested hypothesis: that a stack-resident 4-float aggregate at `0x80..0x8c(r31)` denied MSVC 10224 its reciprocal CSE. **Refuted on structure** — that aggregate IS the local `Hmx::Color c`, the compiler's own equate says so (`c$55635 = 128`), so it was the variable, never a temp. The real lever: spell the bloom divisor **inline at each of the three sites** ⇒ three `fdivs`; hoist it ⇒ one `fdivs` + reciprocal `fmuls`, and the f29/f30 constant swap dissolves with it (97.3588 → 97.9954). Crossed on the ObjPtr side (inline DEFER-BOTH ctor + reference-copy of `mBloomColor`) → **100.000000**. `?Load@RndPostProc@@` moved 99.854164 → 99.895836 and was deliberately **NOT** crossed — `matched_code` is all-or-nothing, so that movement pays **0 B** and is reported as fuzzy, not bytes. `RB3_TU_OBJPTR_FORCEINLINE_CTOR` stays reverted (FF measured it net −668 B via `??0RndPostProc@@IAA@XZ` collapsing to 60.9581), gated here at 100.0. ⚠ Landing hazard gated rather than assumed: `Object.h` is a **PCH input** cascading to 965 TUs, and W16-FI's `os/ContentMgr_Xbox.cpp` — landed four commits earlier, **after** FH measured its "exactly 2 rows moved" control — compiles via `msvc_pch` and sits inside that blast radius, so six `ContentMgr_Xbox` rows incl. three sub-100 ones (`Poll@XboxContent` 89.650940, `??0XboxContentMgr` 23.521740, `StartRefresh` 99.957085) were gated as the sharper collateral detectors. All 14 controls held to 6 dp; 1 row crossed, 0 fell out |
| **W16-FL** Five of six fuzzy-0 STL rows are **identification/pin work, not source work** — and writing them would **fabricate content RB3 does not contain** (opus) | `bdefd621` | **+1 / +416 B** (Δhonest **+1**; `masked_equal` +0; **+0.004060 pp**) | Briefed with three mechanisms that make an STL template row read 0.0% and told to classify each **mechanically off the COFF symbol table before writing source**. It ruled out all three and found a **fourth the brief did not list**: five rows are ABSENT from our compiled object entirely, and the four missing element types (`MatPropEditAction`, `StepMoves`, `Unlockable`, the vector-of-vector form) are **DC3 types with ZERO hits in the rb3-Wii RB3 oracle** — proven, not inferred: the Morph row's exact mangled name is **verbatim in DC3's leaked `ham_xbox_r.map` at `0005:0058ae20`**, and `?A0xf8e4b4b5` appears there 62×. Direct proof is JoypadMsgs, whose retail body is **23 instructions** (one virtual call via vtable+0x60, store 0 and 1, return) — which cannot be a `vector<T>` growth helper, since that must allocate/copy/free. ★★★★ **THE DURABLE FINDING: the linker places template COMDATs FAR from their owning TU, so a `.text`-span pin attributes them to whichever unit's span contains them** — our `Sfx.obj` already defines the correctly-named counterpart of the CheatProvider row (`_M_fill_insert_aux@vector<SfxMap>`, identified from retail bytes via its `??0SfxMap@@QAA@ABV0@@Z` call) while `Sfx.cpp`'s other code sits **~4.8 MB away**; that one fact also explains one anon namespace's helpers scattering across **8 pinned units**. The one real source row: `Character::DrawShowing` 416 B, 0.0 → **100.0** — retail (`.fn fn_8236ECD0`, read **by symbol**; the `.s` address column is synthetic) is **104 instructions** and returns right after `EndShadow()` where ours was ~249; removed the edit-mode `mTest->Draw()` block, the `character.show_name` block and the DC3-only `GetGfxMode()==kNewGfx` test, corroborated **independently of both oracles** by a binary string probe (`character.show_name`/`char_draw`/`char_poll` are **absent from the retail XEX**). `PROLOGUE_MISMATCH` + a 33-instruction `REGISTER_SWAP` **dissolved on their own** — symptoms of the oversized body, the standing `REGISTER_SWAP`-is-a-symptom rule again. ⚠ Hand-off, deliberately NOT done: re-home `0x826ffe38` to `Sfx.cpp` (~392 B in play, our object already defines the right symbol) — needs a second distant `.text` block + full re-split A/B, and re-homing is **not** metric-neutral |
| **W16-FK** The reopened `ReverbConvertI3DL2ToNative` (740 B): ported correctly, **DC3's residual TRANSFERS to cl 10224** ⇒ the bytes are uncollectable by source work (fable) | `aeb0faf0` | **+0 / +0 B** — pre-registered and deliberate (row 0.00000 → **99.82703**, mpn 99.98919; `matched_code` is all-or-nothing so it banks **zero**; whole-binary fuzzy **+0.00722 pp**) | ⚠ **I closed this row wrongly in W16-FG** ("MS vendor implementation, XDK porting out of scope") on a census false negative, and **both halves of that were false**: our row is in an **engine** unit, DC3's body is in an **engine** file (`Synth.cpp:106`), and the function is pure field arithmetic calling **no XDK API**. The reopening was justified; the vein turns out closed for a **different reason**. The residual is five `diff_arg` sites at `DecayTime = DecayTime * DecayHFRatio` under the `pack(1)` lowering — source-operand order and per-operand GPRs are **identical** to retail, only **EMISSION order** differs (retail emits the HF `lwz/stw/lfs` group first), charging two `lwz`, two `stw` and the `fmuls` names. **Eight spellings, one full build each, control watched: INERT** (operand swap, `(float)(double)HF`, `HF * 1.0f` — zero-byte wrappers fold before canonicalisation); **REFUTED WITH COLLATERAL** (merged float local 97.50; `float_control(precise)` inserts `bso` after every `fcmpu` ⇒ retail is `/fp:fast`; `optimize("t")` 84.73 ⇒ retail is `/O1`); ★ **POSITIVE CONTROL** — `(float)fabs((double)HF)` is the **one** spelling that flips emission order, and it flips the operand tuple and GPRs with it and costs 8 B: **MSVC puts a non-leaf operand both left AND first, while retail has a leaf-vs-leaf tuple with right-first emission**, which no landable spelling expresses on this compiler. ⇒ scheduling, not regalloc — the permuter would not reach it either. The finding is that **DC3's wall (measured on cl 11886, three failed spellings) TRANSFERS across the compiler-build boundary** rather than being a DC3 artifact. Two things the match cannot prove, verified separately: all **11 pool float literals** checked against retail `.rdata` (11/11 equal, since `name_check` forgives `lbl_` placeholders), and all **22 `XAUDIO2FX_REVERB_PARAMETERS` field offsets/widths below 0x34 retail-verified** across 22 store sites — incl. one `stfiwx f0, r31, r29` (`r29 = 4`) a base-register scan misses, caught only because the count came up one short. RB3's struct is **0x34**, not DC3's 0x38 (FG's retail-byte finding held). FG's "deliberately NOT decompiled" note withdrawn. **Landed on accuracy**: correct engine source where none existed, and a phantom 740 B prize closed with evidence |
| **W16-FM** A folded spelling **absent from the map** was being dropped as unrepresentable — it is the **strongest** ICF case, not a degenerate one (opus) | `2d2815a9` | **+1 / +932 B** (Δhonest **+1**; `masked_equal` +0; **+0.009096 pp**) | `tools/comdat_fold_gate.py:697` read `int(r["base_addr"], 16)` for every candidate, so a spelling the linker folded so hard it left **no name in the map** had no representable input and was silently dropped. New evidence tier **CF5** admits exactly that shape: 5 folded spellings across 2 groups, 6 charged `name_check` sites, `?StartRefresh@XboxContentMgr@@` **99.957085 → 100.0**. ⛔ **THIS IS THE ONE CHANGE WHOSE NATURAL CONTROL IS STRUCTURALLY INCAPABLE OF CLEARING IT.** Installing an alias installs **forgiveness** — objdiff consults `SymbolEquivalences` and drops the charge — so an *unproven* alias lifts `name_check` **by construction**, and the `none` ruler ignores relocation names and reads **+0 whether the fold is real or fabricated**. The lane reported `none` flat and correctly called that **the signature of the hazard, not a clearance**. ★★★★ **So the defence is retail bytes, and I audited the load-bearing claim myself rather than relaying it:** the body at `0x823d14c0` — which the map names `insert<list<Hmx::Object*>>` — branches at **+0x24** to `0x82520150`, which the map names `_M_create_node<list<Dep<CharPollableSorter>*>>`. Read off `build/45410914/asm/CharClipSet.s:1655` keyed on the **`.fn fn_823D14C0` symbol, never the synthetic address column**; nine instructions precede it, so the `bl` sits at exactly +0x24. That branch is **impossible under any typed reading** and is precisely the ICF signature. Our element type is not the wrong one either: `class Content` has **no base class** (`src/system/os/ContentMgr.h:13`) and `RootContent` derives from it (`:50`), so `list<Hmx::Object*>` cannot hold the `RootContent*` `StartRefresh` pushes. **Side-benefit: the body living in `CharClipSet.s` independently corroborates W16-FL's COMDAT-placement finding.** ★★★★ **THE DEFECT THE AUDIT FOUND, fixed in `e94daac6` BEFORE the merge:** the lane added `?insert@?$list@PAVContent@@…` to `folded` while leaving that spelling's **ALIAS-CONSOLIDATION 2026-08-19 withdrawal record LIVE**, so the ledger asserted one membership was **simultaneously proven and fabricated** — and `tools/alias_withdrawal_audit.py`, **which already existed and was not run**, went **4 → 5** live-and-withdrawn (its `FABRICATED_CLOSURE_NOT_PARTITION` bucket 1 → 2). Not a fabrication, which is why the membership stays **live**: that withdrawal was recorded against the fabricated **closure** wholesale — one folded list shared byte-identically across 10 retail addresses, never adjudicated per membership. ★ **The house convention sat two entries above it in the same group** — W16-Y (2026-09-14) **moved** each superseded record out of `withdrawn` into the restored entry's `withdrawal_record_superseded`, which is why the ledger tolerates 4 and not 6. `e94daac6` does the same; post-fix `folded` 5, `withdrawn` 83, `restored` 3, audit back to the inherited **4 / FCNP 1**. ⇒ **an ADDITIVE diff is not a SELF-CONSISTENT one**: the install was verified additive (1,658 groups in and out, 0 survivors lost, 0 groups losing a folded spelling, all 84 withdrawal records byte-identical) and was still incoherent, because the contradiction lived in a record it never touched. The lane also self-reported two silent data-destruction defects it was bitten by and fixed mid-flight: `install()` **replaced rather than unioned** on OWNED groups, first dropping proven folded spellings (the fail-closed drop guard it added **then fired for real**) and then **erasing lane W10-B's population control and its negative control** at `0x823d14c0` — both verified preserved here. ⚠ **Gate inversion, stated because it looks like a failure:** with no source change the honest expectation is **0 MSVC recompiles** (measured 0), so liveness is carried by the **rendered map** re-rendering + `gen_symbol_alias_map.py --check` green + the measures moving — not by a recompile count. And **whole-binary `fuzzy` did NOT move** (50.345950 → 50.345950): a 932 B row gaining 0.042915 pp is ≈0.0000039 pp of the binary, below the sixth decimal, so flatness is arithmetic here — unlike W16-FK, where fuzzy was the *only* liveness evidence. Every pre-registered key exact (44,030 fns · 4,147,612 B · code% **40.476086** by the three-step rule · `masked_equal` 23,245), 1 row crossed, 0 fell out, all 7 cross-lane controls held to 6 dp. ⛔ **Flagged, not fixed:** `icf_alias_finder --validate` FAILs at **base** with 1 CONTRADICTED (`0x82b8c8c8`) — measured on main before the merge, so **inherited**, and fixing it blind would repeat the CF4 mistake; and **`scripts/namecheck_triage.py` is referenced by six files and DOES NOT EXIST**, freezing the 2026-08-12 worklist as non-regenerable. ⚠ **Refined at landing, because the flag as written over-states the damage and under-states the cause:** the tool was **NEVER COMMITTED** — `git log --all -- scripts/namecheck_triage.py` is **0 commits**, i.e. it was a scratch generator that lived in a lane's working directory and never landed. Its three consumers are **healthy**: `namecheck_gate_accounting.py`, `icf_alias_fixpoint.py` and `wrong_callee_triage.py` all **import cleanly** and name it only in **argparse help strings**, as the expected producer of their `sites.jsonl` input. So this is a missing PRODUCER, not three broken tools. The artifacts do survive — several `sites.jsonl` under `~/tmp/laneQ/` — but they are **undated with respect to the 2026-08-12 `name_check` flip that happened the same day**, so which ruler they were charged on is unknown and they must not be believed without re-deriving it |
| **W16-FO** The re-home vein is **4.84x smaller** than I briefed — and the in-tree tool I sent the lane to instead is vacuous in the **opposite** direction (opus) | `5f2031e6` | **+0 / +0 B** (Δhonest **+0**; code% **+0.000000 pp**; fuzzy **+0.003150 pp** — the *only* mover) | I briefed **423 rows / 49,364 B** off `symbol.encode() in open(obj,'rb').read()`. **A SUBSTRING TEST ON A COFF CANNOT TELL *DEFINED* FROM *REFERENCED***, so every **caller** was booked as a **definer**. The lane held row selection constant as a control — reproducing the fuzzy-0 NAMED population **exactly** (5,637 rows / 2,376,232 B), so the DEFINED test was the only variable — and measured the honest vein at **162 rows / 10,200 B**. **81.5% of the bytes I briefed were pure call sites.** ★★★★ **The instrument I pointed it at as the fix is vacuous the OTHER way**: `tools/undefined_externals_census.py:parse_coff_symbols` has arms for `EXTERNAL`/`WEAK_EXTERNAL` and **no `sclass==3` (STATIC) arm**, so file-static bodies — which *are* definitions — read as absent. That is **49 rows / 1,904 B = 18.7% of the honest vein, including its single largest row.** The lane did **not** edit that shared tool; it wrote `tools/rehome_census.py`, tracking `DEF_STATIC` at `StorageClass==STATIC && SectionNumber>0`. ⇒ **a vacuity correction is not automatically a correct instrument — check the new one in both directions.** ★★★ **`DEFINED_ELSEWHERE` is necessary but NOT sufficient: same spelling ≠ same function.** The largest row (576 B `gen_bitlen`) is a **false positive** — I verified it myself in the map at `0x8292f980`, sitting between `?compress_block@D3DX@@` and `?build_tree@D3DX@@`, i.e. **D3DX9's own vendored zlib**. Pinning it would have scored D3DX's copy against ours: metric fitting on out-of-scope XDK code. **Bare C-style names collide across vendored copies; mangled C++ names carry their namespace and do not.** **W16-FL's hand-off is NOT executable as a pin move** — retail's map name at `0x826ffe38` is the DC3 spelling `vector<StepMoves>` while our `Sfx.obj` defines `vector<SfxMap>`; **objdiff pairs by NAME**, and a re-home changes which *base* object is consulted, never the *target* symbol's name, so it stays fuzzy 0. Closing it needs a **coupled map rename**, a riskier lever (un-pairing is 80.5% of a map edit's delta). Declined, and said so. ★★★ **The pin that was moved is the anti-forgiveness control doing its job:** `NoteTube::CreateMeshes` (392 B, stranded ~3.5 KB above NoteTube's five `.text` blocks) went fuzzy **0.00 → 80.714 and STOPPED**, because our body is 400 B against retail's 392 ⇒ `fuzzy < 100` **by construction** and the row pays **0 B**. A row *crossing to 100* here would have meant forgiveness, not a better pin. **A re-home buys LEGIBILITY; the source fix buys the bytes.** ★★★★ **Its P5 was wrong in the most useful way, and I reconciled the arithmetic on the landed tree: dtk RE-CARVES an `auto_*` block around a new pin.** `auto_03_82C29D84_text` 16 rows / 6,888 B → **2 rows / 3,456 B**, plus a **fresh unit `auto_03_82C2AC90_text`** (named for the *end* of the new pin) at 13 rows / 3,040 B, with NoteTube 19 → 20 rows / +392 B. **`6,888 = 3,456 + 392 + 3,040` and `16 = 2 + 1 + 13`, both exact** — a clean partition, which is *why* `total_code` and `total_functions` are untouched. ⇒ **a lane reading 16→2 as damage would revert a clean change.** ★★ **This is a Δ0 lane, so the gate had to be INVERTED (the W16-FK shape): a clean 'nothing moved' is indistinguishable from the merge never having been measured.** Pre-registered that `fuzzy` MUST strictly increase — measured **50.345950 → 50.349100, exactly the predicted +0.003150** — and that the split must really re-carve (`SPLIT ran: 2`, `renamer patched: 1832 files`, symbols.txt drift **0**, 0 re-split iterations). Every key exact: matched 44,030 (miss 0), bytes 4,147,612 (miss 0), `total_code` 10,247,068 (miss 0), **`total_functions` 69,240 (miss 0 — the key I flagged as least certain)**, masked_equal 23,245 (miss 0), code% 40.476086 (miss 0.000000), honest 20,785. 0 rows crossed, 0 fell out. Native gate `PASS 18/18 skipped=0`. ⚠ **What did NOT reproduce, recorded rather than smoothed over:** the *vacuous* instrument itself. Mine read 423/49,364, the lane's 431/55,268, my re-run **481/60,332**. The pin explains −1 row, not +50 ⇒ **the substring test is unstable across tree states** — further evidence against it, but it weakens the lane's "reproduced within 8 rows" as a *control*. ⚠ Also flagged, not done: `gen_bitlen` deliberately unpinned; the 1,188 B / 43-row block unattempted; the 161 mangled rows not individually verified as same-function matches. The lane additionally **self-reported its own selftest was initially vacuous** (a 16-byte synthetic symbol record missing the 2-byte Type field made two checks compare an empty set to an empty set) and fixed it with non-emptiness assertions. |
| **W16-FN** Our `QuestFilterProvider::Text` was **verbatim the rb3-Wii oracle**, and that is exactly why it was wrong (opus) | `7350bb28` | **+4 / +1,320 B** (Δhonest **+4** — `masked_equal` +0, so all four are REAL matches; **+0.012882 pp**) | Briefed prize was 512 B; delivered **1,320 B**. ★★★★★ **THE ORACLE HAND-EXPANDS TWO HELPERS RETAIL EMITS ONCE AND CALLS** — `GetSetlistType` (retail `fn_82B7A078`, three call sites) and `UpdateSongLabel` (retail `fn_82B7B550`, 428 B). Our source copied the expansion faithfully, so `Text` compiled to **928 B against a 512 B target at fuzzy 0**. `UpdateSongLabel` was **already declared AND defined in our tree** — nothing called it, so MSVC emitted nothing. **The fifth instance in a fortnight of the oracle being the defect**: rb3-Wii is a DEV build, and its text is a hypothesis, not ground truth. ★★★★ **A second, independent defect class in the same unit, with a reusable tell:** retail builds its `Symbol`/`Message` tokens as **function-local statics** where our source referenced file-scope `extern` globals. **A global is constructed in a `??__E` dynamic initializer and NEVER inside a body**, so a `Symbol` ctor → `Message` ctor → `atexit` sequence *in a function body* **proves** a local static. The lane read the nine string literals straight out of `orig/45410914/band.exe` because they live in **unpinned `.rdata`, absent from every `.s`**; `setlist_song_fmt` appearing as **two** statics (guard bits `0x1` and `0x4`) is what revealed the declarations sit inside the `if` arms. ★★★★ **NEITHER HALF OF INCREMENT 2 PAYS ALONE — measured cleanly inside one lane.** The map name alone: **Δ0 B** (row went fuzzy 0.0 → **99.85981**, and `matched_code` needs *exactly* 100). The source flip alone: **Δ0 B** (the row stays anonymous). **Jointly +428 B.** The naming bought **legibility**, not bytes — the documented shape, demonstrated rather than cited. ★★★ **A lone `diff_arg` on a `cmpw` plus `diff_op` on its branch is a SOURCE-LEVEL COMPARISON ORDER, not a regalloc wall.** The entire 428 B residual was `cmpw r27,r3`/`blt` against `cmpw r3,r27`/`bgt`; rewriting `iNumSongs <= iSongNum` as `iSongNum >= iNumSongs` closed it. ⛔ And before the fix, target `bl fn_82B7A078` against our `bl ?GetCurrentGigNum@…` **scored a MATCH under placeholder forgiveness** — a completely wrong callee reading as equal ⇒ **a mismatch count is the wrong instrument for target selection on this stratum.** ★ **Correction to my brief, which the lane tested rather than inherited:** I said three other anonymous rows; there were **seven** at fuzzy 0 plus one at 99.5. Testing that figure literally is what surfaced increment 3's 380 B. **Verified by me before landing, not relayed:** the map adds 3 rows and **drops 0**, with **0 value changes across all 29,583 shared keys** (the W16-FE silent-deletion hazard does not apply); the four row sizes sum to **exactly 1,320 B** on main's own `report.json` (512+428+104+276); and the lane's *"injectivity gate green, 29,465 rows"* is a **distinct-VALUE count, not a row count** (29,569 hex keys) — the 102-address excess is **101 null-valued placeholder keys plus 2 duplicated names, and it is INHERITED**, identical at the merge base. ⚠ **One repair of mine is a separate commit on the branch:** the lane shipped its 3 map rows inside a **whole-file re-indent (1 space → 2)**, rendering as **30,751 deletions for a 3-row change**. Content was never touched, but **a map edit that cannot be read as a diff cannot be reviewed**, and it would conflict with every other map lane. Canonical serialization restored — `json.dumps(d, indent=1, ensure_ascii=False)`, **no trailing newline**, recovered by round-tripping main's own copy — content asserted identical; the map diff is now **3 lines**. ★★★ **The three-step `matched_code_percent` rule DISCRIMINATED here, unlike W16-FM:** the f64 ratio `4148932/10247068` gives **40.488967**, but `report.json` stores an **f32** — `40.48896789550781` — which renders **40.488968** at 6 dp. Predicting off naive f64 would have booked a **false miss**. Every key exact: matched 44,034 (miss 0), bytes 4,148,932 (miss 0), code% 40.488968 (miss 0.000000), `total_code`/`total_functions` Δ0, honest 20,789. **4 rows crossed at their exact predicted sizes, 0 fell out** — the map-edit un-pairing falsifier never fired — renames took (no stale `fn_` spellings), `SPLIT ran: 2`, `renamer patched: 1832 files`. Native gate `PASS 18/18 skipped=0`. ⚠ **Deliberately NOT done:** four anonymous rows (**880 B**) left open with evidence recorded rather than landed half-adjudicated. The 192 B pair (`fn_82B7A7D0`/`fn_82B7A8E0`) has **identical call sequences**, so callee-based identification **cannot** separate `GetSongSelectScreen` from `GetDiffSelectScreen` and a 50/50 guess would score ~0 **while masquerading as a body defect** — they must be split on string literals. `GetSetlistType` (`fn_82B7A078`) not named: it sits below this unit's pinned `.text` and would need a **pin re-home, which is not metric-neutral**. No permuter; every residual was structural. |
| **W16-FP** `PollRefresh` crossed **BEYOND the oracle** — DC3's own text sits at **95.17**, and its residual was a **behavioural divergence** (opus) | `c82cfbfd` | **+2 / +864 B** (⚠ **Δhonest +1, NOT +2** — `masked_equal` +1; **+0.008432 pp**) | `?PollRefresh@XboxContentMgr@@UAAXXZ` (824 B) went fuzzy **0.000000 / mpn 1.043689 → 100.0**. ★★ **ACCURACY, PRE-REGISTERED RATHER THAN CLAIMED AFTERWARDS: only ONE of the two functions is honest work.** The other is `fn_82520F50`, PollRefresh's **own 40 B EH funclet**, which was **already `masked_equal=True` on main at fuzzy 99.8** — it pairs by **byte signature**, so it is **disclosure, not work**, and `masked_equal` rises with it. `824 + 40 = 864`. The lane identified this itself; I pre-registered it as falsifier F5 so it could not be quietly booked as +2 honest. ★★★★★ **The lane's first question was the right one — DC3's oracle sits at 95.17, so what is ITS residual?** The retail listing showed a **structurally different function**: a static 8-string ignored table walked with an `/Oi` `strcmp`, **no `0x7000` arm**, the pending result returning to the epilogue, `numItems` uninitialised, a filename-pointer j-loop induction variable, and **the `kMounting` arm's flag polarity INVERTED in DC3**. That last is a **real behavioural divergence, not a spelling difference**. Porting those on retail bytes reached 96.67. ⇒ **the sixth instance in a fortnight of the oracle being the defect, and the FIRST where the oracle's residual was BEHAVIOURAL rather than an inlining or spelling difference.** ★★★★ **The last 3.33 pp was one confound, separated by FOUR spellings.** Our build hoisted the `Callback` vptr load **above** the `Symbol` ctor `bl`, costing an extra `__savegprlr_19`, frame **+0x10** and a rename cascade. Two variables were confounded — vptr-load *placement* vs *how the `Symbol` is read* — and the lane span them: **named local 99.33** (positive control: the vptr moves after the `bl`, but the Symbol is read from a frame slot) · **`const Symbol&` 96.77, REFUTING its own prediction** (loop-invariant frame address) · **naming `*it` inert both ways** · **the implicit `const char*` → `Symbol` conversion AT THE CALL = 100.0**. `Symbol(const char*)` is **declaration-only in both repos**, so this is the front end's **copy-initialisation path, not a visibility effect**. **DC3's own text is its wall — its failed spellings never spanned this mechanism.** ★★★ **A FALSIFIER FIRED AND IT WAS MY INSTRUMENT, NOT THE LANE.** My landing driver aborted on F3 *"leg B recompiled 0 TUs, absent-vs-absent"* — but the build log reads `[3/14] MSVC build/45410914/src/system/os/ContentMgr_Xbox.obj`. **My grep pattern matched lowercase `msvc` where ninja prints `MSVC`.** ⛔ **The same broken pattern sat in the W16-FO driver, where `0 recompiles` was the CORRECT answer for an alias/splits lane — a wrong instrument agreeing with the truth by accident, which is precisely the vacuity class this campaign keeps auditing.** Re-measured with `^\[[0-9]+/[0-9]+\] MSVC `: **exactly 1 edge, and it is the right TU.** ★★★ **The three-step `matched_code_percent` rule needs ALL THREE steps, and W16-FN hid that.** I predicted **40.497398** (f64 ratio → f32, then rendered at 6 dp) and `report.json` stores **40.4974**. **These are the SAME NUMBER**: `f32(40.4974)` equals `f32(4149796/10247068*100)` exactly, and **40.4974 is the SHORTEST ROUNDTRIP decimal** — step 3, which I skipped. On W16-FN the shortest roundtrip *needed* 6 digits (40.488968), so two steps sufficed **by luck**. ⇒ **the apparent 0.000002 miss is a rendering artifact of my prediction, not a measurement discrepancy.** Every other key exact: matched 44,036 (miss 0), bytes 4,149,796 (miss 0), `total_code` 10,247,068 (miss 0), `total_functions` 69,240 (miss 0), `masked_equal` 23,246 (miss 0), honest **20,790**. **2 rows crossed at their exact predicted sizes, 0 fell out.** Source+docs only — no map, no splits, no `objects.json`, so no forced re-split. Native gate `PASS 18/18 skipped=0`. ⚠ **Deliberately NOT done:** `?Poll@XboxContent@@` (424 B, 89.65) and `??0XboxContentMgr@@` (184 B, 23.52) untouched **in the same unit** — the lane did not start rows it could not finish. Six 44 B `fn_825214B0`-family rows at 99.545 / mpn 100 (one relocation-name arg each) not investigated: **map work, not source**. No alias, no map row, no header edit, no `objects.json` flip (the unit is **72/75** and stays `NonMatching`). No DC3 backport. **No permuter — every point was a named source construct.** |
| **W16-FR** A byte-identical twin pair split on **string-literal evidence**, not a coin flip — the only discriminating relocation of four (opus) | `f8ea35c6` | **+4 / +880 B** (Δhonest **+4** — `masked_equal` **+0**, so all four are real; **40.4974 → 40.50599**) | Unit `default/QuestFilterPanel` reaches **71/71 rows on mpn** (units at 100%: 191 → **192**, `MATCHED_ROSE`). ★★★ **THE LANE'S VALUE IS THE SPLIT, NOT THE BYTES.** `fn_82B7A7D0` and `fn_82B7A8E0` (192 B each) are **instruction-for-instruction identical**, differing in exactly **four relocation targets** — and three of them (static guard word, static `Message`, `atexit` thunk) address objects **the function itself creates**, so they would be satisfied by *either* name and carry **no independent meaning**. Only the string literal discriminates. W16-FN deferred this pair rather than guess; a guess had a **50% chance of writing a permanently wrong map entry that the headline would have booked as a gain**. FR read the literals out of retail (`0x82199420` = `get_songselect_screen` ⇒ `GetSongSelectScreen`; `0x82199498` = `get_diffselect_screen` ⇒ `GetDiffSelectScreen`), corroborated on two disjoint methods (retail's argument evaluation order in `HandleFilterSelected`; both rows reaching fuzzy 100 **simultaneously**), and validated its bespoke PE reader on a **positive control that could have failed** (`GetBackScreen`, already at 100, read back exactly `get_backscreen`). ★ **Re-verified independently before landing, in Python — `grep` is binary-blind here:** each of the three literals occurs **EXACTLY ONCE** in the 14,363,648 B image, so the identification is **unique**, not merely consistent. Pre-registered falsifier **F2 (both or neither)** is what made this checkable: exactly one row crossing would have meant the map was now wrong on one side while the score rose. Both crossed. **F1 (fall-out) = 0 rows** — material because naming an anonymous address converts a **forgiven** placeholder call site into a **checked** one, and un-pairing is 80.5% of a map edit's delta; blast radius was *measured* (the only code unit referencing any of the four is `QuestFilterPanel.s` itself), not assumed. **NOT settled, carried forward:** the +880 B are a **joint product** of the map names and the source fix — no naming-alone leg was run, so "naming alone pays 0 B" is **not** established here; `GetSetlistType` (`fn_82B7A078`) stays anonymous (needs a **re-home**, which is *not* metric-neutral); `fn_82B7A260` (40 B, fuzzy 99.5, `masked_equal`) deliberately untouched, and it is the sole reason the unit is 100% on mpn but **70/71** on fuzzy. ⚠ **Coordinator instrument correction, found by this lane's prediction missing by 1 ULP:** `matched_code_percent` is computed **entirely in f32** (`matched as f32 / total as f32 * 100.0`, `objdiff-core/src/diff/mod.rs:1474`), **not** as an f64 ratio narrowed to f32 — the two differ by one ULP near a rounding boundary. Three prior lanes agreed under **both** methods, so the wrong rule was repeatedly "validated" on non-discriminating cases. |
| **W16-FQ** A **zero-byte lane that SHRANK the vein it was sent to mine** — and the prior lane's classifier had gone **vacuous** (opus) | `a3c23166` | **+0 / +0 B** (Δhonest **0**; ⚠ **fuzzy +0.001627 pp** — movement reported as fuzzy, **not** as bytes) | ★★★★ **W16-FB's CLASSIFIER COULD NOT FIRE.** It keyed on the **ABSENCE** of `match_percent_normalized` from a row; **0 of 69,240 rows lack that key** (verified independently before landing). ★ **Sharper than the lane put it, and this is the transferable part:** the *sibling* key `fuzzy_match_percent` **IS absent on 21,585 rows** — protobuf omits defaults for one and not the other, so "absence means zero" is **correct about one key and vacuous about the other in the same JSON**. FB keyed on the always-emitted one. Re-derived with the COFF-defines-symbol check FB had actually validated, the class is **50 rows / 4,944 B** vs the briefed 43 / 5,684 B — row count roughly confirms, **bytes 13% LOWER**. ★★★ And it is **CONTAMINATED**: a wrong map name is indistinguishable from a divergent body *when our object defines the symbol*, so misidentifications land **inside the class on the favourable side** — six rows (532 B) adjudicated on retail bytes proved misidentified, not divergent (`??DHmx` "operator*" is really a `Keys<TexPtr>::Add` caller taking a float in `f1`). ⇒ **4,944 B is an UPPER BOUND, not an estimate.** The signature predicts *a* structural divergence, **not** the briefed mechanism: ≥4 distinct causes present (outlined helper; force-defined `MILO_DEBUG` dev code — `DrawShowing@BandCharacter` is **68 B retail vs 1,004 B ours**; wrong class shape, the 688 B `PracticeSection` cluster; wrong names). **BYTES: ZERO, pre-registered as P2, not excused** — `ComputeElbowPullAndQuat@BandIKEffector` went fuzzy **0.000000 → 92.5**, mpn 4.239130 → 95.108696, body 288 B → 180 B against a **184 B** target (the 4 B gap *is* the one reload we don't emit); `matched_code` is all-or-nothing per row, so 92.5 banks **exactly nothing**. ★★★ **LANDED AGAINST AN INVERTED GATE** (the W16-FK shape): for a Δ0 lane a clean "nothing moved" is **indistinguishable from the merge never having been measured**, so `fuzzy_match_percent` was pre-registered as *required to strictly increase* or the run would be **void rather than confirmed**. It rose **+0.001627 pp**, matching the lane's own figure measured on a different base, and the set-diff shows **exactly ONE row moved in the whole binary** — the target. Same-TU control `ComputeHandPullAndQuat` held at **89.1982** to the digit. ★★ **One tree-wide fix REFUTED BEFORE BEING WRITTEN** by a single read-only measurement: "move `MultiplyTranspose` out of line" (13 TUs) was already the state of the world — we emit **94 `bl`s to it across 48 functions**, `/O1` does not inline it, and several callers match at fuzzy 100 while doing so. Two further measured negatives: FP sum term order **inert**, declaration order **inert** across three orderings. **Not done:** no map edits (six wrong names found, none renamed — naming is a bet with zero call-site upside); `PracticeSection` diagnosed as a class-layout rewrite, not attempted; `DrawShowing` not gated (blunt `MILO_DEBUG` removal measures **−21** whole-binary; 68 B doesn't justify it). |
...
## 7r. EXECUTION LOG — 2026-09-16, second wave (lanes FX, FY, FS + a record correction)

Four merges, all pushed, all verified at **0 attribution trailers** with a
line-anchored check. ⚠ A naive `grep -ciE 'co-authored-by|generated with|claude'`
reads **3** on `w16-fx` — three commit-body lines contain the literal string
"CLAUDE.md", because the lane documented a contradiction it found in that file.
**Anchor the pattern** (`'^(co-authored-by|.{0,4}generated with)'`), which reads 0
on all four merges. A trailer check that fires on any mention of the word is a
check that cannot distinguish a trailer from prose about trailers.

| merge | lane | commits | Δfns | Δbytes | Δhonest | target row |
|---|---|---:|---:|---:|---:|---|
| `9bb905a0` | W16-FX | 2 | +11 | +600 | **0** | `?ParseDataResultsIntoSetlists@MusicLibraryNetSetlists@@QAAX_N@Z` 81.933 → **99.522** |
| `19564f92` | W16-FY | 2 | +22 | +704 | **0** | `?Configure@Accomplishment@@QAAXPAVDataArray@@@Z` 35.361 → **99.992** |
| `138f8f81` | record fix | 1 | 0 | 0 | 0 | — (corrects `19564f92`'s message) |
| `51d4c06d` | W16-FS | 11 | 0 | 0 | 0 | `?UpdateScrolling@VocalTrack@@QAAXM@Z` 72.278 → **91.330** |

MEASURED STATE AT `51d4c06d` (read off `report.json`, not inherited):

```
matched_functions  44,131      matched_code   4,163,348
matched_code_pct   40.629654   fuzzy          50.487220
masked_equal       23,322      honest            20,809
total_code     10,247,068      total_functions   69,240
units all-rows mpn==100: 194   units all-rows fuzzy==100: 172
matched_code = 66.027% of the 61.535% reachable ceiling (6,305,556 B)
gap to ceiling: 2,142,208 B
```

★ **The wave composes onto 7q's state with miss 0 on both keys** — 44,098+33 and
4,162,044+1,304, against lane sums of +33 and +600+704+0. FX and FY were based on
*different* mains (`c31808c1` and `37e9cb40`) and both composed exactly, each with
a pre-registered statement that a miss would mean interaction. Deltas compose;
absolutes do not.

### 7r.1 ★★★ THE ENTIRE WAVE IS DISCLOSURE — honest is EXACTLY flat

**Δmasked_equal = +33, which equals Δmatched_functions to the unit.** Honest
(`matched − masked_equal`) is **20,809 before and 20,809 after**. Not one of the
33 functions gained this wave is an honest match; every one is a funclet paired
by byte signature. Across the whole day: **+91 functions = +15 honest / +76
disclosure.**

This is not a complaint about the lanes — FY genuinely took a 2,584 B body from
35 to 99.99 — it is a statement about **where the credit lands**. Which leads to
the sharper finding:

### 7r.2 ★★★★ NOT ONE OF THE THREE TARGET ROWS CROSSED. Every byte came from neighbours.

Re-read at `51d4c06d`, all three rows are **below 100 on both rulers**:

```
FX   1,968 B  fuzzy  99.52235  mpn  99.98984
FY   2,584 B  fuzzy  99.99226  mpn  99.99226
FS   8,948 B  fuzzy  91.32990  mpn  92.23514
```

Because `matched_code` is **all-or-nothing per row at `fuzzy == 100`**, all three
banked **exactly zero bytes on the row they were dispatched to fix** — 13,500 B of
target body, 0 B collected. FX's +600 and FY's +704 are entirely *neighbourhood*:
EH funclets and a sibling ctor that crossed because the TU around them got right.

⇒ **This is "price the BLOCK, not the row" arriving as the default case rather
than the exception.** A brief that prices a lane at its target row's size is
wrong by construction unless it also states what has to happen for that row to
reach 100. FY's row sits **0.008 pp short** behind a single relocation-name
charge — which is why the follow-on alias lane is worth the full 2,584 B and is
the only piece of today's work with a genuine *honest* payout available.

### 7r.3 ★★★★★ A SCRIPT GENERATED BY SUBSTITUTION NEEDS A CHECK THAT THE SUBSTITUTION COVERED WHAT IT SHOULD

One sed-generated landing driver produced **three distinct defects across two
lanes**, and the instructive part is that their failure modes were radically
different:

| defect | mechanism | failure mode |
|---|---|---|
| worktree path `wt-w16-fy` vs real `wt-w16fy` | `w16-fx`→`w16-fy` rewrote a path that never had the dash | **LOUD** — `FileNotFoundError`, drive aborted, main untouched |
| `predict_w16fx.txt` / `msg_w16fx.txt` survived **both** generations | they contain `w16fx` (no dash) and no `fx_`, so neither pattern matched | ⛔ **SILENT** |
| `verify_ruler_agreement.py` → `verif**s**_ruler_agreement.py` | `fy_`→`fs_` hit the substring inside **veri`fy_`** | rc=2, "file not found" |

⛔ **The silent one shipped.** `19564f92` — the merge that landed W16-FY — carries
**W16-FX's merge message verbatim**; the two merges print identical subject lines.
`git merge -F` does not care whether the file describes the merge. Caught only by
grepping the *next* generation of the script before running it. **Corrected
forward** in `138f8f81`, never amended — the history is published.

★ The third defect is the one worth internalising: my cross-lane grep could not
catch it, because `verifs_` is not a lane reference. The rule that would have
caught all three is **check that the substitution covered what it should**, not
"check for cross-lane leakage".

★ And the measurement was **unaffected** — FY's gates used the correct symbol,
the correct 35.36068 pre-state and the correct predicted absolutes, every key
miss 0. **A mislabelled record and a wrong measurement are different failures,
and conflating them would have justified re-running work that was sound.**

### 7r.4 ★★★★★ A GATE THAT CANNOT FIND ITS ROW MUST FAIL, NOT PASS

FX's Gate C used a wrong mangled spelling — `?ParseDataResultsIntoSetlists@...@@AAAXXZ`
(private, void) where the real row is `...@@QAAX_N@Z` (public, takes a bool). The
gate reported "target row NOT FOUND" and, because that case was coded as
`sys.exit(3)`, it **failed closed**. Had "row not found" been coded as "no
mismatch", it would have passed vacuously and certified nothing.

Same family as the two vacuity traps already in this doc (`grep`-binary,
`all([])`). Worth stating as a rule: **the absence of the thing you are checking
is never a pass.**

### 7r.5 ★★★★ MSVC PACKS UP TO 32 FUNCTION-LOCAL STATICS INTO ONE GUARD INT

FY's biggest lever was replacing 32 `extern Symbol` globals from `Symbols*.h`
with 32 function-local `static Symbol`s. I re-derived the mechanism on retail
bytes by a different method than the lane used, because it corrected my own brief:

> `.fn fn_82594EF8` holds **65 references to `lbl_82DFEE58`**; per-static shape
> `lwz / clrlwi.|rlwinm. / bne / ori / stw`; **16 `ori` + 16 `oris` immediates,
> union `0xFFFFFFFF`, popcount exactly 32**, and 32 stores back.

⛔ **Two corollaries, both of which had already misled me:**
1. **"No `??_B` in the diff" is NOT evidence against local statics** — one packed
   guard int serves up to 32, so no per-variable guard symbol exists.
2. **A mask census keyed on `andi.`/`andis.` finds ZERO.** MSVC *claims* the bit
   with `ori`/`oris` and *tests* it with `clrlwi.`/`rlwinm.`. My first census read
   0 for exactly this reason and I nearly booked it as a refutation.

### 7r.6 ★★★ ACCURACY CAN COST HEADLINE — and FS was landed on that basis

FS's C19 variant scores **0.36 pp lower** than its C18 variant, and C19 is what
landed. C19 has **exact size identity** (target 8,948 == base 8,948, gated on
objdiff's own `Target Size`/`Base Size`) and **fewer non-equal rows** (618 → 595).
C18 scored higher while being structurally further from retail. Consistent with
the standing directive that a truer denominator beats a higher number.

FS also handed forward two **untried** levers, both concrete: the stack frame is a
**permutation** of ours, not a shift (both `-0x370`) — `0xc0↔0xb4`, `0xb0↔0xd0`,
`0x80↔0x170`, `0xd0..0xd8↔0xc0..0xc8`, `0x170↔0x160` — and **declaration order
controls stack slots**, so reordering `staticFirst/staticLast/staticLeftX/staticY`,
`beginPos`, `isolated`, `lyricPhrases` attacks the 86 frame-slot rows; plus retail
caches `this+0x2e8` in a stack slot at entry (`stfs f0,0x160(r1)`, row 26).
Charged-site inventory at C19: regalloc 204 · other-arg 115 · frame-slot 86 ·
delete 81 · insert 81 · replace 16 · branch-target 9 · diff_op 3.

### 7r.7 Two model corrections, both in the direction of less confidence

- **A ROW COUNT AND A BYTE COUNT ARE ON DIFFERENT RULERS.** FX predicted 11
  crossings and measured **12**, while bytes (+600) and functions (+11) were
  **exact**. `??0NetSavedSetlist` was already at `mpn == 100` with `fuzzy` 98.5,
  so it crossed the *fuzzy* bank — **+160 B with +0 functions**. 160 + 11×40 = 600.
  The prediction was right on both measures it named and wrong on the one it
  inferred.
- **The linear aggregate-fuzzy model** `Δ = row_size × Δfuzzy / total_code` came
  in **4.4% high** on FS (predicted +0.016637, measured +0.015933); FQ measured it
  2% high. **The ±2% band declared in 7q was too tight and is withdrawn.** Use the
  model for **sign and order of magnitude only** — the sign gate is the one that
  does work.

### 7r.8 ★★★★ objdiff `--analyze`'s "Function Call Diff" section is ALIAS-BLIND

Already-forgiven ICF folds appear there as live divergences. **Two of the six
sub-targets I briefed for FX were phantoms read straight out of it.** Any
"target only" / "base only" entry must be checked against
`scripts/symbol_aliases.json` before it is briefed as work.

### 7r.9 Open, and deliberately not done

- **W16-FZ dispatched** (Opus, map-only): install the `push_back<vector<Symbol>>`
  fold FY **adjudicated but deliberately declined to install** — bundling a map
  edit with source changes would defeat `ab_measure`'s `ALIAS_SUSPECT` guard,
  which fires only on map-only patches. FLAT T1 **REFUTED** (bodies byte-identical
  112 B both, relocation targets differ per-`T`); **CHASED T1 PROVEN** (every
  differing slot resolves to a callee pair that itself folds), with `--chasetest`
  confirming the instrument can both pass and fail. Retail calls one address
  `0x822D16F8` from all three loops; `symbol_aliases.json` (1,658 groups) has **no
  group for this survivor**. Worth the row's full **2,584 B**, and — uniquely
  today — **+1 honest**.
- **W16-GA dispatched** (Opus): census the packed-guard signature of 7r.5 across
  all retail function bodies, find the rows where we use `Symbols*.h` externs and
  retail used local statics, then fix the best one. Game layer preferred.
- **FY left 18 of 89 rows** in `default/Accomplishment` untouched (71/89 at 100).
- **W16-CX is LANDED** — its completion notice claimed "never merged, never
  pushed". The branch tip `5c1c842c` **is an ancestor of main**, which settles it.
  ⚠ But the four SHAs the notice named (`e006c248`, `3d2d6829`, `56354b32`,
  `621c38e4`) are **pre-rebase**: they are on no current branch and
  `git merge-base --is-ancestor` returns **false for all four**, while each
  subject appears **exactly once** in main. ⇒ **On a lane that landed by
  rebase+merge, a SHA-ancestry check answers "was this exact object merged", not
  "did this work land" — and it answers NO for work that is fully present.** This
  is the `UNMERGED ≠ UNLANDED` trap wearing a different hat; check the branch tip
  or the subjects, never the pre-rebase SHAs. Its two open leads survive: the
  `ChordbookPanel::Load` ↔ `SetlistToStorePanel::Load` fold at `0x825f5920`, and
  `??$__ucopy_aux@PBQAVBandProfile@@...` in MetaPerformer (4 B at fuzzy 95.0).
- **Not folded into CLAUDE.md yet**: FX found that **declaration order is NOT
  inert for register assignment** on `ParseDataResultsIntoSetlists` (three variants
  spread 1.29 pp). The doc's *conclusion* holds; its stated *mechanism* does not.
- **Drained — do NOT re-fund**: FS's phrase-loop F1 shape (one of only three in
