...
**43,123 / 3,925,452 B / 38.3122 %** (ledger row `76a32373`; W15-D, W15-E, W15-F, W16-A, W16-B and W16-C each additive to the byte). `total_code` 10,245,956
...
| queued (opus) | opus | W15-D handoffs (`THUNK_PERMUTATION_2026-09-14.md` §5): the 20 body rows proven misnamed but needing a pin re-home; the four DC3-onto-RB3 pins (`HamLabel.cpp`→`BandLabel`, `PostProcer`→`SpotlightEnder`, `HamIKEffector`→`SetlistToStorePanel`, `CharUpperTwist.cpp` inside `DialogDisplay`'s run); fix W15-B's `probe.py` `.pdata` length decode (`>>2` should be `>>8`) | re-homes are NOT metric-neutral (PINHOME-1); expect small ± per row, and `BandLabel` 4 % becomes a real source target |
| queued (fable) | fable | W15-D's two r3/r4 thunk-SHAPE mismatches at 50 % — `SynthEmitter::Handle`, `UILabelDir::PreLoad` — are signature divergences in our source (hidden-struct-return vs not); plus the six compiler-vs-retail vtable slot-count divergences (`TourSavable`, `PracticePanel`, `Server`, `PlatformMgr::Callback`, `DxTexRenderer` ×2) — each is a class-layout defect the native port inherits | bug exposure; VTABLE + STRUCT work is the user's stated high-value class |
