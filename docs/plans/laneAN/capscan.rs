...
fn main() {
    ...
    match mode.as_str() {
        ...
        "one" => {
            ...
            for u in &units {
                if u.name != want { continue; }
                let (Some(tp), Some(bp)) = (&u.target, &u.base) else { eprintln!("no paths"); continue };
                eprintln!("t={tp:?} exists={} b={bp:?} exists={}", tp.exists(), bp.exists());
                let t = objdiff_core::obj::read::read(tp, &c, DiffSide::Target);
                let b = objdiff_core::obj::read::read(bp, &c, DiffSide::Base);
                match (&t, &b) {
                    (Ok(t), Ok(b)) => {
                        eprintln!("target code syms={} base code syms={}", code_syms(t).len(), code_syms(b).len());
                        let mut bs: BTreeMap<Vec<u8>, Vec<usize>> = BTreeMap::new();
                        for i in code_syms(b) { if let Some(s)=funclet_signature(b,i){ bs.entry(s).or_default().push(i);} }
                        for i in code_syms(t) {
                            let sym=&t.symbols[i];
                            if !is_fn_anon(&sym.name) { continue }
                            let Some(sig)=funclet_signature(t,i) else { eprintln!("{} nosig",sym.name); continue };
                            match bs.get(&sig) {
                                Some(v)=>eprintln!("{} size={} HIT {}", sym.name, sym.size, b.symbols[v[0]].name),
                                None=>eprintln!("{} size={} miss", sym.name, sym.size),
                            }
                        }
                    }
                    (a, bb) => eprintln!("read err t={:?} b={:?}", a.as_ref().err(), bb.as_ref().err()),
                }
            }
        }
        "pairscan" => {
