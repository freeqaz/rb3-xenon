...
| c | Shared-`src/` edits `HX_NATIVE`-gated, X360 arm token-for-token | ✅ **PASS — 4 files** | `obj/DirLoader.cpp`, `world/Instance.cpp`, `char/CharBonesSamples.{cpp,h}`. All changes wholly inside `#ifdef HX_NATIVE`; the match build passes no `/D`, so its token stream is unchanged. The `CharBonesSamples` fix is a **native-port LE defect** (rb3-Wii is big-endian and has no swap code at all), not a decomp divergence — **no objdiff A/B debt**. |
