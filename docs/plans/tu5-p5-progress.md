...
| unwired-owner scatter-wiring (NEW VEIN) | **18,689** | **+52** (P2 CubeTex `3917a0e4` +9; sweep `f51aec9c` +43) — **already-ported-but-unwired .cpp in-tree** (428 total, not in objects.json → no obj emits them); retail scattered their COMDATs into wired spans → near-free `#include "<owner>.cpp"` append pairs them. LightHue←Reflection +24, FxSendMeterEffect←FxSendWah +14, CubeTex←4 rnddx9 owners +9, MoviePanel←MetaMusicManager +4. One seed pulls the whole COMDAT cluster. ~60-65% flip rate; MI-dtors ride base-layout delta (skip). Probe P1 TrackWatcher = NO-WAVE mirage (own methods done, 78 "bodies" = foreign scatter interleave). Clean scatter-include sub-vein DRAINED (7 cross-unit cands, 5 flipped); residual 428-pool needs STANDARD wiring (objects.json+splits+map) or is DC3-only-no-target |
| missing-template-instantiation (NEW VEIN, triage-classifier find) | **18,691** | **+2** (`ba690393`) — retail TUs out-of-line template COMDATs (`ObjectDir::Find<T>`) that our `dynamic_cast<T*>(FindObject(...))` call sites never instantiate; one explicit-instantiation line at owner-file tail flips strict. BandWardrobe `Find<CharWeightSetter>` + `Find<BandRetargetVignette>`. 17 more ZS-MISSING-INSTANTIATION candidates + full-0%-pool expansion sweep in flight (est +8-14). Found by `scripts/triage/divergence_triage.py` ZERO-SCATTER sub-split |
| missing-instantiation harvest | **18,698** | **+7** (`zsinst` commit) — Find<RndPostProc>, by-value MakeString ×2, SetTokenFmt<char*>, Util::Log<float> (dc3 body-port), BinStream>>Key<float>/Key<Color>. Expansion sweep: +7 new cands beyond classifier's 19, 5/7 flipped. **Discriminator: by-value MakeString flips; const-ref (`AB` mangling) = ODR wall.** Other walls: third-party template classes not in tree (XAUDIO2/ATL), APO virtual stubs, MI-dtor thunks deferred. Vein ~DRAINED for one-liners |
| grind foreman wave (Sonnet, VocalPlayer) | **18,705** | **+7** — VocalPlayer body-ports all→100: HookupTrack, PostLoad, ResetScoring, Restart, LocalScorePhrase, UpdateCrowdMeter, UpdateMicDisplay. Levers: function-local static Message/Symbol (shared guard word, lexical bit order); **branch-assigned bool constants** (`bool x; if(c) x=true; else x=false;` → li 0/1 no clrlwi, `int` if read unmasked) = BOOL_MASK insert-direction IS fixable for locals; retail drops count-loop+assert blocks; `vector(3,0)`; trust target asm over oracle conditions |
| grind foreman waves 2-3 package | **18,710** | **+5** — PushCrowdReaction (int-cast→signed cmpwi), ShowBriefBandMessage (local-static Symbol), SetupForBandLogo (member re-read not param), TestMaterialTextures (arg-eval temp), PatchPanel fn_ cascade (PatchDir.h operator-new lever, header full-rebuild verified). Campaign score 13 flips/3 improves/0 regressions in 24 assignments. **GROUND TRUTH for classifier: shape-screened waves 12/15 vs unscreened 1/9; I/D-cluster≥3 = strongest flip predictor; regswap-only = auto at_limit; 97.5-99.8 band wall-dominated (survivor bias), 78-96 flips. New wall sigs: GameMode vtordisp (4 hits), dead-arg scheduling (3 hits)** |
| BODY-LEVER calibration wave + alloca lever | **18,717** | **+7** — 30-fn stratified sample: **measured flip rate 6.7% vs classifier 80% price (~12x overprice)**; only productive stratum = live 70-90 non-STL (25%); STL rows 0/6 confirmed contamination; mispairs 9/30 dominant FP class. **FORCE-MULTIPLIER LEVER: `static` alloca wrapper -> `__forceinline` in oggvorbis/codec.h inline-expands the _alloca intrinsic — 6 vorbis fns from 1 line** (+ VocalTrackDir::UpdateConfiguration body-port). Intrinsic-wrapper sweep across other C-lib headers in flight |
| bankable grind: MECH-LEVER | **18,725** | **+8** — LEVER-SYMBOL vein (function-local static Symbol + /DRB3_HANDLE_LOCAL_STATIC, 2 bonus anon flips). **Recalibration: LEVER-SYMBOL 44% not 90%; LEVER-STRING ~5% (heuristic near-noise, flags ObjPtr-2ctor/regalloc); ZS-INST 0% drained (MakeString.h already by-value)** |
| bankable grind: STRUCT-ARTIFACT calibration | **18,729** | **+4** — FileMergerOrganizer/CharEyes/HamListRibbon(+cascade). **Calibration: true rate ~12-23% not 50-70%; 68% mislabeled (STL-template mispairs dominant); bimodal (low-band layout bugs + 99.9% STL-stride, mid-band 0/9)**. SongCollision +2 gated (sibling mispair → correlator) |
| bankable grind: BODY-LEVER | **18,742** | **+13** — 13/22 flipped (**59% = 2.4x the 25% prior; shape-screen is the multiplier**). Levers: RefPtrOf(it)->RefOwner() x2, remove-DC3-newer-guard (delete-cluster, x4 highest yield), noinline out-of-line wrapper, ternary->branchless, loop-shape+drop-(int)-cast. Walls all known bail sigs. **~59 of 96-fn bucket UNTAPPED → ~35 more flips at this rate** |
| inverse correlator | **18,744** | **+2** (`invcorr`) — target_symbol_map repointed for 2 unique-byte-identical true-mispairs (__final_insertion_sort<MemDiffEntry>, _List_base<OldMMInst>::clear); reloc-verified, 0 regressions. Vein THIN (2/122) but tool generalizes over ZERO-UNMAPPED. SongCollision=inlining-not-map (stays gated), GemManager=rotated-neighborhood cluster |
| BODY-LEVER drain (Wave A) | **18,761** | **+17** (`bodylever drain`) — **ObjPtrList push_back/copy-ctor force-multiplier** (routed through insert() hidden-return temp → rewrote __forceinline direct Link + new Node → cascade +8 off-worklist). Wave1 73%/Wave2 25%→stopped. **VEIN DRAINED** (remaining 70-90 pool = permuter/FPR-sched/struct/ObjPtr-2ctor walls) |
| ZERO-UNMAPPED correlator scaling (user-chosen lane) | **18,819** | **+58** (`corrscale`) — re-ran the 3-stage reloc-masked byte-identity scanner stack at 18,761; **72 fresh anchors from this session re-opened the pool**. Stage1 +37, ICF-disambig +6, target-twin +15. Every entry = guaranteed strict flip (already byte-identical, just unpaired). Fixed point round 3; 0 regressions. **KEY: correlator is a per-milestone ONE-SHOT — ~72 body-flips → ~58 ident-harvest conversion; re-run after each body-flip batch (the flywheel)** |
