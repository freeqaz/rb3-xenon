...
- **`fn_82516320` (= `PlatformMgr::SetDiskError` per W16-J's relay, 8 `bl` callers) is a `blr` stub whose 208 B `.pdata` extent has ABSORBED an unrelated fragment.** Measured on retail: `.pdata` entry begin 0x82516320 (packed word `0xc0003404`), next begin 0x825163F0 ⇒ 0xD0 = 208 B; `config/45410914/symbols.txt:147625` `fn_82516320 … size:0xD0` mirrors `.pdata`, so the aggregate ORIGINATES IN RETAIL `.pdata`, not in symbols.txt (W16-J's "phantom aggregate" hypothesis is half right: the phantom is real, the source is the unwind table). Instruction 0 is `blr`; the following 204 B use saved r29–r31 with NO prologue in the extent, contain the `stw r4,0x34(r3)` that `PlatformMgr.h:125`'s comment refers to, and carry two raw-displacement branches dtk could not resolve (`b 0x2473c0` / `b 0x2473ac` → 0x8275d6f0 / 0x8275d6e4, both INSIDE `?DataSet@@YA?AVDataNode@@PAVDataArray@@@Z` @0x8275d670..0x8275d6fc) plus `bl`s to `??0DiskErrorMsg@@QAA@XZ` (0x82514ff0), `?GetDiskErrorCallback@@YAP6AXXZXZ` (0x8251b860), `?Release@DataArray@@QAAXXZ`, `Sleep` and `atexit`, and a tail `b` into the shared epilogue at 0x828292ac. The preceding `fn_825162F0` is only 0x28 B, so the swallowing is by the `blr` stub's own record, not by a predecessor. ⇒ a splits/map lane's problem (re-carve at 0x82516324 with its own symbol; the `stw r4,0x34(r3)` field-offset claim in the header rests on the fragment, not on SetDiskError). NOT touched by W16-M.
