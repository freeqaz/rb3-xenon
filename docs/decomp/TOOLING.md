...
| `atexit_fuzzy_verify.py` | **RETIRED 2026-09-11 (lane W4-F)** | Deleted. Promotions are `sync_match_percent.py --promote` (graded by construction); the WITHHELD report is `tools/shape_families.py --withheld`; `--mark-at-limit` deliberately not ported. See `docs/decomp/TOOLING4_2026-09-11.md`. | n/a | n/a |
...
| `classify_nearmiss.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: objdiff-cli fdc5113 changed the JSON `args` spelling and INVERTED its headline verdict — NAME_RELOC to 0, OFFSET filled with phantoms; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline) | Classify the mismatch CAUSE of near-miss functions via objdiff per-fn JSON. For each function in a match band, run objdiff diff -f json and bucket eac | --help | n/a |
...
| `enrich_unattributed.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: objdiff-cli fdc5113's `args` spelling dropped the trailing relocation, cutting CALL_NAMING by 78%; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline) | Sub-classify the UNATTRIBUTED near-miss bucket by instruction-level signature. The fork's pattern detector finds no recognized pattern for ~560 real-b | --help | n/a |
...
| `layout_fix_rank.py` | WORKING — **every ranking it produced before 2026-08-17 is INFLATED toward struct-layout evidence; re-run before citing** (task #114: its `addi` branch built the base register as `'r' + m.group(2)` where group 2 already carried its `r`, so bases were spelled `rr1`/`rr12`/`rr31` and, not being in `STACK_REGS`, every `addi/subi` off the stack pointer / r31 frame alias / r12 funclet establisher was bucketed as STRUCT evidence. Compounding it, the flat `args` spelling of 2026-01-31→2026-08-16 killed the `imm(rBase)` branch, so only three-register `addi/subi` forms parsed at all. Measured paired over all 2128 near-miss fns in [80,100): the as-shipped tool's **stack bucket was structurally EMPTY — 1385 offset rows, 100.0% called struct**; corrected it is 1097 struct / 2416 stack (31.2% struct). Fns credited with struct evidence 689 → 470. Coherent units 21 → 16, keystone clusters 13 → 12, and the **#1 keystone cluster does not survive** (+8, 5 units/13 fns → 1 unit/2 fns, rank 6). The tool is repaired at `b57f9e7e`; re-run of record `<decomp-bench>/archive/runs/rb3x-layout-fix-rank-rerank-2026-08-17/`) | Rank candidate struct/base-class LAYOUT fixes by empirical fan-out. Separates struct-field reads from stack/frame noise, rolls up per unit, and clusters units by shared dominant delta into "keystone candidates". | --help, --selftest | n/a |
...
| `member_delta_finder.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: objdiff-cli fdc5113's `args` spelling zeroed the load/store channel AND the stack-noise rejection channel; its `--bucket` default also named a bucket true_progress cannot emit, so the default invocation scanned zero functions; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline) | detect the DC3 DROPPED/ADDED-MEMBER force-multiplier. THE PATTERN (proven wins: CharSleeve/CharIKSliderMidi mMe -0xC; Gem Tail -0x14; | --help | n/a |
| `member_delta_finder2.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: inherits member_delta_finder's zeroed load/store channel through the imported `collect_accesses`; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline) | CLASSIFY member-offset delta candidates. v2 adds a CLASSIFY stage on top of v1's detection core. Candidates are tagged: | --help | **YES** |
...
| `progress_ledger.py` | WORKING | **Longitudinal gap tracker — the instrument that tells a RULER CHANGE from a REGRESSION.** Appends per merge commit to the COMMITTED `docs/decomp/progress_ledger.jsonl`: headline measures on BOTH rulers (`matched_functions`=mpn, `matched_code`=fuzzy — never conflated), the reachability_census gap strata (imported, not copied), and the TOOL PROVENANCE report.json self-declares (objdiff version/commit/binary hash, resolved `functionRelocDiffs` ruler, diff_config hash, **and the ICF alias-map hash** — a ruler input worth ~7.9pp). `classify` names the mechanism (SOURCE_PROGRESS / REGRESSION / RULER_OR_TOOL_CHANGE / DENOMINATOR_CHANGE / MIXED / INDETERMINATE) and always prints which evidence it used AND which it lacked. Built after the 44,444-vs-42,276 event cost a whole survey lane. Run it at LANDING time, next to `snapshot_landing.py`; it is deliberately NOT a ninja edge. | `--help`; `selftest --prove-can-fail` (10/10, rejects an "everything is progress" classifier on 9) | n/a |
...
| `split_imm_offset.py` | WORKING — **every split it produced before 2026-08-16 is VOID; re-run before citing** (task #104: its `0xNN(rY)` regex could not match the flat `args` spelling objdiff emitted 2026-01-31→2026-08-16, so STACK/STRUCT were structurally empty and every memory diff fell to CONST; the parser itself is correct under objdiff-cli 4.2) | Split the IMM_OFFSET near-miss sub-bucket into STACK vs STRUCT vs CONST. For each IMM_OFFSET fn, pull the instruction diff and look at the differing | --help | n/a |
...
| `true_progress.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: objdiff-cli fdc5113's `args` spelling reclassified STRUCT_OFF as REG and collapsed DATA_LBL; per-function BUCKETS were unaffected, the void fields are each row's `sub_classes`/`sub_counts`/`diff_samples`; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline) | Honest "true progress" classifier for the near-miss pool. Motivation (LTO/ICF investigation, 2026-06-06): the project owner suspected the | --help | n/a |
...
| `wall_classify.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: objdiff-cli fdc5113's `args` spelling zeroed VBASE_VTBL_RE and four detectors — FUNCLET_PAIRING stopped routing DEFER_DEEP, FPR struct-offset diffs misrouted to PERMUTE; 11 of 80 measured functions get a different route; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline). **Its regression suite is now a HERMETIC FIXTURE suite** (task #117, 2026-08-17): `--validate` scores 14 hand-verified cases against frozen verbatim `objdiff-cli` JSON under `tools/testdata/wall_validate/`, needs no build tree or toolchain, and `--mutation-matrix` proves it discriminates — 14/14 repaired vs 9/14 with the ruler-I defect re-introduced. The old live-tree `--validate` scored an identical 4/11 on broken and repaired code; `--validate48` was deleted as vacuous (29 of its 48 symbols no longer resolve). | `--help` | `--validate`, `--mutation-matrix` |
