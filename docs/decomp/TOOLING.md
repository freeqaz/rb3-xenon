...
| `wall_classify.py` | WORKING — **every output it produced between 2026-08-16 and 2026-08-17 is VOID; re-run before citing** (task #103: objdiff-cli fdc5113's `args` spelling zeroed VBASE_VTBL_RE and four detectors — FUNCLET_PAIRING stopped routing DEFER_DEEP, FPR struct-offset diffs misrouted to PERMUTE; 11 of 80 measured functions get a different route; the tool is repaired and its numbers are restored to the pre-2026-08-16 baseline) | Auto-tag HAS_REAL near-miss functions with playbook wall classes. Implements the 8 wall detectors from docs/decomp/playbooks/hasreal-grind.md §3 plus | --help | n/a |
