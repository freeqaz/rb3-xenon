...
|   |   | ✅ **CROSSED by W16-C (Fable), 2026-09-14 — the refusal was WRONG.** The operand order is not canonicalised; it is optimisation-HISTORY-dependent (retail's own five strength-reduced sites split BASE/IV with identical schedules). A codegen-free `Symbol cur = Symbol(*it);` temp + reverting the compensating `->` at the section site gave fuzzy 100.0, **+12,220 B / +0.119267 pp / Δfns 0**, settled `ab_measure` on `name_check`. Full mechanism, refuted hypotheses and the `/FAs` screening harness: `docs/decomp/NEXTSONGPANEL_COMMUTE_AUDIT_2026-09-14.md`. |
