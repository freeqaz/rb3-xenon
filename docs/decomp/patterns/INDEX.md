...
- [two-objdiff-entry-points-two-rulers.md](two-objdiff-entry-points-two-rulers.md) — **`objdiff-cli diff` and `report generate` were two different rulers.** Different hardcoded base configs; `ppc.calculatePoolRelocations` synthesizes "fake" pool relocations only one side reconstructs, and `reloc_eq` charges the target-only ones under every ruler but `none`. 102 fns / 55,604 B read LOW through the per-function path here. Fixed in `objdiff.json` options; guard `scripts/verify_ruler_agreement.py`
