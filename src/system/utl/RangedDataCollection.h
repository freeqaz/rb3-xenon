...
public:
    ...
    int SizeAt(int idx) {
        if (idx >= mRangeDataArray.size()) {
            return 0;
        } else
            return mRangeDataArray[idx].size();
    }

    void GetNthStartEnd(int iDifficulty, int i2, int &start, int &end) const {
        MILO_ASSERT(iDifficulty < mRangeDataArray.size(), 0xB9);
        const RangedData<T> &data = mRangeDataArray[iDifficulty][i2];
        start = data.mStartTick;
        end = data.mEndTick;
    }
...
public:
    ...
    RangeSection(int i, float f)
        : unk0(i), unk4(f), unk8(3.4028234e+38f), unkc(-3.4028234e+38f) {}
    int unk0; // tick
    float unk4;
