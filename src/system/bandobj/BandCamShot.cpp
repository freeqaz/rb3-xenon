    ...
    if (gRev >= 15 && gRev <= 28) {
        ...
        if (trigs.size() != 0) {
            ...
            for (int i = 0; i < (int)trigs.size(); i++) {
                ...
                calls->back().mDir = Dir();
                calls->back().mEvent =
                    Dir()->Find<EventTrigger>(trigs[i].trigger.Str(), false);
            }
        }
    }
    if (gRev >= 16 && gRev <= 28) {
...
void BandCamShot::TeleportTarget(
    RndTransformable *t, const Transform &tns, bool b3
) {
    t->SetLocalXfm(tns);
...
DataNode BandCamShot::OnListAnimGroups(const DataArray *da) {
    ...
    if (handled.Type() != kDataUnhandled) {
        ...
        if (c) {
            ObjectDir *thedir = c->Driver() && c->Driver()->ClipDir()
                ? c->Driver()->ClipDir()
                : Dir();
