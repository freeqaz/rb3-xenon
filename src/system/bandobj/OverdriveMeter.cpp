...
OverdriveMeter::OverdriveMeter()
    : mState((State)0), mResetTrig(this, 0), mSpotlightPhraseSuccessTrig(this, 0),
      mBeDeployingTrig(this, 0), mBeFillingTrig(this, 0), mBeReadyTrig(this, 0),
      mPulseMiloTrig(this, 0), mNoOverdriveTrig(this, 0), mExtendAnimGroup(this, 0),
      mPulseAnimGroup(this, 0), mTestEnergy(0) {}
...
void OverdriveMeter::SetNoOverdrive() {
    if (mNoOverdriveTrig)
        mNoOverdriveTrig->Trigger();
}

void OverdriveMeter::MiloReset() {
    if (mPulseMiloTrig)
        mPulseMiloTrig->Trigger();
    Reset();
    SetShowing(true);
}
...
void OverdriveMeter::EnergyReady(Symbol s, bool b, float f) {
    ...
    if (b) {
        ...
        if (seq)
            ...
            MILO_WARN("%s not found in %s", seqname, mPathName);
    }
}
