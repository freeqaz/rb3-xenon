...
void BandScoreboard::SetNumStars(float f, bool b) {
    if (mStarDisplay)
        mStarDisplay->SetNumStars(f, b);
}

float BandScoreboard::GetNumStars() const {
    if (mStarDisplay)
        return mStarDisplay->GetNumStars();
    else
        return 0;
}
...
void BandScoreboard::SetupScore() {
    if (mNumMeshes.empty()) {
        ...
        const char *separatorStr = Localize(sSep, 0);
        ...
        for (int i = 0; i < 8; i++) {
            ...
            mNumMeshes.push_back(
                ObjPtr<RndMesh>(this, Find<RndMesh>(meshname, true))
            );
        }
        for (int i = 0; i < 10; i++) {
            const char *meshname = MakeString("%d_source.mesh", i);
            mSrcMeshes.push_back(
                ObjPtr<RndMesh>(this, Find<RndMesh>(meshname, true))
            );
        }
    }
...
BEGIN_PROPSYNCS(BandScoreboard)
    SYNC_PROP_SET_STATIC(score, mScore, SetScore(_val.Int()))
    SYNC_PROP_SET_STATIC(num_stars, GetNumStars(), SetNumStars(_val.Float(), true))
    SYNC_PROP_STATIC(star_display, mStarDisplay)
    SYNC_SUPERCLASS(RndDir)
END_PROPSYNCS

BEGIN_HANDLERS(BandScoreboard)
    HANDLE_ACTION_STATIC(reset, Reset())
