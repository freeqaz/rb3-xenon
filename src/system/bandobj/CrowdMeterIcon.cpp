...
CrowdMeterIcon::CrowdMeterIcon()
    : mResetTrig(this, 0), mArrowHideTrig(this, 0), mArrowShowTrig(this, 0),
      mDeployTrig(this, 0), mStopDeployTrig(this, 0), mStateFailedTrig(this, 0),
      mStateFailingTrig(this, 0), mStateNormalTrig(this, 0), mGlowTrig(this, 0),
      mGlowStopTrig(this, 0), mStateQuarantinedTrig(this, 0), mDropInTrig(this, 0),
      mDropOutTrig(this, 0), mIconLabel(this, 0), mIconStateAnim(this, 0), unk240(0),
      mState(kCrowdMeterNormal), mQuarantined(0) {}
...
void CrowdMeterIcon::SyncObjects() {
    ...
    if (!mArrowHideTrig)
        mArrowHideTrig = Find<EventTrigger>("arrow_hide.trig", true);
    if (!mArrowShowTrig)
        mArrowShowTrig = Find<EventTrigger>("arrow_show.trig", true);
    if (!mDeployTrig)
        mDeployTrig = Find<EventTrigger>("deploy.trig", true);
    if (!mStopDeployTrig)
        mStopDeployTrig = Find<EventTrigger>("stop_deploy.trig", true);
    if (!mStateFailedTrig)
        mStateFailedTrig = Find<EventTrigger>("state_failed.trig", true);
    if (!mStateFailingTrig)
        mStateFailingTrig = Find<EventTrigger>("state_failing.trig", true);
    if (!mStateNormalTrig)
        mStateNormalTrig = Find<EventTrigger>("state_normal.trig", true);
    if (!mGlowTrig)
        mGlowTrig = Find<EventTrigger>("glow.trig", true);
    if (!mGlowStopTrig)
        mGlowStopTrig = Find<EventTrigger>("glow_stop.trig", true);
    if (!mStateQuarantinedTrig)
        mStateQuarantinedTrig = Find<EventTrigger>("state_quarantined.trig", true);
    if (!mDropInTrig)
        mDropInTrig = Find<EventTrigger>("drop_in.trig", true);
    if (!mDropOutTrig)
        mDropOutTrig = Find<EventTrigger>("drop_out.trig", true);
    if (!mIconLabel)
        mIconLabel = Find<BandLabel>("icon.lbl", true);
    if (!mIconStateAnim)
        mIconStateAnim = Find<RndAnimatable>("icon_state.anim", true);
}
...
void CrowdMeterIcon::SetGlowing(bool glow) {
    if (glow)
        ...
        mGlowStopTrig->Trigger();
}

void CrowdMeterIcon::SetIcon(const char *cc) { mIconLabel->SetIcon(*cc); }
...
void CrowdMeterIcon::ArrowShow(bool show) {
    if (show)
        mArrowShowTrig->Trigger();
    else
        mArrowHideTrig->Trigger();
}
