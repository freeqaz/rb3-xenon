...
void BandHighlight::Update() {
    ...
    if (textarr) {
        ...
        unk13c = unk130 = unk148->LocalXfm().v;
    }
...
void BandHighlight::UpdateTargetEdge(RndTransformable *t) {
    if (unk148) {
        ...
        if (lbl) {
            ...
            float f4;
            if (txt) {
                f4 = txt->GetStringWidthUTF8(txt->GetText().c_str(), 0, false, 0);
            } else
                f4 = 0;
            unk13c = unk130 = unk148->LocalXfm().v;
            unk13c.x = f4;
...
void BandHighlight::SyncDir() { mResource->Dir()->SetWorldXfm(WorldXfm()); }
...
// (The Lane-AE b2 scatter force-emit of ?StaticClassName@BandButton@@ was removed
// when system/bandobj/BandButton.cpp was wired and 0x823438C8..0x82345034 was
// carved out of this unit -- BandButton now emits its own OBJ_CLASSNAME COMDAT.)
