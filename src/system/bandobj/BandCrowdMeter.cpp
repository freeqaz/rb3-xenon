...
void BandCrowdMeter::Poll() {
    ...
    if (!mDisabled && !LOADMGR_EDITMODE) {
        ...
        int idx = 0;
        for (ObjPtrList<RndGroup>::iterator it = mOrderedPeaks.begin();
             it != mOrderedPeaks.end();
             ++it, ++idx) {
...
TrackInstrument GetTrackInstrument(Symbol s) {
    if (s == guitar)
        ...
        return kInstKeys;
    else if (s == real_guitar)
        return kInstRealGuitar;
    else if (s == real_bass)
        return kInstRealBass;
    else if (s == real_keys)
        return kInstRealKeys;
    else if (s == pending)
        return kInstPending;
    else if (s == pending_vocals)
        return kInstVocals;
    else {
...
void BandCrowdMeter::Disable() { mDisabled = true; }

bool BandCrowdMeter::Draining() const {
    if (mDisabled)
        return false;
    for (int i = 0; i < mIconData.size(); i++) {
        if (mIconData[i].mUsed && mIconData[i].unk18)
            return true;
    }
    return false;
}
...
bool BandCrowdMeter::Deploying() const {
    if (mDisabled)
        return false;
    for (int i = 0; i < mIconData.size(); i++) {
        if (mIconData[i].mUsed && mIconData[i].unk1a)
            return true;
    }
    return false;
}

void BandCrowdMeter::Enable() { mDisabled = false; }

bool BandCrowdMeter::Disabled() const { return mDisabled || mIconData.empty(); }

void BandCrowdMeter::SetCrowd(float f) {
    if (mCrowdMeterAnim)
        mCrowdMeterAnim->SetFrame(f, 1.0f);
...
void BandCrowdMeter::SetMaxed(bool b) {
    mMaxed = b;
    if (mTrackPanel)
        mTrackPanel->PushCrowdReaction(mMaxed);
}
...
void BandCrowdMeter::ShowPeakArrow(bool b) {
    if (b)
        mShowPeakArrowTrig->Trigger();
    else
        mHidePeakArrowTrig->Trigger();
}
...
void BandCrowdMeter::UpdateJoinInProgress(bool b1, bool b2) {
    if (!b1 != unk234) {
        unk234 = !b1;
        if (b1)
            mCanJoinTrig->Trigger();
        else
            mCannotJoinTrig->Trigger();
    }
}

void BandCrowdMeter::FailedJoinInProgress() { mJoinInvalidTrig->Trigger(); }

BandCrowdMeter::IconData::IconData(
    BandCrowdMeter *bcm, CrowdMeterIcon *icon, RndGroup *grp
)
    : unk0(bcm, icon), unkc(bcm, grp), unk18(0), unk19(0), unk1a(0), mUsed(0), unk1c(0),
      unk20(0) {}
...
BEGIN_HANDLERS(BandCrowdMeter)
    ...
    HANDLE_ACTION(
        ...
        SetPlayerIconState(GetTrackInstrument(_msg->Sym(2)), (CrowdMeterState)_msg->Int(3))
    )
...
void BandCrowdMeter::PreLoad(BinStream &bs) {
    ...
    if (!IsProxy()) {
        if (gRev < 3) {
            ObjVector<ObjPtr<RndGroup> > groupvec(this);
            bs >> groupvec;
        }
        if (gRev >= 2)
            bs >> mLevelColors;
...
void BandCrowdMeter::SyncObjects() {
    if (!mBandEnergyDeployTrig)
        mBandEnergyDeployTrig = Find<EventTrigger>("band_energy_deploy.trig", true);
    if (!mBandEnergyStopTrig)
        mBandEnergyStopTrig = Find<EventTrigger>("band_energy_stop.trig", true);
    if (!mDisabledStartTrig)
        mDisabledStartTrig = Find<EventTrigger>("disabled_start.trig", true);
    if (!mDisabledStopTrig)
        mDisabledStopTrig = Find<EventTrigger>("disabled_stop.trig", true);
    if (!mShowPeakArrowTrig)
        mShowPeakArrowTrig = Find<EventTrigger>("show_peak_arrow.trig", true);
    if (!mHidePeakArrowTrig)
        mHidePeakArrowTrig = Find<EventTrigger>("hide_peak_arrow.trig", true);
    if (!mCrowdMeterAnim)
        mCrowdMeterAnim = Find<RndMatAnim>("crowd_meter.mnm", true);
    if (!mCanJoinTrig)
        mCanJoinTrig = Find<EventTrigger>("can_join.trig", true);
    if (!mCannotJoinTrig)
        mCannotJoinTrig = Find<EventTrigger>("cannot_join.trig", true);
    if (!mJoinInvalidTrig)
        mJoinInvalidTrig = Find<EventTrigger>("join_invalid.trig", true);
    if (mIconData.size() != 5) {
        mIconData.clear();
        for (int i = 0; i < 5; i++) {
    ...
    if (mExcitementTrigs.size() != 5) {
        ...
        mExcitementTrigs.push_back(ObjPtr<EventTrigger>(
            this, Find<EventTrigger>("excitement_boot.trig", true)
        ));
        mExcitementTrigs.push_back(ObjPtr<EventTrigger>(
            this, Find<EventTrigger>("excitement_bad.trig", true)
        ));
        mExcitementTrigs.push_back(ObjPtr<EventTrigger>(
            this, Find<EventTrigger>("excitement_okay.trig", true)
        ));
        mExcitementTrigs.push_back(ObjPtr<EventTrigger>(
            this, Find<EventTrigger>("excitement_great.trig", true)
        ));
        mExcitementTrigs.push_back(ObjPtr<EventTrigger>(
            this, Find<EventTrigger>("excitement_peak.trig", true)
        ));
    }
    RndDir::SyncObjects();
}
