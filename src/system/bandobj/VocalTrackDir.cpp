...
const char *DataToString(const DataNode &node) {
    switch (node.Type()) {
    case kDataInt:
        return MakeString("%d", node.Int());
    case kDataFloat:
        return MakeString("%f", node.Float());
    case kDataVar:
        return MakeString("%s", DataVarName(node.Var()));
    case kDataFunc:
        return MakeString("%s", DataFuncName(node.Func()));
    case kDataObject:
        if (node.NotNull())
            return MakeString("%s", node.GetObj()->Name());
        else
...
void VocalTrackDir::PostLoad(BinStream &bs) {
    ...
    if (!IsProxy()) {
        ...
        if (gRev < 2) {
            MILO_NOTIFY_ONCE(
                "VocalTrackDir class has changed significantly since this file was last saved.  Many parameters will need to be updated."
            );
            Hmx::Color col;
            std::vector<Hmx::Color> cols;
            ObjPtr<Hmx::Object> objectPtr(this, 0);
            ...
            if (Type() == vocal_harmony_prototype) {
                ...
                for (int i = 0; i < mTypeProps.Size(); i++) {
                    if (Property(mTypeProps.Key(i), true)->Type()
                        == mTypeProps.Value(i).Type()) {
                        MILO_LOG(
                            "\tSetting property %s to %s\n",
                            mTypeProps.Key(i).Str(),
                            DataToString(mTypeProps.Value(i))
                        );
...
void VocalTrackDir::TrackReset() {
    ...
    if (BandTrack::mParent) {
        ...
        if (mHarmText)
            mHarmText->SetText("");
        if (mLeadPhonemeText)
            mLeadPhonemeText->SetText("");
        if (mHarmPhonemeText)
            mHarmPhonemeText->SetText("");
        for (int i = 0; i < 3; i++)
            GetPitchArrow(i)->Clear();
    }
    if (mPlayerFeedback) {
...
void VocalTrackDir::Tambourine(Symbol s) {
    mTambourineSmasher->Handle(Message(Symbol(s.Str())), true);
}
...
void VocalTrackDir::SetVocalLineColors(VocalHUDColor *color) {
    if (mStreakMeter) {
        for (int i = 0; i < 3; i++)
            mStreakMeter->SetPartColor(i, color[i]);
...
DECOMP_FORCEACTIVE(
    VocalTrackDir, "slider.sld", "arrow_param.anim", "vocal_param.anim", "set_enabled"
)
...
void VocalTrackDir::UpdateVocalMeters(bool b1, bool b2, bool b3, bool b4) {
    if (mStreakMeter) {
        ...
        if (b4)
            mStreakMeter->ForceFadeInactiveParts();
    }
}
...
void VocalTrackDir::ShowPhraseFeedback(int i1, int i2, int i3, bool b) {
    ...
    if (mStreakMeter)
        mStreakMeter->ShowPhraseFeedback(i1, b);
...
void VocalTrackDir::ShowMicDisplay(bool b) { mVocalMics->SetShowing(b); }
...
void VocalTrackDir::CanChat(bool show) {
    Find<RndDir>("chat", true)
        ->Find<EventTrigger>(show ? "show.trig" : "hide.trig", true)
        ->Trigger();
}
...
void VocalTrackDir::SetPlayerLocal(float f) {
    ...
    if (BandTrack::mParent && BandTrack::mParent->HasNetPlayer() || mSimulatedNet)
        ...
        if (mPlayerFeedback)
            mPlayerFeedback->Find<RndEnviron>("environ.env", true)
                ->SetProperty(fade_max, DataNode(1));
    }
}
...
void VocalTrackDir::Reset() {
    ...
    UpdateConfiguration();
    if (mPitchArrow1)
        mPitchArrow1->Reset(mArrowFXDrawGrp);
    if (mPitchArrow2)
        mPitchArrow2->Reset(mArrowFXDrawGrp);
    if (mPitchArrow3)
        mPitchArrow3->Reset(mArrowFXDrawGrp);
    RndGroup *tubesGrp = Find<RndGroup>("tubes_milo.grp", false);
    if (tubesGrp)
        tubesGrp->SetShowing(false);
    if (BandTrack::mParent) {
...
void VocalTrackDir::SyncObjects() {
    RndDir::SyncObjects();
    if (!mLeftTrans)
        mLeftTrans = Find<RndTransformable>("track_left.trans", false);
    if (!mRightTrans)
        mRightTrans = Find<RndTransformable>("track_right.trans", false);
    if (!mBottomTrans)
        mBottomTrans = Find<RndTransformable>("track_bottom.trans", false);
    if (!mTopTrans)
        mTopTrans = Find<RndTransformable>("track_top.trans", false);
    if (!mPitchBottomTrans)
        mPitchBottomTrans = Find<RndTransformable>("pitch_bottom.trans", false);
    if (!mPitchTopTrans)
        mPitchTopTrans = Find<RndTransformable>("pitch_top.trans", false);
    if (!mPitchMidTrans)
        mPitchMidTrans = Find<RndTransformable>("pitch_mid.trans", false);
    if (!mNowTrans)
        mNowTrans = Find<RndTransformable>("now_bar.trans", false);
    if (!mPitchArrow1)
        mPitchArrow1 = Find<PitchArrow>("pitch_arrow_1", false);
    if (!mPitchArrow2)
        mPitchArrow2 = Find<PitchArrow>("pitch_arrow_2", false);
    if (!mPitchArrow3)
        mPitchArrow3 = Find<PitchArrow>("pitch_arrow_3", false);
    if (!mTubeRangeGrp)
        mTubeRangeGrp = Find<RndGroup>("tubes.grp", false);
    if (!mTubeSpotlightGrp)
        mTubeSpotlightGrp = Find<RndGroup>("spotlight.grp", false);
    if (!mTubeBack0Grp)
        mTubeBack0Grp = Find<RndGroup>("back0.grp", false);
    if (!mTubeBack1Grp)
        mTubeBack1Grp = Find<RndGroup>("back1.grp", false);
    if (!mTubeBack2Grp)
        mTubeBack2Grp = Find<RndGroup>("back2.grp", false);
    if (!mTubeFront0Grp)
        mTubeFront0Grp = Find<RndGroup>("front0.grp", false);
    if (!mTubeFront1Grp)
        mTubeFront1Grp = Find<RndGroup>("front1.grp", false);
    if (!mTubeFront2Grp)
        mTubeFront2Grp = Find<RndGroup>("front2.grp", false);
    if (!mTubeGlow0Grp)
        mTubeGlow0Grp = Find<RndGroup>("glow0.grp", false);
    if (!mTubeGlow1Grp)
        mTubeGlow1Grp = Find<RndGroup>("glow1.grp", false);
    if (!mTubeGlow2Grp)
        mTubeGlow2Grp = Find<RndGroup>("glow2.grp", false);
    if (!mTubePhoneme0Grp)
        mTubePhoneme0Grp = Find<RndGroup>("phoneme0.grp", false);
    if (!mTubePhoneme1Grp)
        mTubePhoneme1Grp = Find<RndGroup>("phoneme1.grp", false);
    if (!mTubePhoneme2Grp)
        mTubePhoneme2Grp = Find<RndGroup>("phoneme2.grp", false);
    if (!mVocalsGrp)
        mVocalsGrp = Find<RndGroup>("vocals.grp", false);
    if (!mScroller)
        mScroller = Find<RndTransformable>("scroller.trans", false);
    if (!mLeadLyricScroller)
        mLeadLyricScroller = Find<RndTransformable>("lead_lyric_scroller.trans", false);
    if (!mHarmonyLyricScroller)
        mHarmonyLyricScroller =
            Find<RndTransformable>("harmony_lyric_scroller.trans", false);
    if (!mBREGrp)
        mBREGrp = Find<RndGroup>("bre.grp", false);
    if (!mLeadBREGrp)
        mLeadBREGrp = Find<RndGroup>("lead_bre.grp", false);
    if (!mHarmonyBREGrp)
        mHarmonyBREGrp = Find<RndGroup>("harmony_bre.grp", false);
    if (!mPitchScrollGroup)
        mPitchScrollGroup = Find<RndGroup>("pitch_scroll.grp", false);
    if (!mLeadLyricScrollGroup)
        mLeadLyricScrollGroup = Find<RndGroup>("lead_lyric_scroll.grp", false);
    if (!mHarmonyLyricScrollGroup)
        mHarmonyLyricScrollGroup = Find<RndGroup>("harmony_lyric_scroll.grp", false);
    if (!mBeatAnimsGrp)
        mBeatAnimsGrp = Find<RndGroup>("beat_anims.grp", false);
    if (!mArrowFXDrawGrp)
        mArrowFXDrawGrp = Find<RndGroup>("arrow_fx_draw.grp", false);
    if (!mTambourineSmasher)
        mTambourineSmasher = Find<RndDir>("tambourine_smasher", false);
    if (!mTambourineNowShowTrig)
        mTambourineNowShowTrig = Find<EventTrigger>("tambourine_now_show.trig", false);
    if (!mTambourineNowHideTrig)
        mTambourineNowHideTrig = Find<EventTrigger>("tambourine_now_hide.trig", false);
    if (!mVocalMics)
        mVocalMics = Find<RndDir>("vocals_mics", false);
    if (!mLeadPhraseFeedbackBottomLbl)
        mLeadPhraseFeedbackBottomLbl =
            Find<class BandLabel>("lead_phrase_feedback_bottom.lbl", false);
    UpdateTubeStyle();
}

void VocalTrackDir::ConfigPanels() {
    if (mFontStyle)
        ApplyFontStyle(mFontStyle);
    float f8 = 0;
    if (mHarmLyrics && BandTrack::mParent) {
        if (BandTrack::mParent->UseVocalHarmony())
            f8 += 1.0f;
    }
    if (mRemoteVocals || !mPitchWindow)
        f8 += 2.0f;
    if (TheRnd.GetAspect() == Rnd::kWidescreen)
        f8 += 10.0f;
    Find<RndAnimatable>("config.anim", true)->SetFrame(f8, 1.0f);

    mTrackBottomZ = mBottomTrans->LocalXfm().v.z;
    mTrackTopZ = mTopTrans->LocalXfm().v.z;
    mNowBarX = mNowTrans->LocalXfm().v.x;
    mPitchBottomZ = mPitchBottomTrans->LocalXfm().v.z;
    mPitchTopZ = mPitchTopTrans->LocalXfm().v.z;
    mTrackRightX = mRightTrans->LocalXfm().v.x;
    mTrackLeftX = mLeftTrans->LocalXfm().v.x;
...
void VocalTrackDir::SetRange(float min, float max, int tonic, bool b) {
    if ((b || min != mLastMin || max != mLastMax || tonic != mTonic) && min < max
        ...
        if (tonic != mTonic) {
            if (tonic == -1) {
                Find<RndAnimatable>("pitch_window_mat_config.anim", true)
                    ->SetFrame(1.0f, 1.0f);
                tonicRef = 60;
            } else if (mTonic == -1) {
                Find<RndAnimatable>("pitch_window_mat_config.anim", true)
        ...
        if (range < unk6d8) {
            ...
            MILO_WARN(
                "massive pitch range in vocal HUD: [%.0f, %.0f], probably a bug", min, max
            );
...
void VocalTrackDir::UpdateTubeStyle() {
    if (mTubeStyle) {
        ...
        if (!mHarm1PhonemeMat
            && mTubeStyle->Property(harmony_1_phoneme, true)->NotNull()) {
            mHarm1PhonemeMat =
                mTubeStyle->Property(harmony_1_phoneme, true)->Obj<RndMat>();
        }
        if (mTubeStyle->Property(same_as_harmony_1, true)->Int()) {
            ...
            if (!mHarm2FrontMat
                && mTubeStyle->Property(harmony_2_front, true)->NotNull()) {
                mHarm2FrontMat =
                    mTubeStyle->Property(harmony_2_front, true)->Obj<RndMat>();
            }
            ...
            if (!mHarm2PhonemeMat
                && mTubeStyle->Property(harmony_2_phoneme, true)->NotNull()) {
                mHarm2PhonemeMat =
                    mTubeStyle->Property(harmony_2_phoneme, true)->Obj<RndMat>();
            }
        }
        mGlowSize = mTubeStyle->Property(glow_size, true)->Float();
        mGlowAlpha = mTubeStyle->Property(glow_alpha, true)->Float();
        if (mLeadGlowMat)
            mLeadGlowMat->SetAlpha(mGlowAlpha);
        if (mHarm1GlowMat)
            mHarm1GlowMat->SetAlpha(mGlowAlpha);
        if (mHarm2GlowMat)
            mHarm2GlowMat->SetAlpha(mGlowAlpha);
    }
}
...
void VocalTrackDir::Extend(bool b) {
    ...
    if (tnm) {
        if (b)
            tnm->SetFrame(tnm->EndFrame(), 1.0f);
        else
            tnm->Animate(tnm->StartFrame(), tnm->EndFrame(), tnm->Units(), 0, 0);
    }
}
...
void VocalTrackDir::SortArrowFx() {
    if (mArrowFXDrawGrp
        && (PitchArrow::NeedSort(mPitchArrow1) || PitchArrow::NeedSort(mPitchArrow2)
            || PitchArrow::NeedSort(mPitchArrow3))) {
...
BEGIN_HANDLERS(VocalTrackDir)
    HANDLE_ACTION(
        ...
        SetConfiguration(_msg->Obj<Hmx::Object>(2), (HarmonyShowingState)2)
    )
    ...
    HANDLE_ACTION(
        set_feedback_state,
        ShowPhraseFeedback(_msg->Int(2), _msg->Int(3), _msg->Int(4), false)
    )
    ...
    HANDLE_ACTION_IF(
        rebuild_hud, BandTrack::mParent, BandTrack::mParent->RebuildVocalHUD()
    )
...
void VocalTrackDir::UpdatePartIsolation() {
    if (unk6c8) {
        if (BandTrack::mParent)
            BandTrack::mParent->RebuildVocalHUD();
        if (mStreakMeter)
            mStreakMeter->SetIsolatedPart(unk6c4);
        unk6c8 = false;
    }
}
...
int VocalTrackDir::NumVocalParts() {
    if (BandTrack::mParent)
        return BandTrack::mParent->GetNumVocalParts();
    else
        return 0;
}
