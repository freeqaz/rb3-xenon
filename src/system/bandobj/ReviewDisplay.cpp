...
void ReviewDisplay::DrawShowing() {
    ...
    mFocusAnim->SetFrame(GetState() == kFocused ? 1.0f : 0.0f, 1.0f);
...
void ReviewDisplay::Update() {
    ...
    // (Symbols4.h / Symbols2.h), so each declaration must sit immediately
    // before its use or a sibling scope silently binds the global instead.
