...
void BandWardrobe::LoadMainCharacters(BandCamShot *shot) {
    ...
    // Retail's `mic` is a FUNCTION-LOCAL static constructed right here, at the
    // merge point after the `if (shot)` block (guard word 0x82CBE108 bit 1),
    // even though it is not used until the vocals loop far below.
    ...
    if (InstrumentIndex(syms, mic) != syms.size()) {
        for (int i = 0; i < 4; i++) {
            ...
            if (inst == "none"
                && (bchar->mGender == mVocalGender || mVocalGender.Null())) {
...
DataNode BandWardrobe::OnEnterVignette(DataArray *da) {
        ...
        for (ObjDirItr<Character> it(worldDir, true); it; ++it) {
            ...
            if (!driver) {
                ...
                if (strstr(name, "extra")) {
                    ...
                    if (visemes) {
                        CharLipSyncDriver *lsd =
                            it->Find<CharLipSyncDriver>("vignette.lipdrv", false);
                        if (lsd)
                            lsd->SetClips(visemes);
                        CharFaceServo *fs =
                            it->Find<CharFaceServo>("face.faceservo", false);
                        if (fs)
                            fs->SetClips(visemes);
                    }
                    ...
                    if (ws)
                        ws->SetWeight(0.0f);
                    CharLipSyncDriver *lsd2 =
                        it->Find<CharLipSyncDriver>("vignette.lipdrv", false);
                    if (lsd2) {
                        CharLipSync *ls =
                            charsDir->Find<CharLipSync>(MakeString("%s.lipsync", name), false);
                        lsd2->SetLipSync(ls);
                        lsd2->Sync();
                    }
                }
    ...
    if (charsDir) {
        for (int i = 0; i < 4; i++) {
            ...
            CharLipSyncDriver *lsd = bc->Find<CharLipSyncDriver>("vignette.lipdrv", false);
            if (lsd) {
                CharLipSync *ls =
                    charsDir->Find<CharLipSync>(MakeString("%s.lipsync", name), false);
                lsd->SetLipSync(ls);
