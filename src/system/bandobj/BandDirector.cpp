...
bool BandDirector::DirectedCut(Symbol s) const {
    return strncmp(s.Str(), "directed_", 9) == 0;
}

bool BandDirector::BFTB(Symbol s) const {
    return strncmp(s.Str(), "BFTB_", 5) == 0;
}
...
void GetVenuePath(FilePath &fp, const char *cc) {
    ...
    if (*cc == '\0' || streq(cc, "none")) {
        ...
        for (int i = 0; gVenues[i] != 0; i++) {
            char *str = strstr((char *)cc, gVenues[i]);
            if (str) {
...
bool BandDirector::IsMusicVideo() {
    return strstr(mVenue.Name().Str(), "video") != nullptr;
}
...
BandCharacter *BandDirector::GetCharacter(int idx) const {
    if (TheBandWardrobe)
...
void BandDirector::HarvestDircuts() {
    if (mPropAnim && mVenue.Dir()) {
        ...
        if (!TheBandWardrobe->DemandLoadSym()) {
            ...
            if (strncmp(playmode.Str(), "coop", 4) == 0) {
                if (playmode == coop_bg)
                    mask = 0x100000;
...
DataNode BandDirector::OnCycleShot(DataArray *da) {
    ...
    if (wdir) {
        BandCamShot *shot =
            dynamic_cast<BandCamShot *>(wdir->GetCameraManager()->ShotAfter(mCurShot));
        ForceShot(shot);
    }
    return 0;
}
...
void BandDirector::OnMidiPresetCleanup() {
    ...
    if (!keys)
        return;
    LightPresetManager *pm = &mVenue.Dir()->GetLightPresetMgr();
    ...
    for (int i = 0; i < skeys.size(); i++) {
        ...
        if (lpreset && i > 0) {
            ...
            MILO_WARN(
                "Can't find light preset %s, %f secs", s1.Str(), skeys[i].frame / 30.0f
            );
        }
    }
...
DataNode BandDirector::OnShotAnnotate(DataArray *da) {
    ...
    if (!mPropAnim)
        ...
        if (!keys || 0 > i7)
            ...
            if (strneq(symval.Str(), "directed_", 9)) {
...
DataNode BandDirector::OnSetDircut(DataArray *da) {
    ...
    if (wdir && !NoWorlds()) {
        ...
        std::vector<CameraManager::PropertyFilter> filts;
        mNextShot =
            dynamic_cast<BandCamShot *>(wdir->GetCameraManager()->FindCameraShot(sym, filts));
    }
