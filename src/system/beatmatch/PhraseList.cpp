...
bool PhraseTickCmp(const Phrase &p, int i) { return p.mTick + p.mDurationTicks < i; }

void PhraseList::Clear() { mPhrases.clear(); }
...
void PhraseListCollection::AddPhrase(
    BeatmatchPhraseType ty, float ms, int ticks, float dur_ms, int dur_ticks
) {
