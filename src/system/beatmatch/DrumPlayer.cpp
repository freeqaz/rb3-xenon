...
DrumPlayer::DrumPlayer(SongInfo &info) {
    ...
    mAttenuation =
        SystemConfig("beatmatcher", "audio")->FindFloat("drum_sample_attenuation");
}

DrumPlayer::~DrumPlayer() { mDrumKitBank = 0; }

void DrumPlayer::Play(int i, float f) {
    static const char *seq_names[9] = { "kick.cue",    "snare.cue", "tom1.cue",
                                        "tom2.cue",    "crash.cue", "hat.cue",
                                        "hatopen.cue", "ride.cue",  "tom3.cue" };
