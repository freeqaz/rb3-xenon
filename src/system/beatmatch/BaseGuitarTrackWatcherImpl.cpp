...
BaseGuitarTrackWatcherImpl::BaseGuitarTrackWatcherImpl(
    ...
    : TrackWatcherImpl(track, u, slot, data, gemlist, parent, cfg, 2),
      mLastLateGemHit(-1.0f), mLastNoStrumGemHit(-1), mLastNoStrumGemSwung(-1),
      mMostRecentHit(-1000.0f), mGemNotFretted(-1), mFretWhenStrummed(-1),
      mFretWaitTimeout(0.0f), mFretSlop(60.0f) {}
...
void BaseGuitarTrackWatcherImpl::TryToHopo(float f, int i, bool b1, bool b2) {
    ...
    if (f != mMostRecentHit) {
        ...
        if (CanHopo(closestgem)) {
            ...
            if ((-f2 <= f1) && (f1 - mFretSlop <= f2) && (!gem.GetPlayed())
                && Playable(closestgem) && (closestgem != mLastNoStrumGemSwung)) {
                if (FretMatch(closestgem, true, false)) {
                    int flags = mBaseGuitarFlags | kGemHitFlagHopo;
                    if (b2)
                        flags = flags | kGemHitFlagSolo;
...
bool BaseGuitarTrackWatcherImpl::CanHopo(int i) const {
    ...
    if (gem.IsRealGuitar()) {
        return gem.RightHandTap()
            || gem.GetForceStrum() && mLastGemHit == (i - 1) && mSucceeding
            && (i == 0 || mGemList->GetGem(i - 1).GetPlayed());
    } else {
        return gem.GetForceStrum() && (i > 0) && mLastGemHit == (i - 1) && mSucceeding
            && mGemList->GetGem(i - 1).GetPlayed();
    }
}

void BaseGuitarTrackWatcherImpl::SetLastNoStrumGem(float f, int i) {
    if (mLastNoStrumGemHit != -1)
        SendHopo(f, mLastNoStrumGemHit);
