...
RealGuitarTrackWatcherImpl::RealGuitarTrackWatcherImpl(
    ...
    : BaseGuitarTrackWatcherImpl(track, u, slot, data, gemlist, parent, cfg), unk1f0(0),
      mMissWindowMs(0), mIs22Fret(0) {
    mSlop = 125.0f;
    mFretSlop = 100.0f;
    if (cfg)
        cfg->FindData("real_guitar_miss_window_ms", mMissWindowMs, false);
    mIs22Fret = data->TrackTypeAt(track) == kTrackRealGuitar22Fret
        || data->TrackTypeAt(track) == kTrackRealBass22Fret;
}
...
void RealGuitarTrackWatcherImpl::PollHook(float f) {
    ...
    if (!IsCheating()) {
        bool b2 = false;
        for (std::vector<GemInProgress>::iterator it = mGemsInProgress.begin();
             !b2 && it != mGemsInProgress.end();
             ++it) {
            if (it->mInUse) {
...
bool RealGuitarTrackWatcherImpl::Swing(int i1, bool b2, bool b3, GemHitFlags flags) {
    ...
    if (mParent->InCodaFreestyle(tick, true)) {
        ...
        if (!InSlopWindow(timeAt, now)) {
            ...
            if (now - unk1f0 < mMissWindowMs)
                u7 = (GemHitFlags)(u7 | kGemHitFlagNoPenalize);
            else
                unk1f0 = now;
            for (int i = 0; i < 6; i++) {
                if (i1 & (1 << i)) {
...
void RealGuitarTrackWatcherImpl::RGFretButtonDown(int i) { RecordFretButtonDown(i); }
...
bool RealGuitarTrackWatcherImpl::HandleHitsAndMisses(
    int i1, int i2, float f3, bool b4, bool b5, bool b6, GemHitFlags flags
) {
...
bool RealGuitarTrackWatcherImpl::StrumOK(int i) const {
    if (i == 0)
        ...
        if (prevAbs < mSlop || thisAbs < mSlop)
            return true;
        else
            return false;
    }
...
bool RealGuitarTrackWatcherImpl::GetNextRoll(int i1, unsigned int &uiRef, int &endTick)
    const {
