...
void PhraseAnalyzer::AddInfo(
    int track, TrackType ty, int start_tick, int end_tick, bool independent
) {
...
void PhraseAnalyzer::Analyze() {
    ...
    for (int i = 0; i < mRawPhrases.size(); i++) {
        if (mRawPhrases[i].independent) {
            SetPhraseIDs(
                i, i + 1, 1 << mRawPhrases[i].track, 1 << mRawPhrases[i].track_type
            );
        } else {
            if (abs(mRawPhrases[i].start_tick - rawPhrase.start_tick)
                ...
                if (abs(mRawPhrases[i].end_tick - rawPhrase.end_tick)
                    ...
                    const char *msg = MakeString(
                        "%s: Phrases don't quite coincide at %s, %s and %s",
                        mSongData->SongFullPath(),
                        TickFormat(rawPhrase.start_tick, *mSongData->GetMeasureMap()),
                        mSongData->TrackName(rawPhrase.track),
                        mSongData->TrackName(mRawPhrases[i].track)
                    );
                    if (mNotify)
                        MILO_WARN(msg);
                    else
