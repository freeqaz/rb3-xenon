...
int JoypadController::ButtonToSlot(JoypadButton btn) const {
    if (mSecondaryPedalFunction == kSecondKickPedal && btn == mSecondaryPedalButton)
        return 0;
    else {
...
int JoypadController::OnMsg(const ButtonDownMsg &msg) {
    ...
    if (btn == mForceMercuryBut) {
    ...
    } else if ((btn == mCymbalShiftButton || btn == mPadShiftButton)
               && NoSlotButtonsThisFrame()) {
        if (mLefty) {
        ...
        if (slot != -1) {
            ...
            if (mLefty && BeatMatchController::ButtonToSlot(btn) == 4
                ...
                && thePadData->IsButtonInMask(mPadShiftButton)
                && !thePadData->IsButtonInMask(0xC) && !thePadData->IsButtonInMask(0xE)
...
bool JoypadController::NoSlotButtonsThisFrame() const {
    for (int i = 1; i <= 4; i++) {
        if (GetJoypadData()->IsButtonNewlyPressed(BeatMatchController::SlotToButton(i)))
            return false;
    }
    return true;
}
...
void JoypadController::SetCymbalConfiguration(int i) { mCymbalConfiguration = i; }
