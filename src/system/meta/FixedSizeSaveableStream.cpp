...
bool FixedSizeSaveableStream::HasSymbol(Symbol s) const { return GetID(s) != -1; }
...
FixedSizeSaveableStream::FixedSizeSaveableStream(void *v, int i, bool b)
    : BufStream(v, i, b), m_iCurrentID(0), m_iTableOffset(0) {}

int FixedSizeSaveableStream::AddSymbol(Symbol i_symSymbol) {
    MILO_ASSERT(!HasSymbol( i_symSymbol ), 0x7E);
    m_iCurrentID++;
    MILO_ASSERT(!HasID( m_iCurrentID ), 0x81);
