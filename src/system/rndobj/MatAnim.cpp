...
// Retail (0x824619d8) has NO Hmx::Object::Replace fallback -- the body's only
...
// Retail's only guard is `to == 0` (cmplwi cr6,r3,0 at 0x82461a14) -- there is
// no `mKeysOwner == this` test and no null-check on the dynamic_cast result.
// Offsets cross-check exactly against /d1reportSingleClassLayout: retail's
// `addi r11,r3,28` is mKeysOwner at 0x1c, and `subi r4,r31,120` is the
// Hmx::Object vbase at 0x78.
void RndMatAnim::Replace(ObjRef *ref, Hmx::Object *obj) {
    if (RefIs(ref, mKeysOwner)) {
        if (!obj)
            mKeysOwner.SetOwnerObj(this);
        else
            mKeysOwner.SetOwnerObj(dynamic_cast<RndMatAnim *>(obj)->mKeysOwner.Ptr());
        return;
    }
#ifdef HX_NATIVE
