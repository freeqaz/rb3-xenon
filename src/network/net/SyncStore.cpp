...
void SyncStore::RemoveSyncObj(String &str) {
    for (std::vector<Synchronizable *>::iterator it = mSyncObjs.begin();
         it != mSyncObjs.end();
         ++it) {
...
SyncObjMsg::SyncObjMsg(String &tag, unsigned int mask, Synchronizable *sync)
    : mObjTag(tag), mDirtyMask(mask), mObjData(false) {
    sync->SyncSave(mObjData, mask);
    mObjData.Seek(0, BinStream::kSeekBegin);
}

void SyncObjMsg::GetObjData(BinStream &bs) const {
    bs.Write(mObjData.Buffer(), mObjData.Size());
}
...
SyncUserMsg::SyncUserMsg(const User *user) { mUserGuid = user->mUserGuid; }

void SyncUserMsg::Save(BinStream &bs) const { bs << mUserGuid; }

void SyncUserMsg::Load(BinStream &bs) { bs >> mUserGuid; }
...
void SyncAllMsg::Dispatch() { TheSyncStore->SyncAllUsers(); }
