...
void JoinRequestMsg::GetUserData(int index, BinStream &bs) const {
    ...
    bs.Write(mUserDatas[index].Buffer(), mUserDatas[index].Size());
}
...
void JoinRequestMsg::GetAuthenticationData(BinStream &bs) const {
    bs.Write(mAuthData.Buffer(), mAuthData.Size());
}
...
JoinResponseMsg::JoinResponseMsg(JoinResponseError err, int custom)
    : mError(err), mCustomError(custom) {}
...
void NewUserMsg::GetUserData(BinStream &bs) const {
    bs.Write(mUserData.Buffer(), mUserData.Size());
}
...
void UserLeftMsg::Save(BinStream &bs) const { bs << mUserGuid; }

void UserLeftMsg::Load(BinStream &bs) { bs >> mUserGuid; }
...
void FinishedArbitrationMsg::Save(BinStream &bs) const { bs << mMachineID; }

void FinishedArbitrationMsg::Load(BinStream &bs) { bs >> mMachineID; }

StartGameOnTimeMsg::StartGameOnTimeMsg(unsigned long long ul) : mStartTime(ul) {}

void StartGameOnTimeMsg::Save(BinStream &bs) const { bs << mStartTime; }

void StartGameOnTimeMsg::Load(BinStream &bs) { bs >> mStartTime; }

EndGameMsg::EndGameMsg(int i, bool b, float f)
    : mResultCode(i), mReportStats(b), unkc(f) {}
...
void DataArrayMsg::Save(BinStream &bs) const {
    bs << mBuffer.Size();
    bs.Write(mBuffer.Buffer(), mBuffer.Size());
}
...
void DataArrayMsg::Print(TextStream &ts) const {
    MemStream stream(false);
    stream.Write(mBuffer.Buffer(), mBuffer.Size());
