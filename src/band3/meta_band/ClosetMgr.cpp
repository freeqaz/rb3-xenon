...
DataNode ClosetMgr::OnMsg(const ProfileSwappedMsg &msg) {
    ...
    if (mUser == pLocalUser1)
        mUser = pLocalUser2;
    else if (mUser == pLocalUser2)
        mUser = pLocalUser1;
    return 1;
}
...
void ClosetMgr::ResetPatches() { unk3c->mPatches = mBandCharDesc->mPatches; }
...
void ClosetMgr::UpdateCurrentOutfitConfig() {
    ...
    if (!pAssetMgr->HasAsset(name))
        mCurrentOutfitConfig = 0;
    else {
        const char *cfgname =
            GetConfigNameFromAssetType(pAssetMgr->GetTypeFromName(name));
        mCurrentOutfitConfig = mBandCharacter->GetOutfitConfig(cfgname);
        if (!mCurrentOutfitConfig)
            MILO_WARN("Could not find (%s) for this asset.", cfgname);
    }
}
...
void ClosetMgr::CycleCamera() { mCurrentClosetPanel->CycleCamera(); }

void ClosetMgr::GotoArtMakerShot() { mCurrentClosetPanel->GotoArtMakerShot(); }
...
void ClosetMgr::TakePortrait() { mCurrentClosetPanel->TakePortrait(); }
...
BEGIN_HANDLERS(ClosetMgr)
    ...
    HANDLE_ACTION_STATIC(set_current_outfit_piece, SetCurrentOutfitPiece(_msg->Sym(2)))
    HANDLE_ACTION_STATIC(reset_character_preview, ResetCharacterPreview())
    HANDLE_ACTION_STATIC(reset_new_character_preview, ResetNewCharacterPreview(_msg->Sym(2)))
    HANDLE_ACTION_STATIC(finalize_body_changes, FinalizeBodyChanges(_msg->Sym(2)))
    HANDLE_ACTION_STATIC(finalize_char_creator_changes, FinalizeCharCreatorChanges())
    HANDLE_ACTION_STATIC(finalize_changes, FinalizeChanges(_msg->Int(2), _msg->Int(3)))
    HANDLE_ACTION_STATIC(finalized_colors, FinalizedColors())
    HANDLE_ACTION_STATIC(set_patches, SetPatches())
    HANDLE_ACTION_STATIC(hide_clothes, HideClothes())
    HANDLE_ACTION_STATIC(show_clothes, ShowClothes())
    HANDLE_ACTION_STATIC(
        set_current_closet_panel, SetCurrentClosetPanel(_msg->Obj<ClosetPanel>(2))
    )
    HANDLE_ACTION_STATIC(clear_current_closet_panel, ClearCurrentClosetPanel())
    HANDLE_ACTION_STATIC(cycle_camera, CycleCamera())
    HANDLE_ACTION_STATIC(goto_art_maker_shot, GotoArtMakerShot())
    HANDLE_ACTION_STATIC(leave_art_maker_shot, LeaveArtMakerShot())
    HANDLE_ACTION_STATIC(clear_instrument, ClearInstrument())
    HANDLE_ACTION_STATIC(set_return_screen, SetReturnScreen(_msg->Sym(2)))
    HANDLE_EXPR_STATIC(get_return_screen, GetReturnScreen())
    HANDLE_EXPR_STATIC(has_asset_offer, HasAssetOffer(_msg->Sym(2)))
    HANDLE_EXPR_STATIC(has_any_asset_offers, mAssetStore.HasAnyAssetOffers())
    HANDLE_ACTION_STATIC(show_purchase_ui, ShowPurchaseUI(_msg->Sym(2)))
    HANDLE_EXPR_STATIC(is_downloading, mAssetStore.IsDownloading())
