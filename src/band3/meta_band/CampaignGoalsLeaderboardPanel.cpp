...
void CampaignGoalsLeaderboardPanel::Load() {
    ...
    mCampaignGoalsLeaderboardProvider =
        new PlayerCampaignGoalLeaderboard(pProfile, this, mGoal);
}
...
BEGIN_HANDLERS(CampaignGoalsLeaderboardPanel)
    ...
    HANDLE_ACTION_IF(
        ...
        (MILO_ASSERT(mCampaignGoalsLeaderboardProvider, 0xD9), true),
        mCampaignGoalsLeaderboardProvider->SetMode((Leaderboard::Mode)_msg->Int(2), true)
    )
    HANDLE_ACTION_IF(
        select_lb_row,
        mCampaignGoalsLeaderboardProvider,
        mCampaignGoalsLeaderboardProvider->OnSelectRow(
            _msg->Int(2), _msg->Obj<BandUser>(3)
        )
    )
    HANDLE_ACTION_IF(
        set_to_starting_lb_ix,
        mCampaignGoalsLeaderboardProvider,
        _msg->Obj<UIList>(2)->SetSelected(
            mCampaignGoalsLeaderboardProvider->GetStartingRow(), -1
        )
    )
