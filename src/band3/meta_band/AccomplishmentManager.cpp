...
bool GoalAlpaCmp::operator()(Symbol s1, Symbol s2) const {
...
void AccomplishmentManager::InitializeTourSafeDiscSongs() {
    ...
    std::stable_sort(
        mTourSafeDiscSongs.begin(), mTourSafeDiscSongs.end(), SongDifficultyCmp(gNullStr)
    );
}
...
void AccomplishmentManager::ConfigureAccomplishmentCategoryData(DataArray *arr) {
    for (int i = 1; i < arr->Size(); i++) {
        AccomplishmentCategory *pAccomplishmentCategory =
            new AccomplishmentCategory(arr->Array(i), i);
...
void AccomplishmentManager::ConfigureAccomplishmentGroupData(DataArray *arr) {
    for (int i = 1; i < arr->Size(); i++) {
        AccomplishmentGroup *pAccomplishmentGroup =
            new AccomplishmentGroup(arr->Array(i), i);
...
DECOMP_FORCEACTIVE(
    AccomplishmentManager, "pDataArray", "%s precached filter already exists, skipping"
)
...
void AccomplishmentManager::ConfigureAccomplishmentRewardData(DataArray *arr) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        mAccomplishmentRewardLeaderboardThresholds[i] =
            pLeaderboardThresholds->Array(i + 1)->Int(1);
    }
...
int AccomplishmentManager::GetNumAccomplishmentsInCategory(Symbol s) const {
    ...
    if (symset) {
        for (std::set<Symbol>::const_iterator it = symset->begin(); it != symset->end();
             ++it) {
            if (IsAvailableToView(*it))
                num++;
        }
    }
    return num;
...
int AccomplishmentManager::GetNumAccomplishmentsInGroup(Symbol i_symGroup) const {
    ...
    int num = 0;
    for (std::list<Symbol>::const_iterator it = pCategoryList->begin();
...
DECOMP_FORCEACTIVE(
    ...
    "iNextPointValue > iLastPointValue",
    "i_iPointValue >= iLastPointValue",
    "iNextFanValue > iLastFanValue"
)
...
bool AccomplishmentManager::InqAssetSourceList(
    ...
    if (award == gNullStr)
        ...
        } else if (src != gNullStr)
            o_rSourceList.push_back(src);
        else
            o_rSourceList.push_back(award);
...
void AccomplishmentManager::AddAssetAward(Symbol s1, Symbol s2) {
    if (GetAssetAward(s1) != gNullStr)
        MILO_WARN("Asset:%s is earned by multiple sources!", s1.Str());
    else
        mAssetToAward[s1] = s2;
}
...
String AccomplishmentManager::GetHintStringForSource(Symbol s) const {
    ...
    if (HasAccomplishment(s)) {
        ...
        ret = MakeString(
            Localize(asset_hint_award, 0), Localize(pAward->GetDisplayName(), 0)
        );
    } else
        MILO_ASSERT(false, 0x479);
    return ret;
}
...
void AccomplishmentManager::EarnAccomplishment(BandProfile *p, Symbol s) {
    if (!HasAccomplishment(s)) {
        ...
        if (!IsAvailableToEarn(s))
            return;
        if (pPerformer->IsNoFailActive() && !pAccomplishment->CanBeEarnedWithNoFail())
            return;
    ...
    for (std::map<Symbol, Accomplishment *>::iterator it = mAccomplishments.begin();
        ...
        if (!prog.IsAccomplished(key)) {
            ...
            if (pAccomplishment->GetType() != kAccomplishmentTypeTrainerListConditional
                && pAccomplishment->GetType()
                    != kAccomplishmentTypeTrainerCategoryConditional
                && pAccomplishment->GetType()
                    != kAccomplishmentTypeLessonDiscSongConditional
                && pAccomplishment->GetType()
                    != kAccomplishmentTypeLessonSongListConditional)
                continue;
            if (pAccomplishment->IsFulfilled(pProfile))
                EarnAccomplishment(u, key);
        }
    }
}

void AccomplishmentManager::CheckForIncrementalProgressForUserGoal(
    Symbol s1, Symbol s2, LocalBandUser *i_pUser
) {
...
void AccomplishmentManager::CheckForFinishedTourAccomplishments() {
    if (TheBandUserMgr) {
        ...
        for (std::vector<LocalBandUser *>::iterator it = users.begin(); it != users.end();
            ...
            BandProfile *p = TheProfileMgr.GetProfileForUser(pUser);
            if (p)
                CheckForFinishedTourAccomplishmentsForProfile(p);
        }
        BandProfile *p = TheTour->GetProfile();
        if (p)
            CheckForFinishedTourAccomplishmentsForProfile(p);
...
void AccomplishmentManager::CheckForFinishedTourAccomplishmentsForProfile(
    ...
    for (std::map<Symbol, Accomplishment *>::iterator it = mAccomplishments.begin();
        ...
        if (!prog.IsAccomplished(key)) {
            ...
            if (!IsAvailableToEarn(key))
                continue;
            if (pAccomplishment->GetType() != kAccomplishmentTypeTourConditional)
                continue;
            if (pAccomplishment->IsFulfilled(i_pProfile))
                EarnAccomplishment(i_pProfile, key);
        }
    }
}

void AccomplishmentManager::CheckForFinishedTourAccomplishmentsForUser(
    LocalBandUser *i_pUser
) {
...
void AccomplishmentManager::CheckForFinishedAccomplishmentsForUser(
    Symbol s, LocalBandUser *u
) {
    BandProfile *pProfile = TheProfileMgr.GetProfileForUser(u);
    ...
    for (std::map<Symbol, Accomplishment *>::iterator it = mAccomplishments.begin();
        ...
        if (!pProgress.IsAccomplished(key)) {
            ...
            if (pAccomplishment->GetType() == kAccomplishmentTypeTourConditional)
                continue;
            if (pAccomplishment->GetType() == kAccomplishmentTypeUnique)
                continue;
            if (IsAvailableToEarn(key)) {
                if (!pAccomplishment->IsRelevantForSong(s))
                    continue;
                if (!pAccomplishment->IsUserOnValidScoreType(u))
                    continue;
                if (!pAccomplishment->IsFulfilled(pProfile))
                    continue;
                EarnAccomplishment(u, key);
...
int AccomplishmentManager::GetLeaderboardHardcoreStatus(int n) const {
    int ret = 0;
    for (int i = 0; i < 4; i++) {
        if (n < mAccomplishmentRewardLeaderboardThresholds[i])
            break;
        else
            ret = i;
    }
...
int AccomplishmentManager::GetIconHardCoreStatus(int n) const {
    int ret = 0;
    for (int i = 0; i < 4; i++) {
        if (n < mAccomplishmentRewardIconThresholds[i])
            break;
        else
            ret = i;
    }
...
void AccomplishmentManager::HandleSetlistCompleted(
    Symbol s, bool b, Difficulty diff, int i
) {
    if (TheBandUserMgr) {
        std::vector<LocalBandUser *> users;
        TheBandUserMgr->GetLocalParticipants(users);
...
void AccomplishmentManager::HandleSongCompletedForUser(
...
DECOMP_FORCEACTIVE(
    AccomplishmentManager,
    "pSetlistAccomplishment",
    "No player found while trying to update song status flags.",
    "pBand",
    "pBandPerformer",
    "pOneShotAccomplishment",
    "pSongStatusMgr"
)

bool AccomplishmentManager::CanEquipAward(LocalBandUser *i_pUser, Symbol i_symAward)
    const {
...
Symbol AccomplishmentManager::GetFirstUnfinishedAccomplishmentEntry(
    ...
    if (prog.IsAccomplished(s)) {
        ...
        for (it = vAccomplishmentEntries.begin(); it != vAccomplishmentEntries.end();
             ++it) {
            Symbol sym = *it;
            if (!pAccomplishment->IsSymbolEntryFulfilled(i_pProfile, sym)) {
                symFirst = sym;
                break;
            }
        }
...
bool AccomplishmentManager::IsAvailable(Symbol s, bool b) const {
    ...
    if (!pAccomplishment->IsDynamic())
        return true;
    else if (b && pAccomplishment->GetDynamicAlwaysVisible())
        return true;
    else {
        ...
        if (filt == gNullStr) {
            ...
            if (filtcount >= pAccomplishment->GetDynamicPrereqsNumSongs())
                return true;
        }
        return false;
    }
...
void AccomplishmentManager::HandleRemoteAccomplishmentEarned(
    Symbol s1, const char *cc, Symbol s2
) {
...
void AccomplishmentManager::AddGoalProgressionInfo(
    Symbol s1, const char *cc, Symbol s2, int iii
) {
...
int AccomplishmentManager::GetNumOtherGoalsAcquired(const char *cc, Symbol s) {
    int num = 0;
    for (std::vector<GoalAcquisitionInfo>::iterator it = mGoalAcquisitionInfos.begin();
         it != mGoalAcquisitionInfos.end();
         ++it) {
        if (strcmp(it->unk4.c_str(), cc) == 0 && it->unk0 != s)
            num++;
    }
    return num;
}

bool AccomplishmentManager::InqGoalsAcquiredForSong(
    BandUser *i_pUser, Symbol i_symSong, std::vector<Symbol> &o_rAcquiredGoals
) {
...
bool AccomplishmentManager::DidUserMakeProgressOnGoal(LocalBandUser *i_pUser, Symbol s) {
    ...
    for (std::vector<GoalProgressionInfo>::iterator it = mGoalProgressionInfos.begin();
         it != mGoalProgressionInfos.end();
         ++it) {
        if (it->unk0 == s) {
            if (strcmp(it->unk4.c_str(), i_pUser->ProfileName()) == 0)
                return true;
        }
    }
    return false;
...
Symbol AccomplishmentManager::GetTourSafeDiscSongAtDifficultyIndex(int idx) {
    if (idx >= mTourSafeDiscSongs.size()) {
        MILO_WARN(
            "Trying to access tour disc song %i which is beyond number of songs avaiable!",
            idx
        );
...
BEGIN_HANDLERS(AccomplishmentManager)
    ...
    HANDLE_ACTION(
        earn_accomplishment_for_all, EarnAccomplishmentForAllParticipants(_msg->Sym(2))
    )
    HANDLE_EXPR(get_num_goals, GetNumAccomplishments())
    HANDLE_EXPR(
        get_num_completed_goals,
        GetNumCompletedAccomplishments(_msg->Obj<LocalBandUser>(2))
    )
    HANDLE_EXPR(
        has_completed_goal,
        HasCompletedAccomplishment(_msg->Obj<LocalBandUser>(2), _msg->Sym(3))
    )
    ...
    HANDLE_ACTION(
        update_reason_for_award,
        UpdateReasonLabelForAward(_msg->Sym(2), _msg->Obj<UILabel>(3))
    )
