...
BEGIN_HANDLERS(VignetteViewerProvider)
    HANDLE_EXPR_STATIC(is_locked, IsLocked(_msg->Int(2)))
    HANDLE_EXPR_STATIC(get_screen, GetScreen(_msg->Int(2)))
...
void ManageBandPanel::ShowCharacter() {
    static Message show_character_msg("show_character");
    Handle(show_character_msg, true);
}

void ManageBandPanel::HideCharacter() {
    static Message hide_character_msg("hide_character");
...
BEGIN_HANDLERS(ManageBandPanel)
    ...
    HANDLE_ACTION_STATIC(refresh_to_standins_state, RefreshToStandinsState())
    HANDLE_EXPR_STATIC(get_standin_provider, GetStandInProvider())
    HANDLE_EXPR_STATIC(get_char_provider, GetCharProvider())
    HANDLE_EXPR_STATIC(get_history_provider, GetHistoryProvider())
    HANDLE_ACTION_STATIC(queue_reward_vignette, QueueRewardVignette(_msg->Sym(2)))
    ...
    HANDLE_ACTION_STATIC(refresh_standin_list, RefreshStandinList())
    HANDLE_ACTION_STATIC(set_selected_standin, SetSelectedStandIn(_msg->Int(2)))
    HANDLE_EXPR_STATIC(get_selected_standin, GetSelectedStandIn())
    HANDLE_ACTION_STATIC(set_standin, SetStandIn(_msg->Int(2)))
    HANDLE_EXPR_STATIC(get_state, GetManageBandState())
    HANDLE_ACTION_STATIC(set_state, SetManageBandState((ManageBandState)_msg->Int(2)))
    HANDLE_ACTION_STATIC(check_for_kickout_condition, CheckForKickoutCondition())
