        ...
        for (int j = 1; j < pTrainer->Size(); j++) {
            ...
            for (int k = 1; k < pCategory->Size(); k++) {
                ...
                if (!HasLesson(sym0)) {
                    ...
                    MILO_WARN("Lesson: %s already exists", sym0.Str());
            }
...
int LessonMgr::GetTotalCountFromCategory(Symbol s) {
    return GetLessonsFromCategory(s)->size();
}
...
int LessonMgr::GetCompletedCountFromCategory(BandProfile *pProfile, Symbol s) {
    ...
    for (int i = 0; i < lessons->size(); i++) {
        ...
        if (pProfile->IsLessonComplete(lesson, 1.0f))
            count++;
    }
    return count;
}
...
Lesson::Lesson(Symbol s1, Symbol s2, Symbol s3, Symbol s4, TrackType ty)
    : mTrainer(s1), mCategory(s2), mName(s3), mSong(s4), mTrackType(ty) {}
...
Difficulty Lesson::GetDifficulty() const {
    ...
    if (diff == easy)
        ...
        ret = kDifficultyMedium;
    else if (diff == hard)
        ret = kDifficultyHard;
    else if (diff == expert)
        ret = kDifficultyExpert;
    else {
        ret = kDifficultyEasy;
    }
    return ret;
