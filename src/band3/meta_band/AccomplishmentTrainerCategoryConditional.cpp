...
AccomplishmentTrainerCategoryConditional::AccomplishmentTrainerCategoryConditional(
    ...
    : AccomplishmentTrainerConditional(i_pConfig, i), mCategory(""), mNumLessons(0) {
    Configure(i_pConfig);
}
...
int AccomplishmentTrainerCategoryConditional::GetNumCompletedLessons(BandProfile *profile
) const {
...
bool AccomplishmentTrainerCategoryConditional::InqIncrementalSymbols(
    ...
    if (!lessons) {
        ...
        for (std::vector<Symbol>::iterator it = lessons->begin(); it != lessons->end();
             ++it) {
            Symbol key = *it;
...
Symbol AccomplishmentTrainerCategoryConditional::GetFirstUnfinishedAccomplishmentEntry(
...
bool AccomplishmentTrainerCategoryConditional::InqCompletedLessons(
    BandProfile *i_pProfile, std::vector<Symbol> &o_rLessons
) const {
