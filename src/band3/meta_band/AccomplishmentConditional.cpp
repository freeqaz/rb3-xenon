...
AccomplishmentConditional::AccomplishmentConditional(DataArray *i_pConfig, int i)
...
bool AccomplishmentConditional::CanBeLaunched() const { return true; }

bool AccomplishmentConditional::InqRequiredScoreTypes(std::set<ScoreType> &o_rScoreTypes
) const {
    MILO_ASSERT(!m_lConditions.empty(), 0x70);
    MILO_ASSERT(o_rScoreTypes.empty(), 0x71);

    for (std::vector<AccomplishmentCondition>::const_iterator i = m_lConditions.begin();
         i != m_lConditions.end();
         i++) {
