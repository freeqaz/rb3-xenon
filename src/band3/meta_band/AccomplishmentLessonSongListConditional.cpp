...
AccomplishmentLessonSongListConditional::AccomplishmentLessonSongListConditional(
    ...
    : AccomplishmentSongListConditional(arr, i) {
    Configure(arr);
}
...
AccomplishmentType AccomplishmentLessonSongListConditional::GetType() const {
    return kAccomplishmentTypeLessonSongListConditional;
}

bool AccomplishmentLessonSongListConditional::CheckLessonCompleteCondition(
    ...
    if (!TheSongMgr.HasSong(s, false)) {
        ...
        default:
            MILO_WARN(
                "CheckLessonCompleteCondition is not currently supported with score type: %i \n",
                ty
            );
            return false;
        }
...
bool AccomplishmentLessonSongListConditional::CheckConditionsForSong(
    ...
    for (std::list<AccomplishmentCondition>::const_iterator it = m_lConditions.begin();
        ...
        if (condsym == lesson_complete) {
            if (CheckLessonCompleteCondition(mgr, s, cond))
                return true;
        } else {
            MILO_WARN(
