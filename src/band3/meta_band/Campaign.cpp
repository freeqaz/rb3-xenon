...
Symbol Campaign::GetCampaignLevelForMetaScore(int val) const {
    Symbol ret("");
    for (std::vector<Symbol>::const_reverse_iterator it = m_vCampaignLevels.rbegin();
         it != m_vCampaignLevels.rend();
         ++it) {
        Symbol cur = *it;
...
int Campaign::GetTotalPointsForNextMajorCampaignLevelForPrimary() {
    if (IsPrimaryUserOnLastCampaignLevel())
        return 0;
    else {
        BandMachineMgr *pMachineMgr = TheSessionMgr->mMachineMgr;
        MILO_ASSERT(pMachineMgr, 0x2E8);
        return GetTotalPointsForNextMajorCampaignLevelForMetaScore(
            pMachineMgr->GetLeaderPrimaryMetaScore()
        );
    }
}

int Campaign::GetTotalPointsForNextMajorCampaignLevelForUser(LocalBandUser *user) {
    if (IsUserOnLastCampaignLevel(user))
        return 0;
    else {
        return GetTotalPointsForNextMajorCampaignLevelForMetaScore(
            GetCampaignMetaScoreForUser(user)
        );
    }
}
...
int Campaign::GetCurrentPointsForNextMajorCampaignLevelForPrimary() {
    if (IsPrimaryUserOnLastCampaignLevel())
        ...
        return GetCurrentPointsForNextMajorCampaignLevelForMetaScore(
            pMachineMgr->GetLeaderPrimaryMetaScore()
        );
    }
}
...
bool Campaign::HasCurrentGoal() const { return m_symCurrentAccomplishment != ""; }
...
Symbol Campaign::GetCurrentGoalDescription() const {
    ...
    if (m_symCurrentAccomplishment != gNullStr) {
        Accomplishment *pAccomplishment =
            TheAccomplishmentMgr->GetAccomplishment(m_symCurrentAccomplishment);
...
void Campaign::SetWasLaunchedIntoMusicLibrary(bool b) {
    m_bWasLaunchedIntoMusicLibrary = b;
}

Difficulty Campaign::GetMinimumDifficultyForCurrentAccomplishment() const {
    if (m_symCurrentAccomplishment == "")
...
ScoreType Campaign::GetRequiredScoreTypeForCurrentAccomplishment() const {
    if (m_symCurrentAccomplishment == "")
        ...
        return GetRequiredScoreTypeForGoal(m_symCurrentAccomplishment);
}
...
TrackType Campaign::GetRequiredTrackTypeForCurrentAccomplishment() const {
    if (m_symCurrentAccomplishment == "")
        ...
        return GetRequiredTrackTypeForGoal(m_symCurrentAccomplishment);
}
...
bool Campaign::GetWasLaunchedIntoMusicLibrary() const {
...
void Campaign::UpdateEndGameInfo(
    UILabel *i_pUserLabel, UILabel *i_pStatusLabel, UIPicture *i_pPicture
) {
...
bool Campaign::HasDisplayGoal() { return unk28 != gNullStr; }
...
const char *Campaign::GetCheatMetaLevel() {
    if (unk88 == -1)
        return "Campaign Level Cheat Disabled";
    else
...
BEGIN_HANDLERS(Campaign)
    ...
    HANDLE_EXPR(
        get_campaign_level_icon, GetCampaignLevelIconForUser(_msg->Obj<LocalBandUser>(2))
    )
    HANDLE_EXPR(
        get_total_points_for_next_campaign_level,
        GetTotalPointsForNextCampaignLevelForUser(_msg->Obj<LocalBandUser>(2))
    )
    HANDLE_EXPR(
        get_current_points_for_next_campaign_level,
        GetCurrentPointsForNextCampaignLevelForUser(_msg->Obj<LocalBandUser>(2))
    )
    HANDLE_EXPR(
        get_campaign_level_advertisement, GetCampaignLevelAdvertisement(_msg->Sym(2))
    )
    HANDLE_EXPR(
    ...
    HANDLE_EXPR(
        ...
        HasScoreReachedCampaignLevel(_msg->Int(2), _msg->Sym(3))
    )
    HANDLE_ACTION(
        update_current_major_level_icon,
        UpdateCurrentMajorLevelIcon(_msg->Obj<LocalBandUser>(2), _msg->Obj<UIPicture>(3))
    )
    HANDLE_ACTION(
        update_next_major_level_icon,
        UpdateNextMajorLevelIcon(_msg->Obj<LocalBandUser>(2), _msg->Obj<UIPicture>(3))
    )
    HANDLE_ACTION(
        update_primary_current_major_level_icon,
        UpdatePrimaryCurrentMajorLevelIcon(_msg->Obj<UIPicture>(2))
    )
    HANDLE_ACTION(
        update_primary_next_major_level_icon,
        UpdatePrimaryNextMajorLevelIcon(_msg->Obj<UIPicture>(2))
    )
    ...
    HANDLE_ACTION(
        ...
        UpdateProgressMeter(_msg->Obj<MeterDisplay>(2), _msg->Obj<LocalBandUser>(3))
    )
    HANDLE_ACTION(
        update_primary_progress_meter,
        UpdatePrimaryProgressMeter(_msg->Obj<MeterDisplay>(2))
    )
    ...
    HANDLE_ACTION(
        update_endgame_info,
        UpdateEndGameInfo(
            _msg->Obj<UILabel>(2), _msg->Obj<UILabel>(3), _msg->Obj<UIPicture>(4)
        )
    )
