...
BEGIN_HANDLERS(SongRecord)
    ...
    HANDLE_EXPR_STATIC(short_difficulty_sym, GetShortDifficultySym())
    HANDLE_EXPR_STATIC(get_stars, GetStars())
    HANDLE_EXPR_STATIC(get_notes_pct, GetNotesPct())
    HANDLE_EXPR_STATIC(get_instrument_mask, GetInstrumentMask())
    HANDLE_EXPR_STATIC(get_review, GetReview())
    HANDLE_EXPR_STATIC(update_review, UpdateReview())
...
BEGIN_HANDLERS(SetlistRecord)
    HANDLE_EXPR_STATIC(id, mID)
    HANDLE_EXPR_STATIC(get_owner, GetOwner())
    HANDLE_EXPR_STATIC(get_art_tex, mSetlist->GetArtTex())
    HANDLE_EXPR_STATIC(get_setlist_type_sym, GetSetlistTypeSym())
    HANDLE_EXPR_STATIC(is_local_setlist, mSetlist->GetType() == SavedSetlist::kSetlistLocal)
    HANDLE_EXPR_STATIC(is_net_setlist, IsNetSetlist())
    HANDLE_EXPR_STATIC(is_battle, mSetlist->IsBattle())
    HANDLE_EXPR_STATIC(is_profile_owner, IsProfileOwner(_msg->Obj<BandProfile>(2)))
    HANDLE_EXPR_STATIC(get_setlist, dynamic_cast<LocalSavedSetlist *>(mSetlist))
    HANDLE_EXPR_STATIC(get_song_count, (int)mSetlist->mSongs.size())
    HANDLE_EXPR_STATIC(get_length_ms, mSetlist->GetLengthMs())
    HANDLE_EXPR_STATIC(get_title, mSetlist->GetTitle())
    HANDLE_EXPR_STATIC(get_description, mSetlist->GetDescription())
    HANDLE_EXPR_STATIC(get_battle_time_left, mBattleTimeLeft)
    HANDLE_EXPR_STATIC(has_viewable_gamercard, HasViewableGamercard())
    HANDLE_ACTION_STATIC(view_gamercard, ViewGamercard(_msg->Obj<LocalBandUser>(2)))
    HANDLE_SUPERCLASS(Hmx::Object)
    HANDLE_CHECK(0x17E)
END_HANDLERS