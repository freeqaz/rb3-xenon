...
OvershellPartSelectProvider::OvershellPartSelectProvider(OvershellPanel *panel)
    : mControllerType(kControllerNone), mUser(0), mOvershell(panel) {}
...
bool OvershellPartSelectProvider::IsActive(int data) const {
    ...
    if (TheGameMode->InMode("campaign")
        || TheGameMode->InMode("pro_song_lessons_keyboard")
        || TheGameMode->InMode("pro_song_lessons_real_guitar")
        || TheGameMode->InMode("pro_song_lessons_real_bass")) {
        TrackType reqTrackType =
            TheCampaign->GetRequiredTrackTypeForCurrentAccomplishment();
        ScoreType reqScoreType =
            TheCampaign->GetRequiredScoreTypeForCurrentAccomplishment();
        if (reqTrackType == kTrackNone)
            return true;
        BandUser *user = TheCampaign->GetLaunchUser();
        if (!user)
            return true;
        if (TheCampaign->GetLaunchUser() == mUser) {
            if (entry.unk4 != reqTrackType)
                return false;
            if (reqScoreType == 6 && entry.unk0 != "overshell_drums_pro")
                return false;
            if (reqScoreType == 0 && entry.unk0 != "overshell_drums")
                return false;
            if (reqScoreType == 3 && entry.unk0 != "overshell_vocal_solo")
                return false;
            if (reqScoreType == 4 && entry.unk0 != "overshell_vocal_harmony")
                return false;
        } else if (RepresentSamePart(entry.unk4, reqTrackType))
            return false;
    }
    return true;
}

void OvershellPartSelectProvider::Text(int, int data, UIListLabel *slot, UILabel *label)
    const {
