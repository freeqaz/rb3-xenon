...
DataNode OvershellSlot::OnMsg(const UIComponentScrollMsg &msg) {
    ...
    if (pUser && pUser->IsLocal()) {
        ...
        if (list) {
            pUser->GetLocalBandUser()->SetOvershellFocus(list->SelectedSym(false).Str());
        }
    }
...
BEGIN_HANDLERS(OvershellSlot)
    ...
    HANDLE_EXPR_STATIC(in_game, InGame())
    HANDLE_EXPR_STATIC(in_song, mOvershell->InSong())
    HANDLE_MESSAGE(AddLocalUserResultMsg)
    HANDLE_MESSAGE(LocalUserLeftMsg)
    HANDLE_EXPR_STATIC(get_current_view, GetCurrentView())
    HANDLE_EXPR_STATIC(get_slot_num, GetSlotNum())
    HANDLE_EXPR_STATIC(get_state, mState)
    HANDLE_EXPR_STATIC(get_session_mgr, mSessionMgr)
    HANDLE_ACTION_STATIC(update, Update())
    HANDLE_ACTION_STATIC(leave_options, LeaveOptions())
    HANDLE_ACTION_STATIC(show_state, ShowState((OvershellSlotStateID)_msg->Int(2)))
    HANDLE_ACTION_STATIC(show_chars, ShowState(kState_ChooseChar))
    HANDLE_ACTION_STATIC(show_options, ShowState(kState_Options))
    HANDLE_ACTION_STATIC(show_online_options, ShowOnlineOptions())
    HANDLE_ACTION_STATIC(attempt_register_online, AttemptRegisterOnline())
    HANDLE_ACTION_STATIC(show_game_options, ShowState(kState_GameOptions))
    HANDLE_ACTION_STATIC(show_options_av_settings, ShowState(kState_OptionsAVSettings))
    HANDLE_ACTION_STATIC(show_options_audio, ShowState(kState_OptionsAudio))
    HANDLE_ACTION_STATIC(show_options_vocal, ShowState(kState_OptionsVocal))
    HANDLE_ACTION_STATIC(show_options_extras, ShowState(kState_OptionsExtras))
    HANDLE_ACTION_STATIC(show_options_drum, ShowOptionsDrum())
    HANDLE_ACTION_STATIC(show_saveloadmgr_not_idle, ShowState(kState_SaveloadManagerNotIdle))
    HANDLE_ACTION_STATIC(show_saveloadmgr_not_meta, ShowState(kState_SaveloadManagerNotMeta))
    HANDLE_ACTION_STATIC(
        show_saveloadmgr_autosave_confirm,
        ShowState(kState_SaveloadManagerAutosaveConfirm)
    )
    HANDLE_ACTION_STATIC(show_options_end_game, ShowState(kState_OptionsEndGame))
    HANDLE_ACTION_STATIC(show_linking_code, ShowState(kState_LinkingCode))
    HANDLE_ACTION_STATIC(show_linking_code_error, ShowState(kState_LinkingCodeError))
    HANDLE_ACTION_STATIC(fetch_linking_code, FetchLinkingCode())
    HANDLE_ACTION_STATIC(cancel_linking_code, CancelLinkingCode())
    HANDLE_ACTION_STATIC(show_char_edit, ShowCharEdit(_msg->Int(2)))
    HANDLE_ACTION_STATIC(attempt_show_char_delete, AttemptShowCharDelete())
    HANDLE_ACTION_STATIC(remove_user, RemoveUser())
    HANDLE_ACTION_STATIC(show_song_options, ShowSongOptions())
    HANDLE_ACTION_STATIC(select_part, SelectPart((TrackType)_msg->Int(2)))
    HANDLE_ACTION_STATIC(select_vocal_part, SelectVocalPart(_msg->Int(2)))
    HANDLE_ACTION_STATIC(select_drum_part, SelectDrumPart(_msg->Int(2)))
    HANDLE_ACTION_STATIC(show_choose_part_wait, ShowChoosePartWait())
    HANDLE_ACTION_STATIC(toggle_hihat_pedal, ToggleHiHatPedal())
    HANDLE_ACTION_STATIC(toggle_cymbal, ToggleCymbal(_msg->Sym(2)))
    HANDLE_ACTION_STATIC(finish_cymbal_selection, FinishCymbalSelect(_msg->Int(2)))
    HANDLE_ACTION_STATIC(dismiss_cymbal_message, DismissCymbalMessage())
    HANDLE_ACTION_STATIC(select_difficulty, SelectDifficulty((Difficulty)_msg->Int(2)))
    HANDLE_ACTION_STATIC(leave_choose_part, LeaveChoosePart())
    HANDLE_ACTION_STATIC(leave_choose_part_wait, LeaveChoosePartWait())
    HANDLE_ACTION_STATIC(leave_choose_difficulty, LeaveChooseDifficulty())
    HANDLE_ACTION_STATIC(leave_ready_to_play, LeaveReadyToPlay())
    HANDLE_ACTION_STATIC(leave_char_delete, ShowState(kState_ChooseCharEdit))
    HANDLE_ACTION_STATIC(show_sign_in_wait, ShowState(kState_SignInWait))
    HANDLE_ACTION_STATIC(leave_sign_in_wait, LeaveSignInWait())
    HANDLE_ACTION_STATIC(refresh_highlighted_char, RefreshHighlightedChar(_msg->Int(2)))
    HANDLE_ACTION_STATIC(update_character_list, UpdateCharacterList())
    HANDLE_EXPR_STATIC(get_default_char_index, GetDefaultCharIndex())
    HANDLE_ACTION_STATIC(update_friends_list, UpdateFriendsList())
    HANDLE_ACTION_STATIC(select_char, SelectChar(_msg->Int(2)))
    HANDLE_ACTION_STATIC(
    ...
    HANDLE_ACTION_STATIC(show_enter_calibration, ShowEnterFlowPrompt(kState_EnterCalibration))
    HANDLE_ACTION_STATIC(show_enter_credits, ShowEnterFlowPrompt(kState_EnterCredits))
    HANDLE_ACTION_STATIC(attempt_disconnect, AttemptDisconnect())
    HANDLE_ACTION_STATIC(attempt_remove_user, AttemptRemoveUser())
    HANDLE_ACTION_STATIC(show_kick_users, ShowState(kState_KickUsers))
    HANDLE_ACTION_STATIC(kick_user, KickUser(_msg->Int(2)))
    HANDLE_ACTION_STATIC(show_mute_users, ShowState(kState_MuteUsers))
    HANDLE_ACTION_STATIC(toggle_mute_user, ToggleMuteUser(_msg->Int(2)))
    HANDLE_ACTION_STATIC(show_gamercard_users, ShowState(kState_GamercardUsers))
    HANDLE_ACTION_STATIC(view_user_gamercard, ViewUserGamercard(_msg->Int(2)))
    HANDLE_ACTION_STATIC(show_invite_friends, ShowState(kState_InviteFriends))
    HANDLE_ACTION_STATIC(invite_friend, InviteFriend(_msg->Int(2)))
    HANDLE_ACTION_STATIC(toggle_vocal_style, ToggleVocalStyle())
    HANDLE_ACTION_STATIC(toggle_lefty_flip, ToggleLeftyFlip())
    HANDLE_ACTION_STATIC(show_choose_profile, ShowState(kState_ChooseProfile))
    HANDLE_ACTION_STATIC(attempt_swap_user_profile, AttemptSwapUserProfile(_msg->Int(2)))
    HANDLE_ACTION_STATIC(confirm_swap_user_profile, ConfirmSwapUserProfile())
    HANDLE_ACTION_STATIC(show_modifiers, ShowState(kState_Modifiers))
    HANDLE_ACTION_STATIC(show_modifiers_drum_warning, ShowState(kState_ModifiersDrumWarning))
    HANDLE_ACTION_STATIC(show_modifier_unlock, ShowState(kState_ModifierUnlock))
    HANDLE_ACTION_STATIC(show_modifier_delayed_effect, ShowState(kState_ModifierDelayedEffect))
    HANDLE_ACTION_STATIC(show_toggle_no_fail_denial, ShowState(kState_ToggleNoFailDenial))
    HANDLE_EXPR_STATIC(is_unlocking_modifiers, GetState()->GetStateID() == kState_ModifierUnlock)
    HANDLE_ACTION_STATIC(attempt_toggle_auto_vocals, AttemptToggleAutoVocals())
    HANDLE_ACTION_STATIC(update_part_select_list, UpdatePartSelectList())
    HANDLE_EXPR_STATIC(is_cymbal_selected, IsCymbalSelected(_msg->Sym(2)))
    HANDLE_ACTION_STATIC(confirm_choose_diff, ConfirmChooseDiff())
    HANDLE_ACTION_STATIC(leave_difficulty_confirmation, LeaveDifficultyConfirmation())
    HANDLE_ACTION_STATIC(confirm_kick, ConfirmKick())
    HANDLE_ACTION_STATIC(leave_kick_confirmation, LeaveKickConfirmation())
    HANDLE_EXPR_STATIC(is_in_track_mode, mOvershellDir->Property("in_track_mode", true)->Int())
    HANDLE_ACTION_STATIC(reset_slot_cam, ResetSlotCamera())
    HANDLE_ACTION_STATIC(show_chord_book, ShowChordBook())
    HANDLE_ACTION_STATIC(practice_new_section, PracticeNewSection())
    HANDLE_ACTION_STATIC(toggle_metronome, ToggleMetronome())
    HANDLE_EXPR_STATIC(character_name, mCharForEdit ? mCharForEdit->GetCharacterName() : 0)
    HANDLE_ACTION_STATIC(rename_character, RenameCharacter(_msg->Str(2)))
    HANDLE_ACTION_STATIC(delete_character, DeleteCharacter())
    HANDLE_ACTION_STATIC(refresh_cymbal_provider, mCymbalProvider->ReloadData())
    HANDLE_EXPR_STATIC(can_edit_character, CanEditCharacter(_msg->Int(2)))
    HANDLE_EXPR_STATIC(is_quit_token, IsQuitToken(_msg->Sym(2)))
    HANDLE_ACTION_STATIC(update_mute_users_list, UpdateMuteUsersList())
    HANDLE_ACTION_STATIC(update_kick_users_list, UpdateKickUsersList())
    HANDLE_ACTION_STATIC(update_gamercard_users_list, UpdateGamercardUsersList())
    HANDLE_ACTION_STATIC(update_profiles_list, UpdateProfilesList())
    HANDLE_EXPR_STATIC(can_change_synapse_option, CanChangeSynapseOption())
#ifdef HX_NATIVE
    ...
    HANDLE_ACTION_STATIC(set_wiiprofile_list_mode, SetWiiProfileListMode(_msg->Int(2), false))
    HANDLE_EXPR_STATIC(get_wiiprofile_list_mode, GetWiiProfileListMode())
    HANDLE_EXPR_STATIC(get_wiiprofile_last_index, GetWiiProfileLastIndex())
    HANDLE_EXPR_STATIC(is_wiiprofile_full, IsWiiProfileFull())
    HANDLE_EXPR_STATIC(is_wiiprofile_delete_queue_full, IsWiiProfileDeleteQueueFull())
    HANDLE_EXPR_STATIC(get_wiiprofile_list_selected_name, GetWiiProfileListSelectedName())
    HANDLE_ACTION_STATIC(show_wiiprofile_list, ShowWiiProfileList(_msg->Int(2)))
    HANDLE_ACTION_STATIC(act_on_user_profile, ActOnUserProfile(_msg->Int(2)))
    HANDLE_ACTION_STATIC(act_on_user_profile_confirm, ActOnUserProfileConfirm())
    HANDLE_ACTION_STATIC(show_enter_wiiprofile, ShowEnterWiiProfile())
    HANDLE_ACTION_STATIC(show_register_wiiprofile, ShowEnterWiiProfile())
    HANDLE_ACTION_STATIC(show_switch_wiiprofile, ShowEnterWiiProfile())
    HANDLE_ACTION_STATIC(show_create_wiiprofile, ShowEnterWiiProfile())
    HANDLE_ACTION_STATIC(show_delete_wiiprofile, ShowEnterWiiProfile())
    HANDLE_EXPR_STATIC(get_user_wiiprofile, GetUserWiiProfile())
    HANDLE_ACTION_STATIC(toggle_wii_speak, ToggleWiiSpeak())
    HANDLE_ACTION_STATIC(show_wiiprofile_confirm, ShowWiiProfileConfirm())
    HANDLE_ACTION_STATIC(show_wiiprofile_options, ShowWiiProfileOptions())
    HANDLE_ACTION_STATIC(show_wiiprofile_fail, ShowWiiProfileFail())
    HANDLE_ACTION_STATIC(show_wiiprofile_fail_busy, ShowWiiProfileFailBusy())
    HANDLE_ACTION_STATIC(show_wiiprofile_swap_fail, ShowWiiProfileSwapFail())
    HANDLE_ACTION_STATIC(show_wait_wii, ShowWaitWii())
    HANDLE_ACTION_STATIC(leave_wait_wii, LeaveWaitWii())
    HANDLE_ACTION_STATIC(show_choose_invitation, ShowState(kState_ChooseInvitation))
    HANDLE_ACTION_STATIC(show_accept_invitation, ShowState(kState_AcceptInvitation))
    HANDLE_ACTION_STATIC(show_no_invitations, ShowState(kState_NoInvitations))
    HANDLE_ACTION_STATIC(show_wii_profile_selector, ShowWiiProfileSelector(false))
    HANDLE_ACTION_STATIC(cancel_wii_profile_selector, CancelWiiProfileSelector())
#endif
