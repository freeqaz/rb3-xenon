...
Leaderboard::Leaderboard(EntityID id, Callback *cb)
    : unk40(0), mHasStats(0), mMode(kFriends), mEntityID(id), mCallback(cb),
      mEnumState(kEnumInactive), mSelfColor(0), mFriendColor(0), unk80(-1) {
...
UIColor *
Leaderboard::SlotColorOverride(int, int data, UIListWidget *, UIColor *color) const {
...
bool Leaderboard::EnumerateHigherRankRange() {
    if (mEnumState != kEnumDone && mEnumState != kEnumInactive)
        return false;
    else if (mMode)
        return false;
    else {
...
void Leaderboard::ShowData() {
    ...
    if (mCallback)
        mCallback->ResultSuccess(b1, b2, b3);
}
...
LeaderboardShortcutProvider::LeaderboardShortcutProvider(const Leaderboard *lb)
    : mLeaderboard(lb) {}

void LeaderboardShortcutProvider::Text(int, int data, UIListLabel *slot, UILabel *label)
    const {
