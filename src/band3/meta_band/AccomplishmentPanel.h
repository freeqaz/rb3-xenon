...
public:
    ...
    int GoalIndex(Symbol s) const {
        int idx = 0;
        for (std::vector<Symbol>::const_iterator it = mGoals.begin(); it != mGoals.end();
             ++it, idx++) {
            if (s == *it)
                return idx;
        }
    ...
    void Update(Symbol s) {
        ...
        std::set<Symbol> *symset =
            TheAccomplishmentMgr->GetAccomplishmentSetForCategory(s);
        if (symset) {
            for (std::set<Symbol>::iterator it = symset->begin(); it != symset->end();
                 ++it) {
                Symbol key = *it;
                if (TheAccomplishmentMgr->IsAvailableToView(key))
                    mGoals.push_back(key);
            }
            std::stable_sort(
                mGoals.begin(), mGoals.end(), AccomplishmentCmp(TheAccomplishmentMgr)
            );
        }
    }
...
public:
    ...
    int GroupIndex(Symbol s) const {
        int idx = 0;
        for (std::vector<Symbol>::const_iterator it = mGroups.begin();
             it != mGroups.end();
             ++it, idx++) {
            if (s == *it)
                return idx;
    ...
    void Update() {
        ...
        for (std::map<Symbol, AccomplishmentGroup *>::const_iterator it = groups.begin();
             ...
             ++it) {
            Symbol key = it->first;
            if (TheAccomplishmentMgr->GetNumAccomplishmentsInGroup(key) > 0)
                mGroups.push_back(key);
        }
        std::stable_sort(
            mGroups.begin(), mGroups.end(), AccomplishmentGroupCmp(TheAccomplishmentMgr)
        );
    }
...
public:
    ...
    int CategoryIndex(Symbol s) const {
        int idx = 0;
        for (std::vector<Symbol>::const_iterator it = mCategories.begin();
             it != mCategories.end();
             ++it, idx++) {
            if (s == *it)
                return idx;
