...
void TexLoadPanel::PollForLoading() {
    ...
    case 2:
        ...
        if ((unsigned int)mCurrentFinalizingTexture >= mTexs.size())
            unk3c = 3;
        break;
    default:
        break;
...
void TexLoadPanel::ContentMounted(const char *c1, const char *c2) {
    ...
    if (dlc) {
        ...
        if (name == c1)
            ...
            MILO_WARN("Someone else is mounting %s", c1);
    }
}
...
DLCTex *TexLoadPanel::NextDLCTex() {
    for (std::vector<DynamicTex *>::iterator it = mTexs.begin(); it != mTexs.end();
         ++it) {
...
bool TexLoadPanel::TexturesLoaded() const {
    for (std::vector<DynamicTex *>::const_iterator it = mTexs.begin(); it != mTexs.end();
         ++it) {
        DynamicTex *tex = *it;
