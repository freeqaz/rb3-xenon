...
void SessionMgr::Disconnect() { mSession->Disconnect(); }

bool SessionMgr::IsOnlineEnabled() const { return mSession->IsOnlineEnabled(); }

bool SessionMgr::IsLocal() const { return mSession->IsLocal(); }
...
void SessionMgr::SendMsg(
    const std::vector<RemoteBandUser *> &remotebandusers, NetMessage &msg, PacketType ty
) {
...
BandUser *SessionMgr::GetLeaderUser() const {
    if (mSession && mSession->IsJoining())
        return nullptr;
    else
        return mUserLeader;
}
...
void SessionMgr::ClearLeader() { SetLeaderUser(0); }
...
bool SessionMgr::IsLeaderLocal() const {
    if (mBandUserMgr->GetNumParticipants() == 0)
        return true;
...
bool SessionMgr::HasUser(const User *user) const { return mSession->HasUser(user); }

bool SessionMgr::IsBusy() const { return mSession->IsBusy(); }

int SessionMgr::NumOpenSlots() const { return mSession->NumOpenSlots(); }
...
DataNode SessionMgr::OnMsg(const RemoteUserLeftMsg &msg) {
    RemoteBandUser *user = BandUserMgr::GetRemoteBandUser(msg.GetUser());
    std::vector<RemoteBandUser *>::iterator it =
        std::find(mWaitingUsers.begin(), mWaitingUsers.end(), user);
...
void SessionMgr::ClearWaitingUsers() { mWaitingUsers.clear(); }
