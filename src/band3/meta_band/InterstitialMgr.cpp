...
void InterstitialMgr::GetInterstitialsFromScreen(
    UIScreen *screen, std::vector<UIPanel *> &panels
) {
    panels.clear();
        ...
        for (int i = 1; i < val->Size(); i++) {
            ...
            if (arr->Int(0)) {
                ...
                if (a10size) {
                    ...
                    UIPanel *panel = ObjectDir::Main()->Find<UIPanel>(
                        MakeString("%s_panel", str), true
                    );
                    UIScreen *foundscreen = ObjectDir::Main()->Find<UIScreen>(
                        MakeString("%s_screen", str), false
                    );
...
void InterstitialMgr::SyncLoad(BinStream &bs, unsigned int) { bs >> mRandomSelection; }

bool InterstitialMgr::HasSyncPermission() const { return IsLeaderLocal(); }

#ifdef HX_NATIVE
// Retail stripped the vignette-overlay debug display (BandUI::WriteToVignetteOverlay
// and the whole mVignetteOverlay path are HX_NATIVE-only); PrintOverlay exists only
// to feed that overlay, so it is absent from the retail X360 binary.
