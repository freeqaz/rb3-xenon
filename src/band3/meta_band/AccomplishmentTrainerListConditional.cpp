...
AccomplishmentTrainerListConditional::AccomplishmentTrainerListConditional(
    ...
    : AccomplishmentTrainerConditional(arr, i) {
    Configure(arr);
}
...
AccomplishmentType AccomplishmentTrainerListConditional::GetType() const {
    return kAccomplishmentTypeTrainerListConditional;
}

bool AccomplishmentTrainerListConditional::IsFulfilled(BandProfile *profile) const {
    for (std::vector<Symbol>::const_iterator it = mLessons.begin(); it != mLessons.end();
         ++it) {
        if (!CheckConditionsForLesson(profile, *it))
            return false;
    }
    return true;
}

int AccomplishmentTrainerListConditional::GetNumCompletedLessons(BandProfile *profile
) const {
    int num = 0;
    for (std::vector<Symbol>::const_iterator it = mLessons.begin(); it != mLessons.end();
         ++it) {
...
bool AccomplishmentTrainerListConditional::InqIncrementalSymbols(
