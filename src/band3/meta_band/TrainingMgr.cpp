...
TrainingMgr::TrainingMgr()
    : mUser(0), mMinimumDifficulty(kDifficultyEasy), mReturnScreen(gNullStr),
      mQuitToken(gNullStr), mCurrentLesson(1) {
...
BEGIN_HANDLERS(TrainingMgr)
    ...
    HANDLE_EXPR_STATIC(get_user, mUser)
    HANDLE_ACTION_STATIC(unparticipate_users, UnparticipateUsers())
    HANDLE_ACTION_STATIC(participate_users, ParticipateUsers())
    HANDLE_ACTION_STATIC(set_track_type, SetTrackType((TrackType)_msg->Int(2)))
    HANDLE_ACTION_STATIC(set_preferred_score_type, SetPreferredScoreType((ScoreType)_msg->Int(2)))
    HANDLE_ACTION_STATIC(set_difficulty, SetDifficulty((Difficulty)_msg->Int(2)))
    HANDLE_ACTION_STATIC(set_minimum_difficulty, SetMinimumDifficulty((Difficulty)_msg->Int(2)))
    HANDLE_EXPR_STATIC(get_minimum_difficulty, mMinimumDifficulty)
    HANDLE_ACTION_STATIC(set_return_info, SetReturnInfo(_msg->Sym(2), _msg->Sym(3)))
    HANDLE_EXPR_STATIC(get_return_screen, mReturnScreen)
    HANDLE_EXPR_STATIC(get_quit_token, mQuitToken)
    HANDLE_ACTION_STATIC(set_current_lesson, SetCurrentLesson(_msg->Int(2)))
    HANDLE_EXPR_STATIC(get_current_lesson, mCurrentLesson)
    HANDLE_ACTION_STATIC(clear_current_lesson, ClearCurrentLesson())
    HANDLE_EXPR_STATIC(get_mode_from_lesson_name, GetModeFromLessonName(_msg->Sym(2)))
    HANDLE_EXPR_STATIC(get_mode_from_track_type, GetModeFromTrackType((TrackType)_msg->Int(2)))
    HANDLE_EXPR_STATIC(get_track_type_from_lesson_name, GetTrackTypeFromLessonName(_msg->Sym(2)))
    HANDLE_EXPR_STATIC(get_song_from_lesson_name, GetSongFromLessonName(_msg->Sym(2)))
    HANDLE_EXPR_STATIC(get_difficulty_from_lesson_name, GetDifficultyFromLessonName(_msg->Sym(2)))
    HANDLE_CHECK(0x102)
END_HANDLERS
