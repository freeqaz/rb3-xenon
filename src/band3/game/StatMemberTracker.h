...
public:
    ...
    virtual float GetStatValue(const Stats &stats) const {
        return stats.mUnisonPhraseCompleted;
    }
...
public:
    ...
    virtual float GetStatValue(const Stats &stats) const {
        return stats.mHopoGemCount > 0
            ? (int)(((float)stats.mHopoGemsHopoed / (float)stats.mHopoGemCount) * 100.0f)
            : -1.0f;
    }
    virtual Symbol GetGoalValueSymbol() const { return ""; }
    virtual Symbol GetCurrentValueSymbol() const { return ""; }
...
public:
    UpstrumPercentStatMemberTracker(
        TrackerSource *src, TrackerBandDisplay &banddisp, TrackerBroadcastDisplay &bcdisp
    )
        : StatMemberTracker(src, banddisp, bcdisp) {}
...
public:
    SoloButtonedSoloStatMemberTracker(
        TrackerSource *src, TrackerBandDisplay &banddisp, TrackerBroadcastDisplay &bcdisp
    )
        : StatMemberTracker(src, banddisp, bcdisp) {}
    ...
    virtual bool IsPercentageStat() const { return true; }
    virtual float GetStatValue(const Stats &stats) const {
        return stats.mSoloButtonedSoloPercentage;
    }
    virtual Symbol GetGoalValueSymbol() const { return ""; }
    virtual Symbol GetCurrentValueSymbol() const { return ""; }
    virtual Symbol GetContributionSymbol() const {
