...
void RGTrainerPanel::Swing(int i) {
    if (mLegendMode) {
        if (TheTaskMgr.UISeconds() - unkec < 0.3f)
            return;
        mMatcher.Swing(i, TheTaskMgr.UISeconds() * 1000.0f);
        if (mMatcher.FretMatch(
                mPattern[mLegendGemID % mPattern.size()],
                100.0f,
                TheTaskMgr.UISeconds() * 1000.0f,
                0,
                TheTaskMgr.UISeconds() * 1000.0f,
                false,
                false,
...
Symbol RGTrainerPanel::RGStringToken(int str, bool abbrev) {
    ...
    static Symbol stringTokens[6] = {
        "rg_chordbook_low_e_string", "rg_chordbook_a_string", "rg_chordbook_d_string",
        "rg_chordbook_g_string",     "rg_chordbook_b_string", "rg_chordbook_high_e_string"
    };
    static Symbol stringTokensAbbrev[6] = {
        "rg_chordbook_low_e_short", "rg_chordbook_a_short", "rg_chordbook_d_short",
        "rg_chordbook_g_short",     "rg_chordbook_b_short", "rg_chordbook_high_e_short"
    };
    if (abbrev) {
...
void RGTrainerPanel::SetSongSectionComplete(
    BandProfile *profile, int i1, Difficulty diff, int i2
) {
    for (int i = 0; i <= diff; i++) {
        if (mIsBass) {
            profile->SetProBassSongLessonSectionComplete(
                i1, (Difficulty)i, GetCurrSection()
            );
        } else {
            profile->SetProGuitarSongLessonSectionComplete(
                i1, (Difficulty)i, GetCurrSection()
            );
        }
