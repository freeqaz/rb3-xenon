...
void StatMemberTracker::Poll_(float f) {
    // Retail constructs a function-local static Symbol here (guard bit in
    // 0x82E030D0 + ??0Symbol on 0x82E030CC from "tracker_integer_ratio", plus a
    // ??__F atexit funclet at 0x826D0820). The use site was a MILO_LOG/debug
    // call that retail no-ops away, so only the lazy init survives.
...
void UnisonStatMemberTracker::TranslateRelativeTargets() {
    for (int i = 0; i < mTargets.size(); i++) {
        mTargets[i] = Clamp<float>(0.0f, 1.0f, std::ceil(mTargets[i] * 0.0f));
    }
}
