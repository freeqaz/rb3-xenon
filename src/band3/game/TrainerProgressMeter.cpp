...
TrainerProgressMeter::TrainerProgressMeter()
    : mProgressMeter(0), mBar(0), mGlow(0), mBoxes(0), mCurrent(-1) {}
...
void TrainerProgressMeter::Init(RndDir *progressMeter, int numSections) {
    ...
    RndPropAnim *anim =
        mProgressMeter->Find<RndPropAnim>("progress_meter_config.anim", true);
    if (anim)
        anim->SetFrame(numSections - 1, 1.0f);
    ...
    if (mProgressMeter) {
        ...
        for (int i = 0; i < mCompleted.size(); i++) {
            if (mCompleted[i]) {
                ...
                xfm.v.x = (mBarTrans.v.x - 85.0f) + (float)i * f + f / 2.0f;
        ...
        if (mCurrent >= 0) {
            ...
            xfm.v.x = (mBarTrans.v.x - 85.0f) + (float)mCurrent * f + f / 2.0f;
...
void TrainerProgressMeter::Hide() { mProgressMeter->SetShowing(false); }
