...
// ✅ CLOSED by lane W16-CZ (2026-09-16). INSDEL-3's DIAGNOSIS was right and its
// CONCLUSION was scoped too tightly. It observed that the surplus trailing
// `clrlwi r3,r11,24` is "pure bool materialization (codegen behaviour, not a
// source token)" and deferred -- but that is exactly the point: a `bool` return
// REQUIRES that mask, retail emits NO mask, therefore retail's return type is
// not `bool`. INSDEL-3 held the signature fixed and only ever looked at this
// row; the discriminator is at the CALL SITE, which it never examined.
// ChordbookPanel::PickFretboardView tests the result with `cmpwi r3, 0x0` -- a
// full 32-bit test, which MSVC emits for `int`, never for a `bool` it must
// harden. Two independent rows, one instrument each, same answer.
// The map row 0x826a9f70 carried `_N` (bool) because game map rows are
// generated by tools/gen_game_target_map.py from the rb3-Wii ORACLE, not from
// retail ground truth -- so the signature was never retail evidence at all.
// Changing the return type to `int` requires the map row to change with it
// (?GetFretboardView@GemTrainerPanel@@QBAHABVGameGem@@@Z), or our remangled
// symbol stops pairing.
int GemTrainerPanel::GetFretboardView(const GameGem &gem) const {
