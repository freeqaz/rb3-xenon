...
TrackerManager::TrackerManager(Band *band)
    : unk0(-1), unk4(0), mBandDisplayType(kTrackerBandDisplay_Graphic),
      mBandDisplayStyle((TrackerBandDisplayStyle)0), mBand(band), mTracker(0), unk48(1),
      unk49(0), unk4c(-1.0f) {}

TrackerManager::~TrackerManager() { RELEASE(mTracker); }

void TrackerManager::ForceStars(int stars) { unk0 = stars; }
...
void TrackerManager::Poll(float f) {
    if (mTracker) {
        ...
        if (level != newlevel)
            mTracker->ReachedTargetLevel(newlevel);
        if (!unk49 && mBand) {
...
void TrackerManager::OnPlayerAddEnergy(Player *p, float f) {
    if (mTracker) {
        ...
        if (ft)
            ft->PlayerAddEnergy(p, f);
    }
}

void TrackerManager::OnPlayerSaved(Player *p) {
    if (mTracker)
        mTracker->HandlePlayerSaved(p);
}

void TrackerManager::OnPlayerQuarantined(Player *p) { HandleRemovePlayer(p); }

void TrackerManager::OnRemoteTrackerFocus(Player *p, int i1, int i2, int i3) {
    if (mTracker) {
        FocusTracker *ft = dynamic_cast<FocusTracker *>(mTracker);
        if (!ft)
            MILO_FAIL("Non-focus tracker sent focus related net message!");
...
void TrackerManager::OnRemoteTrackerPlayerProgress(Player *p, float f) {
    if (mTracker)
        mTracker->RemoteSetPlayerProgress(p, f);
}
...
void TrackerManager::OnRemoteTrackerSectionComplete(Player *p, int i1, int i2, int i3) {
    if (mTracker) {
        ...
        if (!pst)
            MILO_FAIL(
                "Non-perfect-section tracker sent perfect-section related net message!"
            );
...
void TrackerManager::OnRemoteTrackerPlayerDisplay(Player *p, int i1, int i2, int i3) {
    if (mTracker)
        mTracker->RemoteTrackerPlayerDisplay(p, i1, i2, i3);
}
...
void TrackerManager::OnRemoteTrackerDeploy(Player *p) {
    if (mTracker) {
        ...
        if (!dct)
            MILO_FAIL("Non-deploy tracker sent deploy related net message!");
        dct->RemoteDeploy(p);
    }
}
...
void TrackerManager::OnRemoteTrackerEndDeployStreak(Player *p, int i) {
    if (mTracker) {
        ...
        if (!ot)
            MILO_FAIL("Non-overdrive tracker sent overdrive related net message!");
...
void TrackerManager::OnRemoteTrackerEndStreak(Player *p, int i1, int i2) {
    if (mTracker)
        mTracker->RemoteEndStreak(p, i1, i2);
}
...
TrackerSource *TrackerManager::CreateSource(const TrackerDesc &desc) const {
    ...
    if (!p) {
        ...
        return new PlayerTrackerSource(p);
}
...
Tracker *TrackerManager::MakeTracker(TrackerType ty, TrackerSource *src) {
    ...
    case kTrackerType_SoloButtonedSoloPercentage:
        tracker =
            new SoloButtonedSoloStatMemberTracker(src, mBandDisplay, mBroadcastDisplay);
        break;
    ...
    case kTrackerType_UpstrumPercent:
        tracker =
            new UpstrumPercentStatMemberTracker(src, mBandDisplay, mBroadcastDisplay);
        break;
    default:
        break;
    }
