...
#ifndef HX_NATIVE
extern "C" int NWC24GetMyUserId(unsigned long long &);
#endif
...
// Retail RB3 360 builds each fixed DataPoint key as a function-local static
// Symbol, and — crucially — all of a function's key statics are *declared before*
...
#define DP_KEYS3(a, b, c) DP_KEY(a), b(#b), c(#c);
#define DP_KEYS4(a, b, c, d) DP_KEY(a), b(#b), c(#c), d(#d);
#define DP_KEYS5(a, b, c, d, e) DP_KEY(a), b(#b), c(#c), d(#d), e(#e);

// Insert using a key (a pre-declared static Symbol, or a runtime/temp value).
#define ADD_DATA_PAIR(first, second)                                                     \
    dataPoint.mNameValPairs.insert(std::make_pair(first, second));

// String-literal-key form ("cymbal_config", ...): a uniquely-named local static
// built from the literal (non-pinned RecordOptionData uses these).
#define _ADP_CAT2(a, b) a##b
#define _ADP_CAT(a, b) _ADP_CAT2(a, b)
#define _ADP_NAME _ADP_CAT(_adpSym, __LINE__)
#define ADD_DATA_PAIR_STR(keystr, second)                                                \
    {                                                                                    \
        static Symbol _ADP_NAME(keystr);                                                 \
        dataPoint.mNameValPairs.insert(std::make_pair(_ADP_NAME, second));               \
    }
...
// NOTE (rb3-xenon port): UpdateOnlineStatus / EnumerateFriends / SendMsg are
// Wii friend-presence + messenger glue (WiiFriendMgr / WiiMessenger / the Wii
// int friend-key). Not in the pinned retail-Xbox range; stubbed for compilation.
...
void RockCentral::GetAccLeaderboardByPlayer(
    ...
    Server *server = IsConnected(o, -1, false);
    if (server) {
        DP_KEYS3(acc_id, lb_mode, num_rows)
...
void RockCentral::GetBattleLeaderboardByPlayer(
    ...
    Server *server = IsConnected(o, -1, false);
    if (server) {
        DP_KEYS3(battle_id, lb_mode, num_rows)
...
void RockCentral::GetAccLeaderboardByRankRange(
    ...
    Server *server = IsConnected(o, -1, false);
    if (server) {
        DP_KEYS3(acc_id, start_rank, end_rank)
...
void RockCentral::GetBattleLeaderboardByRankRange(
    ...
    Server *server = IsConnected(o, -1, false);
    if (server) {
        DP_KEYS3(battle_id, start_rank, end_rank)
...
void RockCentral::UpdateFriendList(
    ...
    if (IsConnected(o, -1, false)) {
        ...
        for (int i = 0; i < _tmp5; i++) {
            ...
            // runtime key (buf): build the Symbol from the runtime char* — not a
            // static — so use the raw insert rather than the static-key macro.
