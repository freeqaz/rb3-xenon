...
void TourPropertyCollection::FakeFill() {
    std::hash_map<Symbol, TourProperty *>::iterator end = TheTour->m_mapTourProperties.end();
    std::hash_map<Symbol, TourProperty *>::iterator it = TheTour->m_mapTourProperties.begin();
    for (; it != end; ++it) {
        Symbol key = it->first;
