...
GemSmasher::GemSmasher(int slot, RndDir *dir, bool keys)
    : mSlot(slot), mDir(dir), mHitSmashTrig(NULL), mHitDrumsTrig(NULL),
      mHitDrumsBonus(NULL), mHitCymsTrig(NULL), mHitCymsBonus(NULL), mHitChordTrig(NULL),
      mHitChordBonus(NULL), mHitBonus(NULL), mBREHitTrig(NULL), mBREHitChord(NULL),
    ...
    if (mDir) {
        ...
        if (mHitSmashTrig == NULL)
            mHitSmashTrig = mDir->Find<EventTrigger>("smash.trig", false);
        ...
        EventTrigger *hitFillLight =
            mDir->Find<EventTrigger>("hit_fill_light.trig", false);
        if (hitFillLight) {
            EventTrigger *hitFillMediumTrig, *hitFillHardTrig;
            hitFillMediumTrig =
                (EventTrigger *)mDir->Find<EventTrigger>("hit_fill_medium.trig", false);
            hitFillHardTrig =
...
void GemSmasher::HitDrum() {
    if (mHitDrumsTrig)
        mHitDrumsTrig->Trigger();
}
void GemSmasher::HitDrumBonus() {
    if (mHitDrumsBonus)
        mHitDrumsBonus->Trigger();
}
void GemSmasher::HitCymbal() {
    if (mHitCymsTrig)
        mHitCymsTrig->Trigger();
}
void GemSmasher::HitCymbalBonus() {
    if (mHitCymsBonus)
        mHitCymsBonus->Trigger();
}
void GemSmasher::HitChord() {
    if (mHitChordTrig)
        mHitChordTrig->Trigger();
}
void GemSmasher::HitChordBonus() {
    if (mHitChordBonus)
        mHitChordBonus->Trigger();
    else
        HitBonus();
}
void GemSmasher::HitBonus() {
    if (mHitBonus)
        mHitBonus->Trigger();
    else
        Hit();
}
void GemSmasher::CodaHit() {
    if (mBREHitTrig)
        mBREHitTrig->Trigger();
    else
        Hit();
}
void GemSmasher::CodaHitChord() {
    if (mBREHitChord)
        mBREHitChord->Trigger();
    else
        HitChord();
}
...
void GemSmasher::StopBurn() {
    if (mBurning && mStopBurnTrig) {
        mStopBurnTrig->Trigger();
        mBurning = false;
    }
}
