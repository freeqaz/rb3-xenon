...
LyricPlate::LyricPlate(RndText *t1, const RndText *t2, const RndText *t3)
    ...
    mText->SetText("");
    mPitchedStyle = t2->GetSingleStyle();
    mUnpitchedStyle = t3->GetSingleStyle();
...
float LyricPlate::CurrentStartX(float start) const {
    return start + mText->LocalXfm().v.x;
}

float LyricPlate::CurrentEndX(float end) const {
    return end + (mWidthX + mText->LocalXfm().v.x);
}
...
void LyricPlate::BakeLyric(Lyric *lyric) {
    ...
    float f7 = lyric->mBeginPos.x - mText->LocalXfm().v.x;
    float fsub = lyric->mBeginPos.z - mText->LocalXfm().v.z;
    lyric->mXWidth = 0;
    Transform tf68;
    tf68.Reset();
