...
def cmd_bsf_solve(args) -> None:
    ...
    objdiff_result = subprocess.run(
        [
            ...
            "diff",
            symbol,
            # No `--build --incremental`: that pair is `ninja <one .obj>`,
            # which stops one edge short of the six post-compile patchers.
            # _ensure_patched() above built through `post-compile` and
            # asserted the manifest instead.
