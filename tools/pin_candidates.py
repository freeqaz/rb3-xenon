...
def rank(args):
    ...
    for path in args.oracles:
        if not os.path.isfile(path):
            ...
            continue
        # dead-index guard (lane BX-4): a dead oracle here would propose PIN
        # SPANS from noise. Skip loudly; if every oracle is dead the run ends
        # with zero candidates and an explicit reason (checked below).
        if _dead_skip(path, what=f"pin oracle {os.path.basename(path)}"):
            continue
