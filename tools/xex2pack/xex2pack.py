...
def synthesize_import_block(pe, pe_info, import_map):
    ...
    for mi, m in enumerate(modules):
        ...
        th = be32(table_size)
        th += b'\x00' * 0x14                 # NextImportDigest (zeroed, unsigned)
...
def build_page_descriptors(pe_data):
    """One descriptor per 64 KiB page. info nibble: page0=readonly(3), pages w/ code chars=code(1),
    else data(2). We classify from the PE section table when available, else default code/data."""
    n_pages = align(len(pe_data), PAGE_SIZE) // PAGE_SIZE
    infos = [SEC_DATA] * n_pages
    infos[0] = SEC_READONLY  # header page

    # classify pages from PE sections (best-effort; digests are zeroed regardless)
    try:
    ...
    out = b''
    for info in infos:
        be_val = (info & 0xF) | (1 << 4)  # info nibble | pageCount=1
        out += be32(be_val) + (b'\x00' * 0x14)
    return out, n_pages
...
def pack(pe_data, out_path, entry_point, base_addr, import_block, pe_name,
    ...
    page_desc_bytes, _ = build_page_descriptors(pe_data)
...
def main():
    ...
    if args.import_map:
        ...
        pe_info = parse_pe(pe_data)
        image = bytearray(map_pe_to_image(pe_data, pe_info))
        import_block, rewrites, summary = synthesize_import_block(pe_data, pe_info, import_map)
