...
//===----------------------------------------------------------------------===//
//
// DC3 native port: patched libc++ __wrap_iter
// Adds implicit conversion to underlying pointer type, matching MSVC's old STL
// behavior where vector::begin()/end() return raw pointers.
// This is the libc++ counterpart of bits/stl_iterator.h (libstdc++ shim).
//
// Based on Emscripten 5.0.2 libc++ __iterator/wrap_iter.h
//
...
public:
  ...
  // DC3 native port: implicit conversion to underlying pointer type.
  // Xbox 360 MSVC's STL returns raw pointers from vector::begin()/end().
  // This allows code like `T** it = vec.begin()` to compile.
  _LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX14 operator iterator_type() const _NOEXCEPT { return __i_; }
