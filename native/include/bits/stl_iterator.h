// Iterators -*- C++ -*-
// PATCHED COPY for DC3 native port (GCC 15.2.1)
// Adds implicit conversion operator to __normal_iterator (line ~1162)
// to match MSVC's old STL where vector::begin() returns T*.
...
# endif
	...
	return *this;
      ...
      // MSVC's old STL used raw T* as vector::iterator; modern libstdc++
      // wraps it in __normal_iterator. This restores implicit T* conversion
      // for 600+ call sites that depend on MSVC behavior.
      __attribute__((__always_inline__))
      _GLIBCXX_CONSTEXPR
