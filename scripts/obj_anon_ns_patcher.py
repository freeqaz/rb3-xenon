...
def process_batch(args):
    ...
    for relpath, decomp_path in sorted(decomp_by_relpath.items()):
        ...
        # Resolve the target .obj for this compiled .obj. Identical relpaths
        # first (free, and unambiguous when the splits heading carries a path),
        # then the authoritative objdiff.json pairing, which is what actually
        # covers the bare-declared units.
        if relpath in orig_by_relpath:
            orig_relpath = relpath
        elif config_pairs.get(relpath) in orig_by_relpath:
            orig_relpath = config_pairs[relpath]
        ...
        # Get original hashes
        if orig_relpath not in orig_hash_cache:
            if args.verbose:
        ...
        orig_hash_set = orig_hash_cache[orig_relpath]
